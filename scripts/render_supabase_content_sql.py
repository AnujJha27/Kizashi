...
def item_sql(item: dict[str, Any], category: str) -> list[str]:
    ...
    specialized = {
        ...
        "kanji": "insert into public.kanji (item_id, character, meanings, onyomi, kunyomi, stroke_count, grade, radical, nanori, components, confusable_kanji, mnemonic, stroke_order, useful_words) values (" + ", ".join([
            sql_text(item_id), sql_text(item.get("character")), sql_array(item.get("meanings")), sql_array(item.get("onyomi")), sql_array(item.get("kunyomi")), sql_int(item.get("strokeCount")), sql_int(item.get("grade")), sql_nullable_text(item.get("radical")), sql_array(item.get("nanori")), sql_array(item.get("components")), sql_array(item.get("confusableKanji")), sql_nullable_text(item.get("mnemonic")), sql_nullable_text(item.get("strokeOrder")), sql_json(item.get("usefulWords")),
        ]) + ") on conflict (item_id) do update set character = excluded.character, meanings = excluded.meanings, onyomi = excluded.onyomi, kunyomi = excluded.kunyomi, stroke_count = excluded.stroke_count, grade = excluded.grade, radical = excluded.radical, nanori = excluded.nanori, components = excluded.components, confusable_kanji = excluded.confusable_kanji, mnemonic = excluded.mnemonic, stroke_order = excluded.stroke_order, useful_words = excluded.useful_words;",
        "grammar": "insert into public.grammar_points (item_id, pattern, meaning, formation, intuition, usage_conditions, examples, common_mistakes, contrast_ids, practice_question_ids, aliases, context) values (" + ", ".join([
            sql_text(item_id), sql_text(item.get("pattern")), sql_text(item.get("meaning")), sql_text(item.get("formation")), sql_text(item.get("intuition")), sql_array(item.get("usageConditions")), sql_json(item.get("examples")), sql_array(item.get("commonMistakes")), sql_array(item.get("contrastIds")), sql_array(item.get("practiceQuestionIds")), sql_array(item.get("aliases")), sql_json_object(item.get("context")),
        ]) + ") on conflict (item_id) do update set pattern = excluded.pattern, meaning = excluded.meaning, formation = excluded.formation, intuition = excluded.intuition, usage_conditions = excluded.usage_conditions, examples = excluded.examples, common_mistakes = excluded.common_mistakes, contrast_ids = excluded.contrast_ids, practice_question_ids = excluded.practice_question_ids, aliases = excluded.aliases, context = excluded.context;",
        "reading": "insert into public.readings (item_id, title, passage, translation, vocabulary_ids, grammar_ids, kanji_ids, estimated_difficulty, visual_format, questions) values (" + ", ".join([
            sql_text(item_id), sql_text(item.get("title")), sql_text(item.get("passage")), sql_text(item.get("translation")), sql_array(item.get("vocabularyIds")), sql_array(item.get("grammarIds")), sql_array(item.get("kanjiIds")), sql_int(item.get("estimatedDifficulty") if isinstance(item.get("estimatedDifficulty"), int) else 1), sql_nullable_text(item.get("visualFormat")), sql_json(item.get("questions")),
        ]) + ") on conflict (item_id) do update set title = excluded.title, passage = excluded.passage, translation = excluded.translation, vocabulary_ids = excluded.vocabulary_ids, grammar_ids = excluded.grammar_ids, kanji_ids = excluded.kanji_ids, estimated_difficulty = excluded.estimated_difficulty, visual_format = excluded.visual_format, questions = excluded.questions;",
