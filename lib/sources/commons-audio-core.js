...
function compatibleLicense(value, acceptedLicenses) {
  const license = normalizeCommonsText(value).toLowerCase();
  if (!license || /unknown|all rights reserved|no license/iu.test(license)) return false;
  if (acceptedLicenses) return acceptedLicenses.some((accepted) => license.includes(String(accepted).toLowerCase()));
  return /public domain|cc0|cc-by|cc by|cc-by-sa|cc by-sa|pdm/iu.test(license);
}

function collectionFor(title, metadata) {
  const category = metadataValue(metadata, "Categories", "Category", "Collection");
  return /lingua libre|ll-q\d+/iu.test(`${title} ${category}`) ? "lingua-libre" : "commons";
}
...
function matchScore(page, searchRanks, text, reading) {
  ...
  const searchRank = searchRanks.get(page.title) ?? 0;
  const score = exactTextLabel ? 100 : exactReadingLabel ? 90 : exactTextFilename ? 80 : exactReadingFilename ? 70 : searchRank ? 10 + searchRank : 0;
...
function resultFrom(match, text) {
  ...
  const speaker = metadataValue(metadata, "Artist", "Creator") || (typeof info.user === "string" ? info.user : "");
  const attribution = metadataValue(metadata, "Credit", "Artist", "Attribution") || speaker;
  ...
  return {
    ...
    filePage,
    label: text,
    ...(speaker ? { speaker, speakerId: typeof info.user === "string" && info.user.trim() ? info.user.trim() : speaker } : {}),
    license,
