...
function orderFor(items, mode, knownIds, { mistakes = {}, sourceProgress = {}, targetSkill = "" } = {}) {
  return [...items].sort((left, right) => {
    ...
    const score = mode === "guided"
      ? rightWeakness - leftWeakness || rightCoverageRatio - leftCoverageRatio || Number(leftNatural) - Number(rightNatural) || (left.difficulty ?? 3) - (right.difficulty ?? 3) || leftOpened - rightOpened
      : mode === "immersion"
        ? Number(rightNatural) - Number(leftNatural) || rightWeakness - leftWeakness || (right.difficulty ?? 3) - (left.difficulty ?? 3) || leftOpened - rightOpened
...
export function selectImmersionClips(items = [], mode = "guided", knownIds = new Set(), limit = 3, options = {}) {
  return orderFor(items.filter((item) => item?.category === "listening" || item?.transcript), mode, knownIds, options).slice(0, Math.max(0, limit));
}
