...
import { interestScore } from "./interest-core.js";
...
function orderFor(items, mode, knownIds, { mistakes = {}, sourceProgress = {}, targetSkill = "", coverageBand = [0.8, 0.95], interests = [] } = {}) {
  return [...items].sort((left, right) => {
    ...
    const leftInterest = interestScore(left, interests);
    const rightInterest = interestScore(right, interests);
    ...
    const score = mode === "guided"
      ? rightWeakness - leftWeakness || rightInterest - leftInterest || rightBand - leftBand || bandDistance(leftCoverageRatio) - bandDistance(rightCoverageRatio) || Number(leftNatural) - Number(rightNatural) || (left.difficulty ?? 3) - (right.difficulty ?? 3) || leftOpened - rightOpened
      : mode === "immersion"
        ? Number(rightNatural) - Number(leftNatural) || rightWeakness - leftWeakness || rightInterest - leftInterest || rightBand - leftBand || bandDistance(leftCoverageRatio) - bandDistance(rightCoverageRatio) || (right.difficulty ?? 3) - (left.difficulty ?? 3) || leftOpened - rightOpened
