function visualAssets(areaId) {
  const path = `/world/${areaId}.svg`;
  return Object.freeze({ shell: path, hero: path, lesson: path, immersion: path, portrait: path });
}

function visualAssetMetadata(area) {
  return Object.freeze(["shell", "hero", "lesson", "immersion", "portrait", "transition"].map((role) => ({
    id: `${area.id}-${role}`,
    areaId: area.id,
    role,
    path: `/world/${area.id}.svg`,
    sourceType: "owned-svg",
    creator: "Kizashi",
    license: "Original repository asset",
    attribution: "Kizashi",
