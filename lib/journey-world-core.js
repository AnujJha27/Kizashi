...
function visualAssets(areaId) {
  ...
  return Object.freeze({ shell: path, hero: path, today: todayVisualPaths[areaId] ?? todayVisualPaths.neighborhood, lesson: "/world/lesson-wide.webp", lessonTracker: "/world/lesson-tracker-portrait.webp", map: "/world/journey-map.webp", immersion: path, portrait: path });
}

function visualAssetMetadata(area) {
  const assets = visualAssets(area.id);
  return Object.freeze(["shell", "hero", "today", "lesson", "lessonTracker", "map", "immersion", "portrait", "transition"].map((role) => ({
    id: `${area.id}-${role}`,
    areaId: area.id,
    role,
