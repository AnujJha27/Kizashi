...
function dialogueStructure(items) {
  ...
  items.forEach((item) => {
    ...
    (item.questions ?? []).forEach((question) => {
      ...
      if (answer && item.transcript?.includes(answer)) answerEchoes += 1;
    });
  });
  return { turnProfiles, speakerPatterns, answerEchoes };
}
...
function questionSignals(items, field) {
  ...
  items.forEach((item) => (item.questions ?? []).forEach((question) => {
    ...
    if (answer && textOf(item, field).includes(answer)) answerEchoes += 1;
  }));
  return { answerEchoes };
}
