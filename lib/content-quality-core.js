...
const readingDiversityFamilies = ["main idea", "sequence", "reason", "reference", "appropriate action", "simple inference", "condition detail", "task-based response"];

function questionFamilyCoverage(items) {
  const underMinimum = (subset) => {
    const counts = questionFamilies(subset);
    return readingDiversityFamilies.filter((family) => (counts[family] ?? 0) < 3).map((family) => ({ family, count: counts[family] ?? 0, minimum: 3 }));
  };
  return { minimum: 3, underMinimum: underMinimum(items), byLevel: Object.fromEntries(["N5", "N4"].map((level) => [level, { underMinimum: underMinimum(items.filter((item) => item.jlptLevel === level)) }])) };
}
...
function visualFormatCounts(items) {
  const result = {};
  items.map((item) => item.visualFormat).filter(Boolean).forEach((format) => { result[format] = (result[format] ?? 0) + 1; });
  return result;
}
...
function linkedTargetLevels(items, catalogs = {}) {
  const definitions = [
    ["vocabulary", "vocabularyIds"],
    ["grammar", "grammarIds"],
    ["kanji", "kanjiIds"],
  ];
  const available = definitions.some(([category]) => Array.isArray(catalogs[category]));
  const unresolved = Object.fromEntries(definitions.map(([category]) => [category, []]));
  if (!available) return { available: false, byReadingLevel: {}, unresolved };
  const indexes = new Map(definitions.map(([category]) => [category, new Map((Array.isArray(catalogs[category]) ? catalogs[category] : []).map((item) => [item.id, item]))]));
  const emptyCounts = () => ({ N5: 0, N4: 0, other: 0, unresolved: 0 });
  const byReadingLevel = Object.fromEntries(["N5", "N4"].map((level) => {
    const levelItems = items.filter((item) => item.jlptLevel === level);
    const summary = Object.fromEntries(definitions.map(([category, field]) => {
      const counts = emptyCounts();
      levelItems.forEach((item) => (item[field] ?? []).forEach((targetId) => {
        const target = indexes.get(category)?.get(targetId);
        if (!target) {
          counts.unresolved += 1;
          unresolved[category].push({ readingId: item.id, targetId });
          return;
        }
        const targetLevel = target.jlptLevel ?? target.level;
        if (targetLevel === "N5" || targetLevel === "N4") counts[targetLevel] += 1;
        else counts.other += 1;
      }));
  ...
  return { available, byReadingLevel, unresolved };
}

function distractorSignals(items, field) {
  ...
  const sourceEchoDetails = [];
  const setsWithSourceEchoDistractors = questions.filter(({ item, question }) => {
    const choices = question.options ?? question.answers ?? [];
    const correctIndex = question.correctIndex ?? question.correctAnswer;
    const source = qualityTemplate(textOf(item, field));
    const distractors = choices
      .filter((_, index) => index !== correctIndex)
      .map((choice) => String(choice ?? "").trim())
      .filter((choice) => qualityTemplate(choice).length >= 2 && source.includes(qualityTemplate(choice)));
    if (distractors.length) sourceEchoDetails.push({ itemId: item.id, questionType: question.questionType || "untyped", distractors });
...
function report(items, field, catalogs) {
  ...
  return {
    ...
    questionFamilyCoverage: field === "passage" ? questionFamilyCoverage(items) : null,
    ...
    visualFormatCounts: visualFormatCounts(items),
    ...
    linkedTargetLevels: field === "passage" ? linkedTargetLevels(items, catalogs) : null,
    distractorSignals: distractorSignals(items, field),
...
/** @param {{ readings?: any[], listening?: any[], vocabulary?: any[], grammar?: any[], kanji?: any[] }} content */
export function buildContentQualityReport(content = {}) {
  ...
  return {
    reading: report(readings, "passage", content),
    listening: report(listening, "transcript", content),
  };
}
