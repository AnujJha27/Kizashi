...
function questionFamilyCoverage(items) {
  const counts = questionFamilies(items);
  return { minimum: 3, underMinimum: readingDiversityFamilies.filter((family) => (counts[family] ?? 0) < 3).map((family) => ({ family, count: counts[family] ?? 0, minimum: 3 })) };
}
