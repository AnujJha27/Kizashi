...
export function deprioritizeFlaggedItems(items, flags = {}) {
  const flagged = (item) => ["vocabulary", "kanji", "grammar"].includes(item.category) && flags[item.id]?.status === "flagged";
  return items
    .map((item, index) => ({ item, index }))
    .sort((left, right) => Number(flagged(left.item)) - Number(flagged(right.item)) || left.index - right.index)
    .map(({ item }) => item);
}
