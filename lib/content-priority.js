...
export function getItemPriority(item, records = {}, mistakes = {}) {
  ...
  const frequency = Math.min(20, (Number.isFinite(Number(item.frequency)) && Number(item.frequency) > 0 ? Math.log10(Number(item.frequency) + 1) * 3 : 0) + Math.max(0, Math.min(5, Number(item.commonness ?? 0))));
  ...
  const reason = weakness ? `Learner weakness · ${mistake?.count ?? 0} misses / ${Math.round(accuracy * 100)}% accuracy` : missing.length ? `Needs enrichment · ${missing.join(", ")}` : frequency ? `${itemBand(item)} · frequent beginner item` : `${itemBand(item)} curriculum candidate`;
  return { score, reason };
}
