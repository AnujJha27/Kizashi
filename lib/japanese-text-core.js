...
export function getUnresolvedJapaneseSegments(text, entries = []) {
  let offset = 0;
  return segmentJapaneseText(text, entries).flatMap((segment) => {
    const result = segment.status === "unresolved" ? [{ ...segment, offset }] : [];
