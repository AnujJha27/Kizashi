...
function registerLine(transcript) {
  ...
  const casual = lines.find((line) => /(?:て|ない|だ|だよ|だね|かな)[。！？]?$/u.test(line));
...
function buildRegister(listening) {
  const candidates = listening.filter((item) => item.jlptLevel === "N5").map((item) => {
    const match = registerLine(item.transcript);
    return match ? { item, ...match } : null;
  }).filter(Boolean);
  const casual = candidates.filter((entry) => entry.register === "casual").slice(0, 10);
  const polite = candidates.filter((entry) => entry.register === "polite").slice(0, 10);
