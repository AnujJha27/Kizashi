...
function registerLine(transcript) {
  const lines = String(transcript ?? "").split("\n").map((line) => line.trim()).filter(Boolean);
  const casual = lines.find((line) => /(?:て|で|ない|だ|だよ|だね|かな)[。！？]?$/u.test(line));
  if (casual) return { line: casual, register: "casual" };
  const polite = lines.find((line) => /(?:です|ます|ください|ませんか|ましょう)[。！？]?$/u.test(line));
  return polite ? { line: polite, register: "polite" } : null;
}

function buildRegister(listening) {
  return Object.entries({ N5: 10, N4: 10 }).flatMap(([level, limit]) => {
    const candidates = listening.filter((item) => item.jlptLevel === level).map((item) => {
      const match = registerLine(item.transcript);
      return match ? { item, ...match } : null;
    }).filter(Boolean);
    const casual = candidates.filter((entry) => entry.register === "casual").slice(0, limit);
    const polite = candidates.filter((entry) => entry.register === "polite").slice(0, limit);
    return [...casual, ...polite].map(({ item, line, register }) => ({
      id: `register-${item.id}`,
      kind: "register",
      level,
      title: item.title,
      japanese: line,
      prompt: "Is this line casual or polite? Notice the ending and the relationship between speakers.",
      model: line,
      reading: line,
      hint: register === "casual" ? "The short て-form here is a casual instruction or request." : "The です／ます／ください ending marks a polite line.",
      choices: ["Casual", "Polite"],
      answer: register === "casual" ? 0 : 1,
...
export function buildOutputBanks(module = {}) {
  return {
    ...
    register: buildRegister(module.listening ?? []),
  };
}
