...
/** @param {{examDate?: string, now?: Date, dueCount?: number, weakCount?: number, recentAccuracy?: number|null, readiness?: string}} options */
export function getAdaptivePlan({ examDate = "", now = new Date(), dueCount = 0, weakCount = 0, recentAccuracy = null, readiness = "untested" } = {}) {
  ...
  let action;
  if (due) action = { key: "review", label: "Review due cards", href: "/review" };
  ...
  const state = daysRemaining === null ? "no-date" : daysRemaining < 0 ? "overdue" : daysRemaining === 0 ? "today" : readiness === "exam-ready" || readiness === "strong" ? "ready" : "active";
