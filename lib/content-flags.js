...
export function setContentCorrection(itemId, category, value, origin) {
  ...
  window.localStorage.setItem(CONTENT_FLAGS_STORAGE_KEY, JSON.stringify(flags));
  window.dispatchEvent(new Event("michi-content-flagged-updated"));
  window.dispatchEvent(new Event("michi-content-correction-updated"));
  return flags[itemId];
}

export function setQuestionCorrection(itemId, options, correctIndex, origin) {
  if (typeof window === "undefined" || !itemId || !Array.isArray(options) || options.length < 2 || !Number.isInteger(correctIndex) || correctIndex < 0 || correctIndex >= options.length) return null;
  const cleanOptions = options.map((option) => String(option ?? "").trim()).filter(Boolean).slice(0, 8);
  if (cleanOptions.length < 2 || new Set(cleanOptions.map((option) => option.normalize("NFKC").toLocaleLowerCase())).size !== cleanOptions.length) return null;
  const flags = readContentFlags();
  const previous = flags[itemId] && typeof flags[itemId] === "object" && !Array.isArray(flags[itemId]) ? flags[itemId] : {};
  const edits = previous.edits && typeof previous.edits === "object" && !Array.isArray(previous.edits) ? previous.edits : {};
  flags[itemId] = { ...previous, itemId, status: "reviewed", reviewedAt: Date.now(), origin, edits: { ...edits, options: cleanOptions, correctIndex: Math.min(correctIndex, cleanOptions.length - 1) } };
  window.localStorage.setItem(CONTENT_FLAGS_STORAGE_KEY, JSON.stringify(flags));
  window.dispatchEvent(new Event("michi-content-flagged-updated"));
  window.dispatchEvent(new Event("michi-content-correction-updated"));
  return flags[itemId];
...
export function applyContentCorrections(module) {
  ...
  const questions = Array.isArray(module.practiceQuestions) ? module.practiceQuestions.map((question) => {
    const edits = flags[question.id]?.edits;
    if (!edits || typeof edits !== "object" || Array.isArray(edits) || !Array.isArray(edits.options) || !edits.options.length) return question;
    return { ...question, options: edits.options, correctIndex: Number.isInteger(edits.correctIndex) ? edits.correctIndex : question.correctIndex };
  }) : module.practiceQuestions;
  return { ...module, vocabulary: correct(module.vocabulary), kanji: correct(module.kanji), grammar: correct(module.grammar), practiceQuestions: questions };
}
