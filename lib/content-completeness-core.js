...
function vocabularyContract(module) {
  ...
  const pending = (item, types) => questions.filter((question) => question.itemId === item.id && types.includes(question.questionType) && !isReadyGrammarQuestion(question)).length;
  ...
  const summarize = (subset) => {
    ...
    const pendingContextualDrafts = subset.reduce((total, item) => total + pending(item, ["contextual vocabulary"]), 0);
    const pendingParaphraseDrafts = subset.reduce((total, item) => total + pending(item, ["paraphrase"]), 0);
    ...
    return { total: subset.length, examplesAtLeast2, collocations, relatedWords, audio, contextualAssessments, paraphraseAssessments, pendingContextualDrafts, pendingParaphraseDrafts, usageAssessments, contractReady };
  };
  return { ...summarize(items), byLevel: Object.fromEntries(["N5", "N4"].map((level) => [level, summarize(items.filter((item) => item.jlptLevel === level))])) };
}
...
export function getContentCompleteness(module = {}) {
  ...
  return {
    ...
    output: Object.fromEntries(["speaking", "writing", "pragmatics", "chunks", "register"].map((kind) => [kind, outputBanks[kind].length])),
