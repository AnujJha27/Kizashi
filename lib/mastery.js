...
export function normalizeAnswer(value, leniency = "kana") {
  const normalized = String(value).normalize("NFKC").trim();
  if (leniency === "strict") return normalized.toLocaleLowerCase();
  return toHiragana(normalized).replace(/\s+/gu, "").toLocaleLowerCase();
}
