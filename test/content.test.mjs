...
test("provisional encounters support in-place review across learner surfaces", async () => {
  ...
  assert.match(flags, /setQuestionCorrection/);
  ...
  assert.match(await readFile(new URL("../components/mistakes/mistake-notebook.tsx", import.meta.url), "utf8"), /editValue=\{item\.category === "grammar"/);
...
test("external sources use native media and safe framing fallbacks", async () => {
  ...
  assert.match(lesson, /className="mx-auto w-full max-w-5xl min-w-0/);
  assert.match(lesson, /className="lesson-layout"/);
  assert.match(lesson, /className="lesson-summary surface-panel min-w-0 h-fit min-h-0 w-full/);
...
test("Studio exposes every pending question through a searchable paged review queue", async () => {
  ...
  assert.match(studio, /averageCharacters/);
...
test("content completeness separates quantity, fields, review, and context coverage", () => {
  ...
  assert.deepEqual(report.output, { speaking: 1, writing: 1, pragmatics: 0, chunks: 1, register: 1 });
...
test("offline worker caches recorded audio without caching arbitrary cross-origin requests", async () => {
  ...
  assert.match(worker, /\/world\/lesson-tracker-portrait\.webp/);
