...
test("the N5 module has meaningful, linked content", async () => {
  ...
  assert.ok(["contrast-n4-te-direction", "contrast-n4-benefit-perspective", "contrast-n4-inference", "contrast-n4-change-habit-decision", "contrast-n4-quotation-thought", "contrast-n4-nominalization-capability", "contrast-n4-contrast-conjunction", "contrast-n4-time-aspect"].every((id) => mergedModule.grammarContrasts.some((contrast) => contrast.id === id)));
...
test("every persisted authored question has a semantic review decision", () => {
  assert.equal(authoredQuestions.length, 250);
...
test("reviewed grammar contrast questions use independent contexts", () => {
  ...
  assert.equal(contrastQuestions.length, 16);
  assert.equal(new Set(contrastQuestions.map((question) => question.prompt)).size, 16);
