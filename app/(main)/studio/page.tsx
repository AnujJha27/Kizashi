...
export default async function ContentStudioPage() {
  ...
  const questionHealth = validatePracticeQuestions(getPracticeQuestions(), new Set(items.map((item) => item.id)), new Map(items.map((item) => [item.id, item.category])));

  return <div className="mx-auto max-w-6xl"><div className="mb-8"><p className="eyebrow mb-3">Studio · provenance</p><h1 className="jp-serif text-3xl tracking-tight text-[#f5f5f2] sm:text-4xl">Make the path worth trusting.</h1><p className="mt-2 max-w-2xl text-sm text-[#9297a1]">A workspace for checking curriculum packages before they become lessons, drills, readings, or listening practice.</p></div><section className="mb-7 rounded-xl border border-white/10 bg-[#101b2b]/60 p-5"><div className="mb-3 flex items-center justify-between gap-3"><p className="eyebrow">Source register</p><span className="text-xs text-[#676c75]">{sources.length} sources</span></div><div className="grid gap-2 sm:grid-cols-2">{sources.map((source) => <div key={source.id} className="flex items-center justify-between gap-3 rounded-lg bg-[#17181d]/70 px-3 py-3"><div className="min-w-0"><p className="truncate text-sm text-[#f5f5f2]">{source.name}</p><p className="mt-1 truncate text-[10px] text-[#676c75]">{source.localFilename ?? source.url ?? "Authored locally"}</p></div><span className="shrink-0 text-[10px] uppercase tracking-[.12em] text-[#e5b85c]">{source.type}</span></div>)}</div></section><ContentStudio seed={module} seedHealth={seedHealth} questionHealth={questionHealth} questions={getPracticeQuestions()} knownItemIds={items.map((item) => item.id)} sources={sources} /></div>;
}
