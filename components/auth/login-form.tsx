...
export function LoginForm() {
  ...
  async function signInWithGoogle() {
    ...
    const supabase = createSupabaseBrowserClient();
    if (!supabase) {
      setMessage("Google sign-in needs Supabase configuration.");
      return;
    }
  ...
  return <form onSubmit={submit} className="space-y-4"><button type="button" onClick={() => void signInWithGoogle()} disabled={pending} className="w-full rounded-xl border border-[#3f4652] bg-[#17181d] px-4 py-3.5 text-sm font-semibold text-[#f5f5f2] enabled:hover:border-[#e5b85c] disabled:cursor-wait disabled:opacity-60">Continue with Google</button><div className="flex items-center gap-3 text-[10px] uppercase tracking-[.16em] text-[#676c75]"><span className="h-px flex-1 bg-[#292b31]" /><span>or email</span><span className="h-px flex-1 bg-[#292b31]" /></div><label className="block"><span className="mb-2 block text-xs font-medium text-[#9297a1]">Allowed email</span><input required type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" autoComplete="email" className="w-full rounded-xl border border-[#292b31] bg-[#17181d] px-4 py-3 text-sm text-[#f5f5f2] placeholder:text-[#676c75] focus:border-[#e5b85c] focus:outline-none" /></label>{message ? <p className="text-sm text-[#e5b85c]" role="status">{message}</p> : null}<button type="submit" disabled={pending} className="w-full rounded-xl bg-[#e34a3f] px-4 py-3.5 text-sm font-semibold text-[#0b0b0d] disabled:cursor-wait disabled:opacity-60">{pending ? "Sending…" : "Send magic link"}</button></form>;
}
