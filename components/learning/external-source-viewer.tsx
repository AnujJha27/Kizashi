...
export function ExternalSourceFrame({ source }: Readonly<{ source: ExternalSourceLink }>) {
  return <div className="mt-3 w-full overflow-hidden rounded-xl border border-[#3f4652] bg-[#0b0b0d]">{source.mediaUrl ? <video title={`${source.name} original video`} controls playsInline preload="metadata" poster={source.posterUrl} className="max-h-[48rem] w-full bg-black"><source src={source.mediaUrl} type="video/mp4" />Your browser does not support video playback.</video> : <iframe title={`${source.name} original source`} src={source.url} loading="lazy" referrerPolicy="strict-origin-when-cross-origin" allow="autoplay; fullscreen; picture-in-picture; encrypted-media" allowFullScreen className="h-[70vh] min-h-[34rem] max-h-[48rem] w-full border-0" />}<p className="border-t border-white/10 px-3 py-2 text-[11px] leading-5 text-[#9297a1]">{source.mediaUrl ? "Streaming the provider's original media directly; Kizashi does not store or proxy it." : "If the provider blocks framing or requires a login, use "}<a href={source.url} target="_blank" rel="noreferrer" className="text-[#e5b85c] hover:text-[#f1cf7c]">Open original source ↗</a>{source.mediaUrl ? "." : ". This view does not copy, proxy, cache, or upload the source."}</p></div>;
}
