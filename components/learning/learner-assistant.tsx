...
function ResultView({ task, result }: Readonly<{ task: Task; result: AssistResult }>) {
  if (task === "explain") return <div className="mt-5 space-y-4"><div><p className="eyebrow">Parts</p><p className="mt-2 jp-serif text-lg text-[#e5b85c]">{strings(result.segmentation).join(" · ")}</p></div><div><p className="eyebrow">Grammar</p><ul className="mt-2 space-y-1 text-sm leading-6 text-[#c3c7ce]">{strings(result.grammar).map((entry) => <li key={entry}>— {entry}</li>)}</ul></div><div className="grid gap-3 sm:grid-cols-2"><div><p className="eyebrow">Literal</p><p className="mt-1 text-sm text-[#c3c7ce]">{text(result.literalTranslation)}</p></div><div><p className="eyebrow">Natural</p><p className="mt-1 text-sm text-[#c3c7ce]">{text(result.naturalTranslation)}</p></div></div></div>;
  if (task === "conversation") return <div className="mt-5 space-y-3"><p className="jp-serif text-2xl text-[#e5b85c]">{text(result.reply)}</p><p className="text-sm text-[#c3c7ce]">{text(result.translation)}</p><div className="rounded-lg border border-white/10 bg-[#101b2b]/60 p-3"><p className="eyebrow">Your next line</p><p className="mt-1 jp-serif text-lg text-[#f5f5f2]">{text(result.question)}</p></div><p className="text-xs leading-5 text-[#9297a1]">Tip · {text(result.tip)}</p></div>;
  return <div className="mt-5 space-y-3"><div><p className="eyebrow">Suggested correction</p><p className="mt-1 jp-serif text-xl text-[#e5b85c]">{text(result.corrected)}</p></div><p className="text-sm leading-6 text-[#c3c7ce]">{text(result.explanation)}</p>{strings(result.alternatives).length ? <div><p className="eyebrow">Alternatives</p><ul className="mt-1 space-y-1 text-sm text-[#c3c7ce]">{strings(result.alternatives).map((entry) => <li key={entry}>— {entry}</li>)}</ul></div> : null}</div>;
}

export function LearnerAssistant({ defaultText = "", itemId }: Readonly<{ defaultText?: string; itemId?: string }>) {
