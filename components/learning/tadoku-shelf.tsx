...
export function TadokuShelf() {
  ...
  return <section className="mx-auto mt-6 rounded-xl border border-white/10 bg-[#101b2b]/55 p-5 sm:p-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="eyebrow">読む · 多読</p><h2 className="mt-1 text-2xl font-medium text-[#f5f5f2]">Beginner graded reading</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#9297a1]">Read the original Tadoku books at the source. Start easy, keep moving, and use the audio when it is available.</p></div><span className="text-xs text-[#9297a1]">Opened · {entries.filter((entry) => progress[entry.id]).length} / {entries.length}</span></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{entries.map((entry) => { const source = sourceFor(entry); return <article key={entry.id} className="flex min-h-64 flex-col rounded-xl border border-white/10 bg-[#17181d]/75 p-4"><div><p className="text-sm font-medium text-[#f5f5f2]">{entry.title}</p><p className="mt-1 text-xs uppercase tracking-[.12em] text-[#e5b85c]">{entry.level} · {entry.genre}</p><p className="mt-2 text-xs text-[#9297a1]">{entry.length}{entry.audio.available ? " · audio available" : ""}</p>{progress[entry.id] ? <p className="mt-2 text-[11px] text-[#8bcca6]">✓ Opened at Tadoku</p> : null}</div><div className="mt-auto pt-5"><AudioControls text={entry.title} externalUrl={entry.audio.url} /><ExternalSourceViewer source={source} open={selectedId === entry.id} onToggle={() => { markExternalSourceOpened(entry.id); setProgress((value) => ({ ...value, [entry.id]: true })); setSelectedId((value) => value === entry.id ? null : entry.id); }} /><details className="mt-3 text-[11px] text-[#676c75]"><summary className="cursor-pointer">ⓘ Source</summary><p className="mt-2 leading-5">{entry.attribution} · {entry.license}</p></details></div></article>; })}</div></section>;
}
