...
export function TadokuShelf() {
  ...
  const catalog = getExternalResourceById("tadoku-free-books");
  const catalogSource = catalog ? externalResourceToSourceLink(catalog) : null;
  ...
  return <section className="mx-auto mt-6 rounded-xl border border-white/10 bg-[#101b2b]/55 p-5 sm:p-6"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="eyebrow">読む · 多読</p><h2 className="mt-1 text-2xl font-medium text-[#f5f5f2]">Beginner graded reading</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-[#9297a1]">Featured books stay thumb-friendly here. Browse Tadoku&apos;s complete free catalog when you want every level, genre, and title.</p></div><div className="flex flex-wrap items-center gap-3"><span className="text-xs text-[#9297a1]">Opened · {entries.filter((entry) => progress[entry.id]).length} / {entries.length} featured</span>{catalogSource ? <ExternalSourceLauncher source={catalogSource} children="Browse all free books ↗" /> : null}</div></div><div className="mt-5 divide-y divide-white/10">{entries.map((entry) => { const source = sourceFor(entry); return <article key={entry.id} className="flex flex-wrap items-center justify-between gap-5 py-5 first:pt-0 last:pb-0"><div className="min-w-0"><p className="text-lg font-medium text-[#f5f5f2]"><JapaneseText text={entry.title} vocabulary={module.vocabulary} kanji={module.kanji} always inspect={false} /></p><p className="mt-1 text-xs uppercase tracking-[.12em] text-[#e5b85c]">{entry.level} · {entry.genre}</p><p className="mt-2 text-xs text-[#9297a1]">{entry.length}{entry.audio.available ? " · audio available" : ""}{progress[entry.id] ? " · opened at Tadoku" : ""}</p></div><div className="flex flex-wrap items-center gap-2"><AudioControls text={entry.title} externalUrl={entry.audio.url} /><ExternalSourceViewer source={source} open={selectedId === entry.id} onToggle={() => { markExternalSourceOpened(entry.id); setProgress((value) => ({ ...value, [entry.id]: true })); setSelectedId((value) => value === entry.id ? null : entry.id); }} /><details className="text-[11px] text-[#676c75]"><summary className="cursor-pointer">ⓘ Source</summary><p className="mt-2 leading-5">{entry.attribution} · {entry.license}</p></details></div></article>; })}</div></section>;
}
