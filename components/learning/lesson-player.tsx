...
function Prompt({ item, onReveal, vocabulary = [], kanji = [] }: Readonly<{ item: LessonContentItem; onReveal: () => void; vocabulary?: VocabularyItem[]; kanji?: KanjiItem[] }>) {
  if (item.category === "vocabulary") return <><p className="jp-serif text-6xl text-[#f5f5f2]"><JapaneseWord written={item.writtenForm} reading={item.reading} /></p><TypedRecall item={item} onReveal={onReveal} /></>;
  if (item.category === "kanji") return <><p className="jp-serif text-8xl text-[#f5f5f2]">{item.character}</p><TypedRecall item={item} onReveal={onReveal} /></>;
  if (item.category === "grammar") return <p className="jp-serif text-4xl text-[#f5f5f2]"><JapaneseText text={item.pattern} vocabulary={vocabulary} kanji={kanji} /></p>;
...
function Answer({ item, contrasts, vocabulary, kanji }: Readonly<{ item: LessonContentItem; contrasts: GrammarContrast[]; vocabulary?: VocabularyItem[]; kanji?: KanjiItem[] }>) {
  if (item.category === "vocabulary") return <><p className="text-lg font-medium text-[#f5f5f2]">{item.meanings.join(" · ")}</p><p className="mt-3 text-sm text-[#9297a1]">{item.partOfSpeech} · {item.collocations.slice(0, 2).join(" · ")}</p><p className="jp-serif mt-4 text-lg text-[#e5b85c]"><JapaneseText text={item.exampleSentences[0]?.japanese ?? ""} vocabulary={vocabulary ?? []} kanji={kanji} /></p><p className="mt-1 text-sm text-[#9297a1]">{item.exampleSentences[0]?.translation}</p>{item.exampleSentences[0] ? <SaveSentence sourceItemId={item.id} japanese={item.exampleSentences[0].japanese} translation={item.exampleSentences[0].translation} /> : null}</>;
  ...
  if (item.category === "grammar") { const contrast = contrasts.find((entry) => item.contrastIds.includes(entry.id)); return <><p className="text-lg font-medium text-[#f5f5f2]">{item.meaning}</p><p className="mt-3 text-sm leading-6 text-[#9297a1]">{item.formation}</p><p className="mt-4 text-sm leading-6 text-[#c3a998]">{item.intuition}</p>{item.examples[0] ? <GrammarVisual sentence={item.examples[0].japanese} vocabulary={vocabulary} kanji={kanji} /> : null}<div className="mt-5 space-y-3">{item.examples.slice(0, 2).map((example) => <div key={example.japanese} className="rounded-lg bg-[#17181d]/80 p-3"><p className="jp-serif text-lg text-[#e5b85c]"><JapaneseText text={example.japanese} vocabulary={vocabulary ?? []} kanji={kanji} /></p><p className="mt-1 text-xs text-[#9297a1]">{example.translation}</p><SaveSentence sourceItemId={item.id} japanese={example.japanese} translation={example.translation} /></div>)}</div><p className="mt-5 text-sm leading-6 text-[#9297a1]">Common mistake: {item.commonMistakes[0]}</p>{contrast ? <div className="mt-5 border-l-2 border-[#e34a3f] pl-4"><p className="eyebrow">Contrast · {contrast.title}</p><p className="mt-2 text-sm leading-6 text-[#c3a998]">{contrast.explanation}</p></div> : null}</>;
  }
  if (item.category === "reading") return <ReadingPanel item={item} vocabulary={vocabulary} kanji={kanji} />;
  const playTranscript = (rate = 0.9) => { if (!("speechSynthesis" in window)) return; window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(item.transcript); utterance.lang = "ja-JP"; utterance.rate = rate; window.speechSynthesis.speak(utterance); };
  return <>{item.audioUrl ? <audio controls src={item.audioUrl} className="mb-4 w-full" /> : <button type="button" onClick={() => playTranscript()} onDoubleClick={() => playTranscript(0.6)} title="Double-tap to replay slowly" className="mb-4 inline-flex min-h-12 items-center gap-3 rounded-xl border border-[#4f9ac0] bg-[#102536]/70 px-5 py-3 text-sm font-semibold text-[#d9eef8] hover:border-[#8cc9e5]"><span className="text-lg" aria-hidden="true">🔊</span> Play Japanese audio</button>}<p className="whitespace-pre-line text-sm leading-7 text-[#f5f5f2]"><JapaneseText text={item.transcript} vocabulary={vocabulary ?? []} kanji={kanji} /></p><p className="mt-4 text-sm text-[#9297a1]">{item.questions.length} listening questions ready.</p></>;
}
