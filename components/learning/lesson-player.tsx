...
function Answer({ item, contrasts, vocabulary, kanji }: Readonly<{ item: LessonContentItem; contrasts: GrammarContrast[]; vocabulary?: VocabularyItem[]; kanji?: KanjiItem[] }>) {
  const [showGrammarHint, setShowGrammarHint] = useState(true);
  const [showListeningTranscript, setShowListeningTranscript] = useState(true);
  useEffect(() => {
    ...
    if (item.category === "grammar") setShowGrammarHint(!experienced);
    if (item.category === "listening") setShowListeningTranscript(!experienced);
  }, [item.category, item.id]);
  if (item.category === "vocabulary") return <><p className="text-3xl font-medium text-[#f5f5f2]">{item.meanings.join(" · ")}</p><p className="mt-3 text-lg text-[#9297a1]">{item.partOfSpeech} · {item.collocations.slice(0, 2).join(" · ")}</p><p className="jp-serif mt-4 text-3xl text-[#e5b85c]"><JapaneseText text={item.exampleSentences[0]?.japanese ?? ""} vocabulary={vocabulary ?? []} kanji={kanji} always /></p><p className="mt-1 text-lg text-[#9297a1]">{item.exampleSentences[0]?.translation}</p>{item.exampleSentences[0] ? <><AudioControls text={item.exampleSentences[0].japanese} metadata={item.exampleSentences[0].audio} humanFirst className="mt-3" /><SaveSentence sourceItemId={item.id} japanese={item.exampleSentences[0].japanese} translation={item.exampleSentences[0].translation} /></> : null}</>;
  if (item.category === "kanji") return <><p className="text-lg font-medium text-[#f5f5f2]">{item.meanings.join(" · ")}</p><p className="mt-3 text-sm text-[#9297a1]">On: {item.onyomi.join(" · ") || "—"} · Kun: {item.kunyomi.join(" · ") || "—"}</p><div className="mt-4 flex flex-wrap gap-2">{item.usefulWords.slice(0, 3).map((word) => <span key={word.word} className="rounded-lg border border-[#3f3427] bg-[#211d18] px-3 py-2 text-sm text-[#e5b85c]"><JapaneseText text={word.word} vocabulary={vocabulary ?? []} kanji={kanji} readingEntries={[[word.word, word.reading]]} always inspect={false} /></span>)}</div></>;
  if (item.category === "grammar") {
    const contrast = contrasts.find((entry) => item.contrastIds.includes(entry.id));
    if (!item.meaning?.trim()) return <IrodoriPatternReference item={item} vocabulary={vocabulary} kanji={kanji} />;
    return <><p className="text-lg font-medium text-[#f5f5f2]">{item.meaning}</p>{showGrammarHint ? <><p className="mt-3 text-sm leading-6 text-[#9297a1]">{item.formation}</p><p className="mt-4 text-sm leading-6 text-[#c3a998]">{item.intuition}</p></> : <button type="button" onClick={() => setShowGrammarHint(true)} className="mt-3 rounded-lg border border-[#3f4652] px-3 py-2 text-xs font-semibold text-[#c3c7ce] hover:border-[#e5b85c]">Show formation hint</button>}{item.examples[0] ? <GrammarVisual sentence={item.examples[0].japanese} vocabulary={vocabulary} kanji={kanji} /> : null}<div className="mt-5 space-y-3">{item.examples.slice(0, 2).map((example) => <div key={example.japanese} className="rounded-lg bg-[#17181d]/80 p-3"><p className="jp-serif text-lg text-[#e5b85c]"><JapaneseText text={example.japanese} vocabulary={vocabulary ?? []} kanji={kanji} always /></p><p className="mt-1 text-xs text-[#9297a1]">{example.translation}</p><AudioControls text={example.japanese} metadata={example.audio} humanFirst className="mt-3" /><SaveSentence sourceItemId={item.id} japanese={example.japanese} translation={example.translation} /></div>)}</div><p className="mt-5 text-sm leading-6 text-[#9297a1]">Common mistake: {item.commonMistakes[0]}</p>{contrast ? <div className="mt-5 border-l-2 border-[#e34a3f] pl-4"><p className="eyebrow">Contrast · {contrast.title}</p><p className="mt-2 text-sm leading-6 text-[#c3a998]">{contrast.explanation}</p></div> : null}</>;
  }
  ...
  const visualQuestion = item.questions.find((question) => question.visualScene);
  return <><AudioControls text={item.transcript} externalUrl={item.audioUrl} metadata={item.audio} preferredRate={item.speed} className="mb-4" />{visualQuestion?.visualScene ? <ListeningScene scene={visualQuestion.visualScene} description={visualQuestion.visualContext} /> : null}{showListeningTranscript ? <p className="whitespace-pre-line text-sm leading-7 text-[#f5f5f2]"><JapaneseText text={item.transcript} vocabulary={vocabulary ?? []} kanji={kanji} always /></p> : <button type="button" onClick={() => setShowListeningTranscript(true)} className="rounded-lg border border-[#3f4652] px-3 py-2 text-xs font-semibold text-[#c3c7ce] hover:border-[#e5b85c]">Show transcript</button>}<p className="mt-4 text-sm text-[#9297a1]">{item.questions.length} listening questions ready.</p></>;
}
