...
function Answer({ item, contrasts, vocabulary, kanji }: Readonly<{ item: LessonContentItem; contrasts: GrammarContrast[]; vocabulary?: VocabularyItem[]; kanji?: KanjiItem[] }>) {
  ...
  if (item.category === "vocabulary") return <><p className="text-2xl font-medium text-[#f5f5f2]">{item.meanings.join(" · ")}</p><p className="mt-3 text-base text-[#9297a1]">{item.partOfSpeech} · {item.collocations.slice(0, 2).join(" · ")}</p><p className="jp-serif mt-4 text-2xl text-[#e5b85c]"><JapaneseText text={item.exampleSentences[0]?.japanese ?? ""} vocabulary={vocabulary ?? []} kanji={kanji} always /></p><p className="mt-1 text-base text-[#9297a1]">{item.exampleSentences[0]?.translation}</p>{item.exampleSentences[0] ? <><AudioControls text={item.exampleSentences[0].japanese} metadata={item.exampleSentences[0].audio} humanFirst className="mt-3" /><SaveSentence sourceItemId={item.id} japanese={item.exampleSentences[0].japanese} translation={item.exampleSentences[0].translation} /></> : null}</>;
