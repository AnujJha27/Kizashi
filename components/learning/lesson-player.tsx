...
function Prompt({ item, onReveal, vocabulary = [], kanji = [] }: Readonly<{ item: LessonContentItem; onReveal: () => void; vocabulary?: VocabularyItem[]; kanji?: KanjiItem[] }>) {
  if (item.category === "vocabulary") return <><p className="jp-serif text-6xl text-[#f5f5f2]"><JapaneseWord written={item.writtenForm} reading={item.reading} /></p><AudioControls text={item.writtenForm} /><TypedRecall item={item} onReveal={onReveal} /></>;
  if (item.category === "kanji") return <><p className="jp-serif text-8xl text-[#f5f5f2]">{item.character}</p><AudioControls text={item.character} /><TypedRecall item={item} onReveal={onReveal} /></>;
  if (item.category === "grammar") return <><p className="jp-serif text-4xl text-[#f5f5f2]"><JapaneseText text={item.pattern} vocabulary={vocabulary} kanji={kanji} /></p><AudioControls text={item.pattern} /></>;
...
function Answer({ item, contrasts, vocabulary, kanji }: Readonly<{ item: LessonContentItem; contrasts: GrammarContrast[]; vocabulary?: VocabularyItem[]; kanji?: KanjiItem[] }>) {
  if (item.category === "vocabulary") return <><p className="text-lg font-medium text-[#f5f5f2]">{item.meanings.join(" · ")}</p><p className="mt-3 text-sm text-[#9297a1]">{item.partOfSpeech} · {item.collocations.slice(0, 2).join(" · ")}</p><p className="jp-serif mt-4 text-lg text-[#e5b85c]"><JapaneseText text={item.exampleSentences[0]?.japanese ?? ""} vocabulary={vocabulary ?? []} kanji={kanji} /></p><p className="mt-1 text-sm text-[#9297a1]">{item.exampleSentences[0]?.translation}</p>{item.exampleSentences[0] ? <><AudioControls text={item.exampleSentences[0].japanese} metadata={item.exampleSentences[0].audio} className="mt-3" /><SaveSentence sourceItemId={item.id} japanese={item.exampleSentences[0].japanese} translation={item.exampleSentences[0].translation} /></> : null}</>;
  ...
  if (item.category === "grammar") { const contrast = contrasts.find((entry) => item.contrastIds.includes(entry.id)); return <><p className="text-lg font-medium text-[#f5f5f2]">{item.meaning}</p><p className="mt-3 text-sm leading-6 text-[#9297a1]">{item.formation}</p><p className="mt-4 text-sm leading-6 text-[#c3a998]">{item.intuition}</p>{item.examples[0] ? <GrammarVisual sentence={item.examples[0].japanese} vocabulary={vocabulary} kanji={kanji} /> : null}<div className="mt-5 space-y-3">{item.examples.slice(0, 2).map((example) => <div key={example.japanese} className="rounded-lg bg-[#17181d]/80 p-3"><p className="jp-serif text-lg text-[#e5b85c]"><JapaneseText text={example.japanese} vocabulary={vocabulary ?? []} kanji={kanji} /></p><p className="mt-1 text-xs text-[#9297a1]">{example.translation}</p><AudioControls text={example.japanese} metadata={example.audio} className="mt-3" /><SaveSentence sourceItemId={item.id} japanese={example.japanese} translation={example.translation} /></div>)}</div><p className="mt-5 text-sm leading-6 text-[#9297a1]">Common mistake: {item.commonMistakes[0]}</p>{contrast ? <div className="mt-5 border-l-2 border-[#e34a3f] pl-4"><p className="eyebrow">Contrast · {contrast.title}</p><p className="mt-2 text-sm leading-6 text-[#c3a998]">{contrast.explanation}</p></div> : null}</>;
  }
  if (item.category === "reading") return <><AudioControls text={item.passage} metadata={item.audio} /><ReadingPanel item={item} vocabulary={vocabulary} kanji={kanji} /></>;
  return <><AudioControls text={item.transcript} externalUrl={item.audioUrl} metadata={item.audio} preferredRate={item.speed} className="mb-4" /><p className="whitespace-pre-line text-sm leading-7 text-[#f5f5f2]"><JapaneseText text={item.transcript} vocabulary={vocabulary ?? []} kanji={kanji} /></p><p className="mt-4 text-sm text-[#9297a1]">{item.questions.length} listening questions ready.</p></>;
}
