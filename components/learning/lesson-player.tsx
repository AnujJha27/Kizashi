...
function Answer({ item, contrasts, vocabulary, kanji }: Readonly<{ item: LessonContentItem; contrasts: GrammarContrast[]; vocabulary?: VocabularyItem[]; kanji?: KanjiItem[] }>) {
  ...
  if (item.category === "vocabulary") return <><p className="text-2xl font-medium text-[#f5f5f2]">{item.meanings.join(" · ")}</p><p className="mt-3 text-base text-[#9297a1]">{item.partOfSpeech} · {item.collocations.slice(0, 2).join(" · ")}</p><p className="jp-serif mt-4 text-2xl text-[#e5b85c]"><JapaneseText text={item.exampleSentences[0]?.japanese ?? ""} vocabulary={vocabulary ?? []} kanji={kanji} always /></p><p className="mt-1 text-base text-[#9297a1]">{item.exampleSentences[0]?.translation}</p>{item.exampleSentences[0] ? <><AudioControls text={item.exampleSentences[0].japanese} metadata={item.exampleSentences[0].audio} className="mt-3" /><SaveSentence sourceItemId={item.id} japanese={item.exampleSentences[0].japanese} translation={item.exampleSentences[0].translation} /></> : null}</>;
  ...
  if (item.category === "grammar") {
    ...
    return <><p className="text-lg font-medium text-[#f5f5f2]">{item.meaning}</p>{showGrammarHint ? <><p className="mt-3 text-sm leading-6 text-[#9297a1]">{item.formation}</p><p className="mt-4 text-sm leading-6 text-[#c3a998]">{item.intuition}</p></> : <button type="button" onClick={() => setShowGrammarHint(true)} className="mt-3 rounded-lg border border-[#3f4652] px-3 py-2 text-xs font-semibold text-[#c3c7ce] hover:border-[#e5b85c]">Show formation hint</button>}{item.examples[0] ? <GrammarVisual sentence={item.examples[0].japanese} vocabulary={vocabulary} kanji={kanji} /> : null}<div className="mt-5 space-y-3">{item.examples.slice(0, 2).map((example) => <div key={example.japanese} className="rounded-lg bg-[#17181d]/80 p-3"><p className="jp-serif text-lg text-[#e5b85c]"><JapaneseText text={example.japanese} vocabulary={vocabulary ?? []} kanji={kanji} always /></p><p className="mt-1 text-xs text-[#9297a1]">{example.translation}</p><AudioControls text={example.japanese} metadata={example.audio} className="mt-3" /><SaveSentence sourceItemId={item.id} japanese={example.japanese} translation={example.translation} /></div>)}</div><p className="mt-5 text-sm leading-6 text-[#9297a1]">Common mistake: {item.commonMistakes[0]}</p>{contrast ? <div className="mt-5 border-l-2 border-[#e34a3f] pl-4"><p className="eyebrow">Contrast · {contrast.title}</p><p className="mt-2 text-sm leading-6 text-[#c3a998]">{contrast.explanation}</p></div> : null}</>;
  }
