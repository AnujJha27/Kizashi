...
function ShunVideoCatalog({ source, rotation, interests }: Readonly<{ source: ExternalSourceLink; rotation: number; interests: readonly string[] }>) {
  ...
  useEffect(() => {
    let active = true;
    fetch("/api/immersion/shun").then((response) => response.ok ? response.json() as Promise<ShunVideo[]> : []).then((catalog) => { if (active && Array.isArray(catalog) && catalog.length) setVideos(catalog); }).catch(() => undefined).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, []);
...
function TeppeiPodcastCatalog({ source, rotation, interests }: Readonly<{ source: ExternalSourceLink; rotation: number; interests: readonly string[] }>) {
  ...
  useEffect(() => {
    let active = true;
    fetch("/api/immersion/teppei").then((response) => response.ok ? response.json() as Promise<TeppeiEpisode[]> : []).then((catalog) => { if (active && Array.isArray(catalog)) setEpisodes(catalog); }).catch(() => undefined).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, []);
