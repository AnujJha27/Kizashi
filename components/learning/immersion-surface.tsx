...
export function ImmersionSurface() {
  ...
  return <div className="space-y-7">
    ...
    <section className="surface-panel overflow-hidden p-6 sm:p-9">
      ...
      <AudioControls text={shadowing ? phrase : clip.transcript} externalUrl={clip.audioUrl} metadata={clip.audio} className="mt-6" />
      ...
      {shadowing ? <div className="mt-5 rounded-xl border border-[#315d4b] bg-[#162b26]/60 p-4"><p className="eyebrow">Shadowing · phrase {phrasePosition + 1} / {Math.max(phrases.length, 1)}</p><p className="jp-serif mt-3 text-xl text-[#f5f5f2]"><JapaneseText text={phrase} vocabulary={module.vocabulary} kanji={module.kanji} always /></p><div className="mt-4 flex flex-wrap gap-2"><button type="button" onClick={() => setPhrasePosition((value) => Math.min(value + 1, Math.max(phrases.length - 1, 0)))} className="rounded-lg border border-[#3f4652] px-3 py-2 text-xs text-[#c3c7ce] hover:border-[#e5b85c]">Next phrase</button><button type="button" onClick={() => setSpeakingAlone((value) => !value)} className="rounded-lg bg-[#6fb98f] px-3 py-2 text-xs font-semibold text-[#0b0b0d]">{speakingAlone ? "I said it" : "Speak alone"}</button></div>{speakingAlone ? <p className="mt-3 text-xs text-[#8bcca6]" role="status">Your turn—say the phrase, then tap “I said it” to continue.</p> : <p className="mt-3 text-xs leading-5 text-[#9297a1]">Listen, shadow along, or speak alone. Audio stays external/browser-based; Kizashi does not record your voice.</p>}</div> : null}
    </section>

    <section className="rounded-xl border border-white/10 bg-[#101b2b]/55 p-5 sm:p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">Original-source shelf</p><h3 className="mt-1 text-xl font-medium text-[#f5f5f2]">Keep source ownership with the source.</h3><p className="mt-1 max-w-2xl text-xs leading-5 text-[#9297a1]">View the original provider page here when framing works. Kizashi stores no third-party corpus audio here and does not proxy or re-host it.</p></div><span className="text-xs text-[#9297a1]">Sources opened · {openedSourceCount} / {trackableSourceIds.length}</span></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{sources.map((source) => <article key={source.id} className="rounded-xl border border-white/10 bg-[#17181d]/70 p-4"><p className="text-sm font-medium text-[#f5f5f2]">{source.title ?? source.name}</p><p className="mt-1 text-xs uppercase tracking-[.1em] text-[#676c75]">{source.name}{source.level ? ` · ${source.level}` : ""}{source.context ? ` · ${source.context}` : ""}</p><p className="mt-1 text-xs leading-5 text-[#9297a1]">{source.description}</p>{source.targetSkills?.length ? <p className="mt-2 text-[11px] text-[#e5b85c]">Target: {source.targetSkills.join(" · ")}</p> : null}{source.targetItemIds?.length ? <p className="mt-1 text-[10px] leading-4 text-[#9297a1]">Maps to: {source.targetItemIds.join(" · ")}</p> : null}{source.resourceTypes?.length ? <p className="mt-1 text-[10px] leading-4 text-[#9297a1]">On source: {source.resourceTypes.join(" · ")}</p> : null}{source.id === "erin" ? <label className="mt-3 block text-xs text-[#9297a1]"><span className="mr-2">Lesson</span><select value={selectedErinSource.id} onChange={(event) => setSelectedErinId(event.target.value)} className="mt-1 w-full rounded-lg border border-[#3f4652] bg-[#101b2b] px-3 py-2 text-xs text-[#f5f5f2]">{erinLessons.map((lesson) => <option key={lesson.id} value={lesson.id}>{lesson.title}</option>)}</select></label> : null}<ExternalSourceViewer source={source.id === "erin" ? selectedErinSource : source} open={selectedSourceId === source.id} onToggle={() => setSelectedSourceId((value) => value === source.id ? null : source.id)} /></article>)}</div>{selectedViewerSource ? <ExternalSourceFrame source={selectedViewerSource} /> : null}</section>
  </div>;
}
