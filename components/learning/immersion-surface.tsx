...
function YouTubeVideoCatalog({ source, rotation, interests, journeyAreaId }: Readonly<{ source: ExternalSourceLink; rotation: number; interests: readonly string[]; journeyAreaId?: string }>) {
  ...
  useEffect(() => {
    ...
    fetch(endpoint).then((response) => { if (!response.ok) throw new Error("provider catalog unavailable"); return response.json() as Promise<YouTubeVideo[]>; }).then((catalog) => { if (active && Array.isArray(catalog) && catalog.length) setVideos(catalog); }).catch(() => { if (active) setError(true); }).finally(() => { if (active) setLoading(false); });
    return () => { active = false; };
  }, []);
