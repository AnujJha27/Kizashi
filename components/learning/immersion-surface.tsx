...
export function ImmersionSurface() {
  ...
  return <div className="space-y-7">
    ...
    <section className="surface-panel overflow-hidden p-6 sm:p-9">
      ...
      {transcriptVisible ? <div className="mt-6 rounded-xl border border-white/10 bg-[#101b2b]/70 p-4"><p className="eyebrow mb-2">Transcript</p><p className="jp-serif whitespace-pre-line text-lg leading-8 text-[#f5f5f2]">{clip.transcript}</p></div> : <div className="mt-6 rounded-xl border border-[#3f4652] bg-[#101b2b]/40 p-4 text-sm text-[#9297a1]">Audio first. Try the question before revealing the transcript.</div>}
    ...
    <section className="rounded-xl border border-white/10 bg-[#101b2b]/55 p-5 sm:p-6"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">Original-source shelf</p><h3 className="mt-1 text-xl font-medium text-[#f5f5f2]">Keep source ownership with the source.</h3><p className="mt-1 max-w-2xl text-xs leading-5 text-[#9297a1]">These links open the original provider page. Kizashi stores no third-party corpus audio here and does not proxy or re-host it.</p></div></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{sources.map((source) => <article key={source.id} className="rounded-xl border border-white/10 bg-[#17181d]/70 p-4"><p className="text-sm font-medium text-[#f5f5f2]">{source.title ?? source.name}</p><p className="mt-1 text-xs uppercase tracking-[.1em] text-[#676c75]">{source.name}{source.level ? ` · ${source.level}` : ""}{source.context ? ` · ${source.context}` : ""}</p><p className="mt-1 text-xs leading-5 text-[#9297a1]">{source.description}</p>{source.targetSkills?.length ? <p className="mt-2 text-[11px] text-[#e5b85c]">Target: {source.targetSkills.join(" · ")}</p> : null}{source.targetItemIds?.length ? <p className="mt-1 text-[10px] leading-4 text-[#9297a1]">Maps to: {source.targetItemIds.join(" · ")}</p> : null}{source.resourceTypes?.length ? <p className="mt-1 text-[10px] leading-4 text-[#9297a1]">On source: {source.resourceTypes.join(" · ")}</p> : null}<ExternalSourceLauncher source={source}>{source.id.startsWith("erin-") ? "Open original lesson ↗" : "Open original source ↗"}</ExternalSourceLauncher></article>)}</div></section>
  </div>;
}
