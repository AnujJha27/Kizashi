...
const kinds: Array<[ActivityKind, string]> = [["speaking", "Speak"], ["writing", "Write"], ["pragmatics", "Meaning in context"], ["chunks", "Natural phrase"]];
...
export function OutputPractice({ module }: Readonly<{ module: N5Module }>) {
  ...
  const activities = useMemo(() => (banks[kind] as Activity[]).filter((item) => (item.level ?? item.targetLevel) === level).sort((left, right) => {
  ...
  }), [banks, kind, level, reviewRecords]);
  ...
  return <section className="border-t border-white/10 pt-7">
    ...
    <div className="mt-4 flex flex-wrap items-center gap-2" aria-label="Output level"><span className="text-xs text-[#676c75]">Level</span>{(["N5", "N4"] as const).map((value) => <button key={value} type="button" onClick={() => setLevel(value)} aria-pressed={level === value} className={`rounded-lg border px-3 py-2 text-xs ${level === value ? "border-[#e5b85c] bg-[#302818] text-[#f1cf7c]" : "border-[#3f4652] text-[#9297a1] hover:border-[#e5b85c]"}`}>{value} <span className="text-[10px] opacity-70">({(banks[kind] as Activity[]).filter((item) => (item.level ?? item.targetLevel) === value).length})</span></button>)}</div>
