...
export function OutputPractice({ module }: Readonly<{ module: N5Module }>) {
  ...
  useEffect(() => setHintRevealed(false), [activity?.id]);

  if (!activity) return null;
  const activityRecord = reviewRecords[outputReviewId(activity.id)];
  const hintFaded = Boolean((activityRecord && (["stable", "strong"] as string[]).includes(activityRecord.masteryState ?? "")) || (activityRecord?.attempts ?? 0) >= 3);
  ...
  return <section className="border-t border-white/10 pt-7">
    ...
    <div className="mt-5 border-l-2 border-[#e5b85c] pl-4" role="tabpanel"><p className="text-sm text-[#f5f5f2]">{activity.prompt}</p><p className="mt-4 whitespace-pre-line jp-serif text-2xl leading-relaxed text-[#f1cf7c]"><JapaneseText text={activity.japanese} vocabulary={module.vocabulary} kanji={module.kanji} always inspect={false} /></p>{hintFaded && !hintRevealed ? <button type="button" onClick={() => setHintRevealed(true)} className="mt-3 rounded-lg border border-[#3f4652] px-3 py-2 text-xs font-semibold text-[#c3c7ce] hover:border-[#e5b85c]">Show support hint</button> : <p className="mt-3 text-xs leading-5 text-[#9297a1]">{activity.hint}</p>}
