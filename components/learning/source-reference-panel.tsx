...
export function grammarReferencesFor(grammarId: string) {
  return [taeKimById[grammarId], wikibooksById[grammarId]].filter((reference): reference is Reference => Boolean(reference));
}
...
export function SourceReferencePanel({ grammarId }: Readonly<{ grammarId: string }>) {
  ...
  return <section className="mt-8 rounded-xl border border-[#3f3427] bg-[#211d18]/45 p-4 sm:p-5"><p className="eyebrow">別の見方 · Alternative explanations</p><div className="mt-3 grid gap-3 sm:grid-cols-2">{references.map((reference) => <article key={`${reference.sourceId}-${reference.sectionTitle}`} className="rounded-lg border border-white/10 bg-[#101b2b]/65 p-4"><p className="text-sm font-medium text-[#f5f5f2]">{reference.sourceId === "tae-kim" ? "Tae Kim" : "Wikibooks · Japanese"}</p><p className="mt-1 text-xs text-[#e5b85c]">{reference.sectionTitle}</p><p className="mt-2 text-xs leading-5 text-[#9297a1]">{reference.description}</p><div className="mt-3 flex flex-wrap items-center gap-3"><a href={reference.url} target="_blank" rel="noreferrer" className="rounded-lg border border-[#3f4652] px-3 py-2 text-xs font-semibold text-[#c3c7ce] hover:border-[#e5b85c]">{reference.sourceId === "tae-kim" ? "Read explanation ↗" : "Open reference ↗"}</a>{reference.sourceId === "wikibooks-japanese" ? <Preview reference={reference} /> : null}</div><details className="mt-3 text-[11px] text-[#676c75]"><summary className="cursor-pointer hover:text-[#c3c7ce]">ⓘ Source</summary><p className="mt-2 leading-5">{reference.attribution} · {reference.license}</p></details></article>)}</div><p className="mt-3 text-[11px] text-[#676c75]">Kizashi's explanation and practice remain the primary lesson.</p></section>;
}
