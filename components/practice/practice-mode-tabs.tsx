...
const focusValues = new Set<PracticeMode>(["micro", "vocabulary", "kanji", "kanji-writing", "grammar", "conjugation", "mixed"]);
...
export function PracticeModeTabs({ modes, activeMode, activeTopic, targetLevel, onNavigate }: Readonly<{ modes: PracticeModeEntry[]; activeMode: PracticeMode; activeTopic?: string; targetLevel: TargetLevel; onNavigate?: (href: string) => void }>) {
  ...
  return <div className="mb-6 space-y-4">
    <nav aria-label="Practice modes" className="flex gap-4 overflow-x-auto border-b border-white/10 pb-px sm:gap-6">
      {intents.map((entry) => {
        const href = `${entry.href}${contextQuery(targetLevel, activeTopic)}`;
        const selected = entry.value === intent;
        return <Link key={entry.value} ref={selected ? activeTab : undefined} prefetch={false} href={href} onClick={onNavigate ? (event) => { event.preventDefault(); onNavigate(href); } : undefined} aria-current={selected ? "page" : undefined} className={`shrink-0 border-b-2 px-1 py-3 text-left ${selected ? "border-[#e34a3f] text-[#f5f5f2]" : "border-transparent text-[#9297a1] hover:border-[#5d3936]"}`}><span className="block text-sm font-semibold">{entry.label}</span><span className="jp-serif text-xs text-[#e5b85c]">{entry.jp}</span><span className="mt-1 hidden text-[11px] leading-4 text-[#9297a1] sm:block">{entry.detail}</span></Link>;
      })}
    </nav>
    {intent === "focus" ? <nav aria-label="Focus skills" className="flex flex-wrap items-center gap-2 border-l border-[#e5b85c] pl-3"><span className="eyebrow mr-1 text-[10px]">Choose a skill</span>{focusModes.map((entry) => { const selected = entry.value === activeMode; const href = `/practice?mode=${entry.value}${contextQuery(targetLevel, activeTopic)}`; return <Link key={entry.value} prefetch={false} href={href} onClick={onNavigate ? (event) => { event.preventDefault(); onNavigate(href); } : undefined} aria-current={selected ? "page" : undefined} className={`rounded-lg border px-3 py-2 text-xs ${selected ? "border-[#e5b85c] bg-[#302818] text-[#f1cf7c]" : "border-[#3f4652] text-[#9297a1] hover:border-[#e5b85c]"}`}>{entry.label} <span className="jp-serif ml-1 text-[#e5b85c]">{entry.jp}</span></Link>; })}</nav> : null}
    {intent === "test" ? <nav aria-label="Test modes" className="flex flex-wrap items-center gap-2 border-l border-[#e5b85c] pl-3"><span className="eyebrow mr-1 text-[10px]">Choose a test</span>{testModes.map((entry) => { const selected = entry.value === activeMode; const href = `/practice?mode=${entry.value}${contextQuery(targetLevel, activeTopic)}`; return <Link key={entry.value} prefetch={false} href={href} onClick={onNavigate ? (event) => { event.preventDefault(); onNavigate(href); } : undefined} aria-current={selected ? "page" : undefined} className={`rounded-lg border px-3 py-2 text-xs ${selected ? "border-[#e5b85c] bg-[#302818] text-[#f1cf7c]" : "border-[#3f4652] text-[#9297a1] hover:border-[#e5b85c]"}`}>{entry.label} <span className="jp-serif ml-1 text-[#e5b85c]">{entry.jp}</span></Link>; })}</nav> : null}
  </div>;
}
