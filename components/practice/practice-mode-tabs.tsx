  ...
  return <div className="mb-6 space-y-4">
    <nav aria-label="Practice modes" className="grid grid-cols-2 gap-2 sm:grid-cols-4">
      {intents.map((entry) => {
        ...
        return <Link key={entry.value} ref={selected ? activeTab : undefined} prefetch={false} href={href} onClick={onNavigate ? (event) => { event.preventDefault(); onNavigate(href); } : undefined} aria-current={selected ? "page" : undefined} className={`rounded-xl border px-4 py-3 text-left ${selected ? "border-[#e34a3f] bg-[#3a2023] text-[#f5f5f2]" : "border-[#292b31] bg-[#17181d]/65 text-[#9297a1] hover:border-[#5d3936]"}`}><span className="block text-sm font-semibold">{entry.label}</span><span className="jp-serif text-xs text-[#e5b85c]">{entry.jp}</span><span className="mt-1 hidden text-[11px] leading-4 text-[#9297a1] sm:block">{entry.detail}</span></Link>;
      })}
    </nav>
