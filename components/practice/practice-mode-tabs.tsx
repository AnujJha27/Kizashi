...
function topicQuery(activeTopic?: string) {
  return activeTopic ? `&topic=${encodeURIComponent(activeTopic)}` : "";
}
...
export function PracticeModeTabs({ modes, activeMode, activeTopic }: Readonly<{ modes: PracticeModeEntry[]; activeMode: PracticeMode; activeTopic?: string }>) {
  ...
  return <div className="mb-6 space-y-4">
    <nav aria-label="Practice modes" className="grid grid-cols-2 gap-2 sm:grid-cols-4">
      {intents.map((entry) => {
        const href = `${entry.href}${topicQuery(activeTopic)}`;
    ...
    {intent === "focus" ? <nav aria-label="Focus skills" className="flex flex-wrap items-center gap-2 border-l border-[#e5b85c] pl-3"><span className="eyebrow mr-1 text-[10px]">Choose a skill</span>{focusModes.map((entry) => { const selected = entry.value === activeMode; return <Link key={entry.value} prefetch={false} href={`/practice?mode=${entry.value}${topicQuery(activeTopic)}`} aria-current={selected ? "page" : undefined} className={`rounded-lg border px-3 py-2 text-xs ${selected ? "border-[#e5b85c] bg-[#302818] text-[#f1cf7c]" : "border-[#3f4652] text-[#9297a1] hover:border-[#e5b85c]"}`}>{entry.label} <span className="jp-serif ml-1 text-[#e5b85c]">{entry.jp}</span></Link>; })}</nav> : null}
    {intent === "test" ? <nav aria-label="Test modes" className="flex flex-wrap items-center gap-2 border-l border-[#e5b85c] pl-3"><span className="eyebrow mr-1 text-[10px]">Choose a test</span>{testModes.map((entry) => { const selected = entry.value === activeMode; return <Link key={entry.value} prefetch={false} href={`/practice?mode=${entry.value}${topicQuery(activeTopic)}`} aria-current={selected ? "page" : undefined} className={`rounded-lg border px-3 py-2 text-xs ${selected ? "border-[#e5b85c] bg-[#302818] text-[#f1cf7c]" : "border-[#3f4652] text-[#9297a1] hover:border-[#e5b85c]"}`}>{entry.label} <span className="jp-serif ml-1 text-[#e5b85c]">{entry.jp}</span></Link>; })}</nav> : null}
  </div>;
}
