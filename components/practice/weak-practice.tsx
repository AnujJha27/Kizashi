...
export function WeakPractice({ questions, vocabulary = [], kanji = [], readingEntries = [], items = [], grammarContrasts = [], learnerErrorAggregates = [], repairId, sessionId, onComplete }: Readonly<{ questions: PracticeQuestion[]; vocabulary?: VocabularyItem[]; kanji?: KanjiItem[]; readingEntries?: [string, string][]; items?: LearningItem[]; grammarContrasts?: GrammarContrast[]; learnerErrorAggregates?: IjasAggregate[]; repairId?: string; sessionId?: string; onComplete?: () => void }>) {
  ...
  return <>{repair ? <RepairCard repair={repair} vocabulary={vocabulary} kanji={kanji} /> : null}<PracticePlayer questions={weakQuestions.length ? weakQuestions : questions.slice(0, 7)} vocabulary={vocabulary} kanji={kanji} readingEntries={readingEntries} items={items} grammarContrasts={grammarContrasts} sessionId={sessionId} onComplete={() => { if (repairId) completeRepair(repairId); onComplete?.(); }} /></>;
}
