...
export function WeakPractice({ questions, vocabulary = [], kanji = [], items = [], grammarContrasts = [], learnerErrorAggregates = [], repairId, sessionId }: Readonly<{ questions: PracticeQuestion[]; vocabulary?: VocabularyItem[]; kanji?: KanjiItem[]; items?: LearningItem[]; grammarContrasts?: GrammarContrast[]; learnerErrorAggregates?: IjasAggregate[]; repairId?: string; sessionId?: string }>) {
  ...
  return <>{repair ? <RepairCard repair={repair} vocabulary={vocabulary} kanji={kanji} /> : null}<PracticePlayer questions={weakQuestions.length ? weakQuestions : questions.slice(0, 7)} vocabulary={vocabulary} kanji={kanji} items={items} grammarContrasts={grammarContrasts} sessionId={sessionId} onComplete={() => { if (repairId) completeRepair(repairId); }} /></>;
}
