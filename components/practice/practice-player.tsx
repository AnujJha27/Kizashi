...
import { ListeningScene } from "@/components/learning/listening-scene";
import { ContentFlagButton, ContentReviewControls } from "@/components/library/content-flag-button";
...
function CompletionReview({ questions, answers, signals, filter, onFilter, vocabulary, kanji, readingEntries, items, grammarContrasts, onRepair }: Readonly<{ questions: PracticeQuestion[]; answers: Record<string, boolean>; signals: Record<string, SessionSignal>; filter: CompletionFilter; onFilter: (filter: CompletionFilter) => void; vocabulary: VocabularyItem[]; kanji: KanjiItem[]; readingEntries: [string, string][]; items: LearningItem[]; grammarContrasts: GrammarContrast[]; onRepair: (question: PracticeQuestion) => void }>) {
  ...
  const conceptLabel = (itemId: string) => [...items, ...vocabulary, ...kanji].find((item) => item.id === itemId)?.title ?? itemId;
  const contrastLabel = (question: PracticeQuestion) => grammarContrasts.find((contrast) => contrast.grammarPointIds.some((itemId) => question.targetItemIds?.includes(itemId)))?.title;
  return <section className="mx-auto mt-7 max-w-2xl border-t border-white/10 pt-6 text-left"><div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow mb-2">Question review · 見直し</p><h3 className="text-lg font-medium text-[#f5f5f2]">Turn the signals into another pass.</h3></div><p className="text-xs text-[#9297a1]">Wrong, uncertain, and slow answers stay visible.</p></div><div className="mt-4 flex flex-wrap gap-2" role="tablist" aria-label="Filter question review">{(Object.keys(labels) as CompletionFilter[]).map((value) => <button key={value} type="button" role="tab" aria-selected={filter === value} onClick={() => onFilter(value)} className={`rounded-lg border px-3 py-2 text-xs ${filter === value ? "border-[#e5b85c] bg-[#302818] text-[#f1cf7c]" : "border-[#3f4652] text-[#9297a1] hover:border-[#e5b85c]"}`}>{labels[value]} · {counts[value]}</button>)}</div>{visible.length ? <div className="mt-4 space-y-2">{visible.map((question) => { const signal = signals[question.id]; const tags = [answers[question.id] === false ? "wrong" : "correct", signal?.confidence === "guess" || signal?.confidence === "unsure" ? signal.confidence : "", (signal?.responseMs ?? 0) >= 12000 ? "slow" : ""].filter(Boolean); const needsRepair = answers[question.id] === false || signal?.confidence === "guess" || signal?.confidence === "unsure" || (signal?.responseMs ?? 0) >= 12000; return <article key={question.id} className="rounded-xl border border-white/10 bg-[#101b2b]/70 p-4"><div className="flex flex-wrap items-center gap-2 text-[10px] uppercase tracking-[.12em]">{tags.map((tag) => <span key={tag} className={tag === "wrong" ? "text-[#ef675d]" : tag === "correct" ? "text-[#6fb98f]" : "text-[#8cc9e5]"}>{tag}</span>)}<span className="text-[#e5b85c]">{question.category}</span><span className="text-[#676c75]">{question.questionType}</span></div>{contrastLabel(question) ? <p className="mt-2 text-xs font-semibold uppercase tracking-[.12em] text-[#e5b85c]">Contrast: {contrastLabel(question)}</p> : null}{question.contextText && question.prompt.indexOf(question.contextText) === -1 ? <p className="mt-3 whitespace-pre-line rounded-lg border border-[#4b3a29] bg-[#211d18]/60 p-3 text-sm leading-6 text-[#f5f5f2]"><LearningText text={question.contextText} vocabulary={vocabulary} kanji={kanji} readingEntries={readingEntries} examMode={false} /></p> : null}<p className="mt-2 whitespace-pre-line text-sm leading-6 text-[#f5f5f2]"><LearningText text={question.prompt} vocabulary={vocabulary} kanji={kanji} readingEntries={readingEntries} examMode={false} /></p><p className="mt-3 text-xs text-[#e5b85c]">Answer: {answerText(question)}</p>{question.testedSkills?.length ? <p className="mt-2 text-xs text-[#9297a1]">Concept focus: {question.testedSkills.join(" · ")}</p> : null}{question.targetItemIds?.length ? <p className="mt-2 text-xs text-[#9297a1]">Related concepts: {question.targetItemIds.map(conceptLabel).join(" · ")}</p> : null}<div className="mt-3 flex flex-wrap items-start justify-between gap-3"><p className="text-xs leading-5 text-[#9297a1]">{question.explanation}</p><div className="flex shrink-0 flex-wrap gap-3"><Link href={`/entry/${question.itemId}`} className="text-xs font-semibold text-[#e5b85c] hover:text-[#f1cf7c]">Entry →</Link>{needsRepair ? <button type="button" onClick={() => onRepair(question)} className="text-xs font-semibold text-[#ef675d] hover:text-[#f58b83]">Start repair</button> : null}</div></div></article>; })}</div> : <p className="mt-4 rounded-xl border border-[#315d4b] bg-[#162b26]/70 p-4 text-sm text-[#c6ded2]">Nothing in this group. Keep the clean answers warm.</p>}</section>;
}

function LearningText({ text, vocabulary, kanji, readingEntries, examMode }: Readonly<{ text: string; vocabulary: VocabularyItem[]; kanji: KanjiItem[]; readingEntries?: [string, string][]; examMode: boolean }>) {
  return examMode ? <>{text}</> : <JapaneseText text={text} vocabulary={vocabulary} kanji={kanji} readingEntries={readingEntries} always inspect={false} />;
}

export function PracticePlayer({ questions, vocabulary = [], kanji = [], readingEntries = [], items = [], grammarContrasts = [], examMode = false, targetLevel = "N5", examLabel = "N5 sampler", onComplete, onRestart, sessionId, timeLimitSeconds }: Readonly<{ questions: PracticeQuestion[]; vocabulary?: VocabularyItem[]; kanji?: KanjiItem[]; readingEntries?: [string, string][]; items?: LearningItem[]; grammarContrasts?: GrammarContrast[]; examMode?: boolean; targetLevel?: TargetLevel; examLabel?: string; onComplete?: (result: CompletionResult) => void; onRestart?: () => void; sessionId?: string; timeLimitSeconds?: number }>) {
  ...
  const submit = () => {
    ...
    if (examMode && (!isCorrect || confidence === "guess" || confidence === "unsure" || (responseMs ?? 0) >= 12000)) recordMistake(question.itemId, question.questionType, question.contextSetId, question.targetItemIds);
