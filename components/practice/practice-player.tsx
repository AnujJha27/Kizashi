...
function CompletionReview({ questions, answers, signals, filter, onFilter, vocabulary, kanji, items, grammarContrasts, onRepair }: Readonly<{ questions: PracticeQuestion[]; answers: Record<string, boolean>; signals: Record<string, SessionSignal>; filter: CompletionFilter; onFilter: (filter: CompletionFilter) => void; vocabulary: VocabularyItem[]; kanji: KanjiItem[]; items: LearningItem[]; grammarContrasts: GrammarContrast[]; onRepair: (question: PracticeQuestion) => void }>) {
  void items;
...
function LearningText({ text, vocabulary, kanji, examMode }: Readonly<{ text: string; vocabulary: VocabularyItem[]; kanji: KanjiItem[]; examMode: boolean }>) {
  return examMode ? <>{text}</> : <JapaneseText text={text} vocabulary={vocabulary} kanji={kanji} always inspect={false} />;
}
