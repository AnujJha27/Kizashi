...
export function CommandPalette({ isAdmin }: Readonly<{ isAdmin: boolean }>) {
  ...
  useEffect(() => {
    if (wasOpen.current && !open) previousFocus.current?.focus();
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (event.key.toLowerCase() === "a" && !["INPUT", "TEXTAREA", "SELECT"].includes((event.target as HTMLElement).tagName)) { event.preventDefault(); openPalette("add"); }
