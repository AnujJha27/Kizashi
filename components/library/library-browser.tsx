...
type LibraryFilter = "all" | LessonContentItem["category"] | "queue" | "new" | "weak" | "learned" | "provisional" | "reviewed" | "flagged" | "N5" | "N4";
const filters: LibraryFilter[] = ["all", "new", "weak", "learned", "provisional", "reviewed", "flagged", "queue", "N5", "N4", "vocabulary", "kanji", "reading", "listening", "grammar"];
...
function filterLabel(filter: (typeof filters)[number]) {
  return filter === "all" ? "All" : filter === "queue" ? "Study later" : filter === "new" ? "New" : filter === "weak" ? "Weak" : filter === "learned" ? "Learned" : filter === "provisional" ? "Provisional" : filter === "reviewed" ? "Reviewed" : filter === "flagged" ? "Flagged" : filter === "N5" || filter === "N4" ? filter : categoryLabel(filter);
}
