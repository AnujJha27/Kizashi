...
import { CONTENT_FLAG_REASONS, readContentFlags, readContentReview, setContentCorrection, setContentReview, setQuestionCorrection, toggleContentFlag } from "@/lib/content-flags.js";
...
export function ContentReviewControls({ itemId, category, reviewStatus, tags, origin, question = false, editValue = "", questionOptions = [], questionCorrectIndex = 0 }: Readonly<{ itemId: string; category: ReviewCategory; reviewStatus?: unknown; tags?: unknown; origin: ReviewOrigin; question?: boolean; editValue?: string; questionOptions?: string[]; questionCorrectIndex?: number }>) {
  const provisional = question || getContentReviewStatus({ reviewStatus, tags }) === "pending";
  const questionOptionsText = questionOptions.join("\n");
  ...
  const [questionCorrection, setQuestionCorrectionText] = useState(questionOptionsText);
  const [questionCorrectChoice, setQuestionCorrectChoice] = useState(questionCorrectIndex);
  ...
  useEffect(() => {
    ...
    setQuestionCorrectionText(questionOptionsText);
    setQuestionCorrectChoice(questionCorrectIndex);
    window.addEventListener("michi-content-flagged-updated", refresh);
    return () => window.removeEventListener("michi-content-flagged-updated", refresh);
  }, [editValue, itemId, questionCorrectIndex, questionOptionsText]);
