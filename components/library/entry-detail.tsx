...
function GrammarDetail({ item, contrasts, vocabulary, kanji }: Readonly<{ item: GrammarItem; contrasts: GrammarContrast[]; vocabulary: VocabularyItem[]; kanji: KanjiItem[] }>) {
  ...
  return <><p className="jp-serif text-3xl text-[#e5b85c]"><JapaneseText text={item.pattern} vocabulary={vocabulary} kanji={kanji} /></p><AudioControls text={item.pattern} metadata={item.audio} className="mt-4" /><h2 className="mt-5 text-xl font-medium">{item.meaning}</h2><p className="mt-3 text-sm leading-7 text-[#9297a1]">{item.formation}</p><p className="mt-5 text-sm leading-7 text-[#c3a998]">{item.intuition}</p><div className="mt-7"><p className="eyebrow mb-3">Usage conditions</p><ul className="space-y-2 text-sm leading-6 text-[#c3c7ce]">{item.usageConditions.map((condition) => <li key={condition}>— {condition}</li>)}</ul></div><div className="mt-7 space-y-3">{item.examples.map((example) => <div key={example.japanese} className="rounded-xl bg-[#101b2b]/70 p-4"><p className="jp-serif text-lg text-[#e5b85c]"><JapaneseText text={example.japanese} vocabulary={vocabulary} kanji={kanji} /></p><p className="mt-1 text-sm text-[#9297a1]">{example.translation}</p><AudioControls text={example.japanese} metadata={example.audio} className="mt-3" /><GrammarVisual sentence={example.japanese} vocabulary={vocabulary} kanji={kanji} /><SaveSentence sourceItemId={item.id} japanese={example.japanese} translation={example.translation} /></div>)}</div><div className="mt-7 rounded-xl border border-[#713b37]/70 bg-[#21191a]/60 p-4"><p className="eyebrow mb-2">Common mistake</p><p className="text-sm leading-6 text-[#c3c7ce]">{item.commonMistakes.join(" ")}</p></div>{contrast ? <div className="mt-7 border-l-2 border-[#e34a3f] pl-4"><p className="eyebrow">Contrast · {contrast.title}</p><p className="mt-2 text-sm leading-6 text-[#c3a998]">{contrast.explanation}</p></div> : null}<SourceReferencePanel grammarId={item.id} /></>;
}
...
function OtherDetail({ item, vocabulary, kanji }: Readonly<{ item: ReadingItem | ListeningItem; vocabulary: VocabularyItem[]; kanji: KanjiItem[] }>) {
  if (item.category === "reading") return <><AudioControls text={item.passage} metadata={item.audio} className="mt-6" /><ReadingPanel item={item} vocabulary={vocabulary} kanji={kanji} /></>;
  return <><p className="text-sm leading-7 text-[#9297a1]">{item.situation}</p><AudioControls text={item.transcript} externalUrl={item.audioUrl} metadata={item.audio} preferredRate={item.speed} className="mt-6" /><p className="jp-serif mt-6 whitespace-pre-line text-lg leading-8 text-[#f5f5f2]"><JapaneseText text={item.transcript} vocabulary={vocabulary} kanji={kanji} /></p><p className="mt-6 text-sm text-[#9297a1]">{item.questions.length} listening questions ready in Practice.</p></>;
}
...
export function EntryDetail({ item, contrasts, vocabulary = n5Module.vocabulary, kanji = n5Module.kanji }: Readonly<{ item: Entry; contrasts: GrammarContrast[]; vocabulary?: VocabularyItem[]; kanji?: KanjiItem[] }>) {
  const detail = item.category === "vocabulary" ? <VocabularyDetail item={item} vocabulary={vocabulary} kanji={kanji} /> : item.category === "kanji" ? <KanjiDetail item={item} vocabulary={vocabulary} /> : item.category === "grammar" ? <GrammarDetail item={item} contrasts={contrasts} vocabulary={vocabulary} kanji={kanji} /> : <OtherDetail item={item} vocabulary={vocabulary} kanji={kanji} />;
  ...
  return <div className="mx-auto max-w-4xl"><div className="mb-6"><Link href="/library" className="text-sm text-[#e5b85c] hover:text-[#f1cf7c]">← Back to Library</Link></div><section className="surface-panel overflow-hidden p-7 sm:p-10"><div className="mb-8 flex items-start justify-between gap-4"><div><p className="eyebrow">{item.category} · {item.jlptLevel ?? "open"}</p><h1 className="mt-2 text-2xl font-medium text-[#f5f5f2]">{item.title}</h1><p className="mt-1 text-xs text-[#9297a1]">{item.subcategory ?? "N5 Foundations"} · difficulty {item.difficulty}</p></div><div className="flex flex-col items-end gap-3"><span className="seal" aria-label={`${item.jlptLevel ?? "Open"} entry`}><span>{item.jlptLevel ?? "—"}</span><small>道</small></span><StudyLaterButton itemId={item.id} /><ContentFlagButton itemId={item.id} /></div></div><div className="mb-7 flex flex-wrap gap-2 text-[10px] text-[#676c75]"><span className="rounded-md border border-white/10 px-2 py-1">{classification ? `${classification.band} · ${classification.confidence} confidence` : "unclassified"}</span>{autoReleased ? <span className="rounded-md border border-[#5d4c2c] bg-[#302818] px-2 py-1 text-[#e5b85c]">Auto-released · not human reviewed</span> : null}{item.sourceIds?.map((sourceId) => <span key={sourceId} className="rounded-md border border-white/10 px-2 py-1">source · {sourceId}</span>)}</div>{detail}<LearnerAssistant defaultText={assistantText} itemId={item.id} /><div className="mt-8 flex flex-wrap gap-3 border-t border-white/10 pt-5"><Link href={`/practice?mode=${item.category}`} className="inline-flex rounded-xl bg-[#e34a3f] px-4 py-3 text-sm font-semibold text-[#0b0b0d] hover:bg-[#ef675d]">Practice {item.category} <span className="ml-2" aria-hidden="true">→</span></Link><Link href="/immersion" className="inline-flex rounded-xl border border-[#3f4652] px-4 py-3 text-sm text-[#c3c7ce] hover:border-[#e5b85c]">{immersionLabel} <span className="ml-2" aria-hidden="true">→</span></Link></div><ItemNote itemId={item.id} /></section></div>;
}
