...
import { KanjiWordWriting } from "@/components/learning/kanji-word-writing";
...
export function EntryDetail({ item, contrasts, vocabulary = n5Module.vocabulary, kanji = n5Module.kanji }: Readonly<{ item: Entry; contrasts: GrammarContrast[]; vocabulary?: VocabularyItem[]; kanji?: KanjiItem[] }>) {
  ...
  return <div className="mx-auto max-w-4xl"><div className="mb-6"><Link href="/library" className="text-sm text-[#e5b85c] hover:text-[#f1cf7c]">← Back to Library</Link></div><section className="surface-panel overflow-hidden p-7 sm:p-10"><div className="mb-8 flex items-start justify-between gap-4"><div><p className="eyebrow">{item.category} · {item.jlptLevel ?? "open"}</p><h1 className="mt-2 text-2xl font-medium text-[#f5f5f2]">{title}</h1><p className="mt-1 text-xs text-[#9297a1]">{item.subcategory ?? "N5 Foundations"} · difficulty {item.difficulty}</p></div><div className="flex flex-col items-end gap-3"><span className="seal" aria-label={`${item.jlptLevel ?? "Open"} entry`}><span>{item.jlptLevel ?? "—"}</span><small>道</small></span><StudyLaterButton itemId={item.id} /><ContentFlagButton itemId={item.id} /><ContentReviewControls itemId={item.id} category={item.category} reviewStatus={item.reviewStatus} tags={item.tags} origin="library" editValue={item.category === "grammar" ? item.intuition : "meanings" in item ? item.meanings.join(" · ") : ""} /></div></div><div className="mb-7 flex flex-wrap gap-2 text-[10px] text-[#676c75]"><span className="rounded-md border border-white/10 px-2 py-1">{classification ? `${classification.band} · ${classification.confidence} confidence` : "unclassified"}</span>{autoReleased ? <span className="rounded-md border border-[#5d4c2c] bg-[#302818] px-2 py-1 text-[#e5b85c]">Auto-released · not human reviewed</span> : null}{item.sourceIds?.map((sourceId) => <span key={sourceId} className="rounded-md border border-white/10 px-2 py-1">source · {sourceId}</span>)}</div>{detail}<LearnerAssistant defaultText={assistantText} itemId={item.id} vocabulary={vocabulary} kanji={kanji} /><div className="mt-8 flex flex-wrap gap-3 border-t border-white/10 pt-5"><Link href={`/practice?mode=${item.category}`} className="inline-flex rounded-xl bg-[#e34a3f] px-4 py-3 text-sm font-semibold text-[#0b0b0d] hover:bg-[#ef675d]">Practice {item.category} <span className="ml-2" aria-hidden="true">→</span></Link>{item.category === "kanji" ? <Link href="/practice?mode=kanji-writing" className="inline-flex rounded-xl border border-[#5d3936] px-4 py-3 text-sm font-semibold text-[#f5f5f2] hover:border-[#e5b85c]">Practice writing <span className="ml-2" aria-hidden="true">→</span></Link> : null}<Link href="/immersion" className="inline-flex rounded-xl border border-[#3f4652] px-4 py-3 text-sm text-[#c3c7ce] hover:border-[#e5b85c]">{immersionLabel} <span className="ml-2" aria-hidden="true">→</span></Link></div><ItemNote itemId={item.id} /></section></div>;
}
