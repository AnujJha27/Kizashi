...
export function SaveSentence({ sourceItemId, japanese, translation }: Readonly<{ sourceItemId: string; japanese: string; translation: string }>) {
  ...
  return <button type="button" onClick={() => setSaved(toggleSavedSentence({ id, sourceItemId, japanese, translation }))} className="mt-2 text-xs font-semibold text-[#9297a1] hover:text-[#e5b85c]" aria-pressed={saved}>{saved ? "★ Saved sentence" : "☆ Save sentence"}</button>;
}
