...
import { getDueReviewIds, getTodayStudyMinutes, readDailyGoal, readExamAttempts, readKanjiWritingProgress, readReviewRecords } from "@/lib/session";
...
export function DailySession({ lessonId, items, allItems = items, module, targetLevel = "N5", world }: Readonly<{ lessonId: string; items: LessonContentItem[]; allItems?: LessonContentItem[]; module: N5Module; targetLevel?: TargetLevel; world?: { area: { title: string; japaneseTitle: string; description: string; visualAssets: { today: string }; focalPoint: { desktop: string; mobile: string } }; stage: { label: string } } }>) {
  ...
  const writingProgress = readKanjiWritingProgress();
  const writingCandidates = items.filter((item) => item.category === "kanji" && writingProgress[item.id]?.state !== "written-from-memory").slice(0, 3);
  const showWritingRecommendation = writingCandidates.length > 0 && (priority === "kanji" || due === 0);
