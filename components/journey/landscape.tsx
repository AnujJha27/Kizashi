...
export function Landscape({ areaId = "neighborhood", stageId = "arrival" }: Readonly<{ areaId?: string; stageId?: string }>) {
  ...
  return (
    <div className="absolute inset-0 overflow-hidden opacity-95" data-world-area={areaId} data-world-stage={stageId} aria-hidden="true" style={{ "--world-focal": visual.focalPoint.desktop, "--world-focal-mobile": visual.focalPoint.mobile } as CSSProperties}>
      <img src={visual.visualAssets.map} alt="" width={972} height={1619} loading="lazy" decoding="async" className="world-scene-image h-full w-full object-cover" onError={(event) => { event.currentTarget.hidden = true; }} />
      <div className="absolute inset-0 bg-gradient-to-b from-[#111216]/95 via-[#111216]/70 to-[#111216]/45" />
