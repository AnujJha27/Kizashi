...
function ContextField({ value, onChange }: Readonly<{ value: unknown; onChange: (value: DraftRecord) => void }>) {
  const context = typeof value === "object" && value !== null && !Array.isArray(value) ? value as DraftRecord : {};
  return <div className="rounded-lg border border-white/10 bg-[#101b2b]/60 p-3"><p className="text-xs text-[#a8adb7]">Mini dialogue/context</p><div className="mt-2 space-y-3"><TextArea label="Japanese context" value={stringValue(context.japanese)} onChange={(next) => onChange({ ...context, japanese: next })} rows={2} /><TextArea label="Context translation" value={stringValue(context.translation)} onChange={(next) => onChange({ ...context, translation: next })} rows={2} /></div></div>;
}
...
function ItemFieldsContent({ kind, record, update }: Readonly<{ kind: EditableKind; record: DraftRecord; update: (field: string, value: unknown) => void }>) {
  ...
  if (kind === "kanji") return <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-3"><Input label="Character" value={stringValue(record.character)} onChange={(value) => update("character", value)} /><Input label="Stroke count" type="number" value={numberValue(record.strokeCount, 0)} onChange={(value) => update("strokeCount", Number(value) || undefined)} /><Input label="School grade" type="number" value={numberValue(record.grade, 0)} onChange={(value) => update("grade", Number(value) || undefined)} /></div><div className="grid gap-3 sm:grid-cols-3"><ListField label="Meanings" values={stringList(record.meanings)} onChange={(value) => update("meanings", value)} /><ListField label="On readings" values={stringList(record.onyomi)} onChange={(value) => update("onyomi", value)} /><ListField label="Kun readings" values={stringList(record.kunyomi)} onChange={(value) => update("kunyomi", value)} /></div><div className="grid gap-3 sm:grid-cols-4"><Input label="Radical" value={stringValue(record.radical)} onChange={(value) => update("radical", value)} /><ListField label="Nanori" values={stringList(record.nanori)} onChange={(value) => update("nanori", value)} /><ListField label="Components" values={stringList(record.components)} onChange={(value) => update("components", value)} /><ListField label="Visual comparisons" values={stringList(record.confusableKanji)} onChange={(value) => update("confusableKanji", value)} hint="One kanji per line; provisional and owner-editable." /></div><TextArea label="Mnemonic" value={stringValue(record.mnemonic)} onChange={(value) => update("mnemonic", value)} /><Input label="Stroke-order reference" value={stringValue(record.strokeOrder)} onChange={(value) => update("strokeOrder", value)} /><UsefulWordsField values={recordList(record.usefulWords)} onChange={(value) => update("usefulWords", value)} /></div>;
  if (kind === "grammar") return <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-2"><Input label="Pattern" value={stringValue(record.pattern)} onChange={(value) => update("pattern", value)} /><Input label="Meaning" value={stringValue(record.meaning)} onChange={(value) => update("meaning", value)} /></div><Input label="Formation" value={stringValue(record.formation)} onChange={(value) => update("formation", value)} /><TextArea label="Intuition" value={stringValue(record.intuition)} onChange={(value) => update("intuition", value)} /><div className="grid gap-3 sm:grid-cols-2"><ListField label="Aliases / variants" values={stringList(record.aliases)} onChange={(value) => update("aliases", value)} hint="One learner-facing name or variant per line." /><ContextField value={record.context} onChange={(value) => update("context", value)} /></div><div className="grid gap-3 sm:grid-cols-2"><ListField label="Usage conditions" values={stringList(record.usageConditions)} onChange={(value) => update("usageConditions", value)} /><ListField label="Common mistakes" values={stringList(record.commonMistakes)} onChange={(value) => update("commonMistakes", value)} /></div><div className="grid gap-3 sm:grid-cols-2"><ListField label="Contrast IDs" values={stringList(record.contrastIds)} onChange={(value) => update("contrastIds", value)} /><ListField label="Practice question IDs" values={stringList(record.practiceQuestionIds)} onChange={(value) => update("practiceQuestionIds", value)} /></div><ExamplesField values={recordList(record.examples)} onChange={(value) => update("examples", value)} /></div>;
