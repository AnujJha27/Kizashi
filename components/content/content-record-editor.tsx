...
function ItemFieldsContent({ kind, record, update }: Readonly<{ kind: EditableKind; record: DraftRecord; update: (field: string, value: unknown) => void }>) {
  if (kind === "vocabulary") return <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-3"><Input label="Written form" value={stringValue(record.writtenForm)} onChange={(value) => update("writtenForm", value)} /><Input label="Reading" value={stringValue(record.reading)} onChange={(value) => update("reading", value)} /><Input label="Part of speech" value={stringValue(record.partOfSpeech)} onChange={(value) => update("partOfSpeech", value)} /></div><div className="grid gap-3 sm:grid-cols-2"><Input label="Commonness (1–5)" type="number" value={numberValue(record.commonness, 0)} onChange={(value) => update("commonness", Number(value) || undefined)} hint="Curated beginner frequency signal." /><Input label="Written frequency" type="number" value={numberValue(record.frequency, 0)} onChange={(value) => update("frequency", Number(value) || undefined)} hint="Keep the source and corpus in field provenance." /></div><div className="grid gap-3 sm:grid-cols-3"><ListField label="Meanings" values={stringList(record.meanings)} onChange={(value) => update("meanings", value)} /><ListField label="Collocations" values={stringList(record.collocations)} onChange={(value) => update("collocations", value)} /><ListField label="Related words" values={stringList(record.relatedWords)} onChange={(value) => update("relatedWords", value)} /></div><ListField label="Antonyms" values={stringList(record.antonyms)} onChange={(value) => update("antonyms", value)} /><TextArea label="Notes" value={stringValue(record.notes)} onChange={(value) => update("notes", value)} /><ExamplesField values={recordList(record.exampleSentences)} onChange={(value) => update("exampleSentences", value)} /></div>;
