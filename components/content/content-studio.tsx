...
function DifficultyAudit({ module }: Readonly<{ module: N5Module }>) {
  ...
  return <section className="rounded-xl border border-[#3f3427] bg-[#211d18]/55 p-4 sm:p-5"><div className="flex flex-wrap items-end justify-between gap-2"><div><p className="eyebrow">Level calibration signal</p><h2 className="mt-1 text-lg font-medium text-[#f5f5f2]">Check the authored difficulty metadata.</h2><p className="mt-1 text-xs leading-5 text-[#c3a96d]">N5/N4 difficulty and source-length fields are visible by bank and level; this is structural evidence for review, not a native-language calibration verdict.</p></div><span className="text-[10px] uppercase tracking-[.12em] text-[#e5b85c]">automated signal</span></div><div className="mt-4 grid gap-3 text-xs sm:grid-cols-2">{(["reading", "listening"] as const).map((category) => <div key={category} className="rounded-lg border border-white/10 bg-[#17181d]/55 p-3"><p className="text-[10px] uppercase tracking-[.12em] text-[#676c75]">{category}</p><p className="mt-2 text-[#c3c7ce]">N5 {report[category].difficultyByLevel.N5.average ?? "—"} average · N4 {report[category].difficultyByLevel.N4.average ?? "—"} average</p><p className="mt-1 text-[10px] text-[#676c75]">N5 {report[category].difficultyByLevel.N5.count} records · N4 {report[category].difficultyByLevel.N4.count} records</p><p className="mt-1 text-[10px] text-[#676c75]">Length · N5 {report[category].complexity.byLevel.N5.averageLines.toFixed(1)} lines / {report[category].complexity.byLevel.N5.averageCharacters} chars · N4 {report[category].complexity.byLevel.N4.averageLines.toFixed(1)} lines / {report[category].complexity.byLevel.N4.averageCharacters} chars</p></div>)}</div></section>;
}
...
function ListeningStructureAudit({ module }: Readonly<{ module: N5Module }>) {
  ...
  return <section className="rounded-xl border border-[#5d4c2c] bg-[#2b2418]/55 p-4 sm:p-5"><div className="flex flex-wrap items-end justify-between gap-2"><div><p className="eyebrow">Listening structure signals</p><h2 className="mt-1 text-lg font-medium text-[#f5f5f2]">See repetition before human review.</h2><p className="mt-1 text-xs leading-5 text-[#c3a96d]">Deterministic shape checks expose repeated turn patterns and answer echoes; they do not judge native naturalness.</p></div><span className="text-[10px] uppercase tracking-[.12em] text-[#e5b85c]">automated signal</span></div><div className="mt-4 grid gap-3 text-xs sm:grid-cols-3"><div className="rounded-lg border border-white/10 bg-[#17181d]/55 p-3"><p className="text-[10px] uppercase tracking-[.12em] text-[#676c75]">Turn profiles</p><p className="mt-2 text-[#c3c7ce]">{turnProfiles || "None"}</p></div><div className="rounded-lg border border-white/10 bg-[#17181d]/55 p-3"><p className="text-[10px] uppercase tracking-[.12em] text-[#676c75]">Speaker patterns</p><p className="mt-2 text-[#c3c7ce]">{speakerPatterns || "None"}</p></div><div className="rounded-lg border border-white/10 bg-[#17181d]/55 p-3"><p className="text-[10px] uppercase tracking-[.12em] text-[#676c75]">Answer echoes</p><p className="mt-2 text-[#e5b85c]">{structure.answerEchoes} question answers repeat transcript wording</p><EchoReviewList details={structure.answerEchoDetails} /></div></div></section>;
}
...
function ReviewExamples({ examples, vocabulary, kanji }: Readonly<{ examples: ExampleSentence[]; vocabulary: VocabularyItem[]; kanji: KanjiItem[] }>) {
  return <section><p className="text-[10px] font-semibold uppercase tracking-[.12em] text-[#e5b85c]">Examples</p><div className="mt-2 space-y-2">{examples.length ? examples.map((example, index) => <div key={`${example.japanese}-${index}`} className="rounded-lg border border-white/10 bg-[#101b2b]/60 p-3"><p className="jp-serif text-base text-[#f5f5f2]"><JapaneseText text={example.japanese} vocabulary={vocabulary} kanji={kanji} always inspect={false} /></p><p className="mt-1 text-xs text-[#9297a1]">{example.translation}</p>{example.note ? <p className="mt-1 text-[11px] text-[#e5b85c]">{example.note}</p> : null}</div>) : <p className="text-sm text-[#676c75]">None recorded.</p>}</div></section>;
}

function ReviewQuestions({ questions, vocabulary, kanji }: Readonly<{ questions: Array<{ prompt: string; choices: string[]; correctAnswer: number; questionType?: string; explanation?: string }>; vocabulary: VocabularyItem[]; kanji: KanjiItem[] }>) {
  return <section><p className="text-[10px] font-semibold uppercase tracking-[.12em] text-[#e5b85c]">Questions</p><div className="mt-2 space-y-2">{questions.length ? questions.map((question, index) => <div key={`${question.prompt}-${index}`} className="rounded-lg border border-white/10 bg-[#101b2b]/60 p-3"><p className="text-sm text-[#f5f5f2]"><JapaneseText text={question.prompt} vocabulary={vocabulary} kanji={kanji} always inspect={false} /></p>{question.questionType ? <p className="mt-1 text-[10px] uppercase tracking-[.1em] text-[#676c75]">{question.questionType}</p> : null}<div className="mt-2 flex flex-wrap gap-2">{question.choices.map((choice, choiceIndex) => <span key={`${choice}-${choiceIndex}`} className={`rounded-md px-2 py-1 text-xs ${choiceIndex === question.correctAnswer ? "bg-[#183225] text-[#8bcca6]" : "bg-[#17181d] text-[#9297a1]"}`}><JapaneseText text={choice} vocabulary={vocabulary} kanji={kanji} always inspect={false} /></span>)}</div>{question.explanation ? <p className="mt-2 text-xs leading-5 text-[#9297a1]">{question.explanation}</p> : null}</div>) : <p className="text-sm text-[#676c75]">None recorded.</p>}</div></section>;
}

function ReadableRecord({ item, vocabulary, kanji }: Readonly<{ item: LessonContentItem; vocabulary: VocabularyItem[]; kanji: KanjiItem[] }>) {
  if (item.category === "vocabulary") return <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-3"><ReviewField label="Japanese"><JapaneseText text={item.writtenForm} vocabulary={vocabulary} kanji={kanji} always inspect={false} /></ReviewField><ReviewField label="Reading"><span className="jp-serif text-xl text-[#f5f5f2]">{item.reading}</span></ReviewField><ReviewField label="Part of speech">{item.partOfSpeech}</ReviewField></div><ReviewField label="Meanings"><ReviewPills values={item.meanings} /></ReviewField><ReviewExamples examples={item.exampleSentences} vocabulary={vocabulary} kanji={kanji} /><div className="grid gap-3 sm:grid-cols-3"><ReviewField label="Collocations"><ReviewPills values={item.collocations} /></ReviewField><ReviewField label="Related words"><ReviewPills values={item.relatedWords} /></ReviewField><ReviewField label="Antonyms"><ReviewPills values={item.antonyms} /></ReviewField></div>{item.notes ? <ReviewField label="Notes">{item.notes}</ReviewField> : null}</div>;
  if (item.category === "kanji") return <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-3"><ReviewField label="Character"><JapaneseText text={item.character} vocabulary={vocabulary} kanji={kanji} always inspect={false} /></ReviewField><ReviewField label="On'yomi"><ReviewPills values={item.onyomi} /></ReviewField><ReviewField label="Kun'yomi"><ReviewPills values={item.kunyomi} /></ReviewField></div><ReviewField label="Meanings"><ReviewPills values={item.meanings} /></ReviewField><ReviewField label="Useful words"><div className="space-y-2">{item.usefulWords.length ? item.usefulWords.map((word) => <div key={`${word.word}-${word.reading}`}><JapaneseText text={word.word} vocabulary={vocabulary} kanji={kanji} always inspect={false} /><span className="ml-2 text-xs text-[#e5b85c]">{word.reading}</span><span className="ml-2 text-xs text-[#9297a1]">{word.meaning}</span></div>) : <span className="text-[#676c75]">None recorded.</span>}</div></ReviewField>{item.confusableKanji?.length ? <ReviewField label="Visual comparisons · provisional"><ReviewPills values={item.confusableKanji} /></ReviewField> : null}<div className="grid gap-3 sm:grid-cols-3"><ReviewField label="Stroke count">{item.strokeCount ?? "Not recorded"}</ReviewField><ReviewField label="Grade">{item.grade ?? "Not recorded"}</ReviewField><ReviewField label="Radical">{item.radical ?? "Not recorded"}</ReviewField></div>{item.mnemonic ? <ReviewField label="Mnemonic">{item.mnemonic}</ReviewField> : null}</div>;
  if (item.category === "grammar") return <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-2"><ReviewField label="Pattern"><JapaneseText text={item.pattern} vocabulary={vocabulary} kanji={kanji} always inspect={false} /></ReviewField><ReviewField label="Meaning">{item.meaning}</ReviewField></div><ReviewField label="Formation">{item.formation}</ReviewField><ReviewField label="Intuition">{item.intuition}</ReviewField><div className="grid gap-3 sm:grid-cols-2"><ReviewField label="Aliases / variants"><ReviewPills values={item.aliases ?? []} /></ReviewField><ReviewField label="Mini dialogue/context">{item.context ? <><p className="jp-serif text-[#f5f5f2]"><JapaneseText text={item.context.japanese} vocabulary={vocabulary} kanji={kanji} always inspect={false} /></p><p className="mt-1 text-xs text-[#9297a1]">{item.context.translation}</p></> : <span className="text-[#676c75]">None recorded.</span>}</ReviewField></div><ReviewField label="Usage conditions"><ul className="list-disc space-y-1 pl-5">{item.usageConditions.map((condition) => <li key={condition}>{condition}</li>)}</ul></ReviewField><ReviewExamples examples={item.examples} vocabulary={vocabulary} kanji={kanji} /><ReviewField label="Common mistakes"><ReviewPills values={item.commonMistakes} /></ReviewField><div className="grid gap-3 sm:grid-cols-2"><ReviewField label="Contrast IDs"><ReviewPills values={item.contrastIds} /></ReviewField><ReviewField label="Practice question IDs"><ReviewPills values={item.practiceQuestionIds} /></ReviewField></div></div>;
  if (item.category === "reading") return <div className="space-y-4"><ReviewField label="Passage"><p className="jp-serif whitespace-pre-wrap text-base text-[#f5f5f2]"><JapaneseText text={item.passage} vocabulary={vocabulary} kanji={kanji} always inspect={false} /></p></ReviewField><ReviewField label="Translation">{item.translation}</ReviewField><div className="grid gap-3 sm:grid-cols-3"><ReviewField label="Vocabulary links"><ReviewPills values={item.vocabularyIds} /></ReviewField><ReviewField label="Grammar links"><ReviewPills values={item.grammarIds} /></ReviewField><ReviewField label="Kanji links"><ReviewPills values={item.kanjiIds} /></ReviewField></div><ReviewQuestions questions={(item.questions ?? []).map((question) => ({ ...question, choices: question.options }))} vocabulary={vocabulary} kanji={kanji} /></div>;
  return <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-3"><ReviewField label="Situation">{item.situation}</ReviewField><ReviewField label="Source type">{item.sourceType}</ReviewField><ReviewField label="Voice / rate">{item.voice} · {item.speed}×</ReviewField></div><ReviewField label="Transcript"><p className="whitespace-pre-wrap text-[#f5f5f2]"><JapaneseText text={item.transcript} vocabulary={vocabulary} kanji={kanji} always inspect={false} /></p></ReviewField><ReviewQuestions questions={item.questions.map((question) => ({ ...question, choices: question.answers }))} vocabulary={vocabulary} kanji={kanji} /></div>;
}

function ContentReviewModal({ item, sourceById, vocabulary, kanji, onClose, onEdit, onReview }: Readonly<{
  item: ReviewItem;
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener("keydown", closeOnEscape); previousFocus.current?.focus(); };
  }, [onClose]);

  return <div className="fixed inset-0 z-[100] flex items-start justify-center overflow-y-auto bg-[#05070b]/75 p-4 backdrop-blur-md sm:p-8" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby={`content-review-${item.id}`} className="my-4 w-full max-w-4xl overflow-hidden rounded-2xl border border-[#e5b85c]/60 bg-[#0d1522] shadow-[0_24px_90px_rgba(0,0,0,.55)]"><div className="flex items-start justify-between gap-4 border-b border-white/10 bg-[#101b2b] px-5 py-4 sm:px-7"><div><p className="eyebrow">Content review · {item.category}</p><h2 id={`content-review-${item.id}`} className="jp-serif mt-1 text-2xl text-[#f5f5f2]"><JapaneseText text={preview.japanese} vocabulary={vocabulary} kanji={kanji} readingEntries={preview.reading ? [[preview.japanese, preview.reading]] : []} always inspect={false} /></h2><p className="mt-1 text-xs text-[#9297a1]">{item.title}</p></div><button ref={closeButtonRef} type="button" onClick={onClose} aria-label="Close content review" className="rounded-lg border border-[#3f4652] px-3 py-2 text-xl leading-none text-[#c3c7ce] hover:border-[#e5b85c] hover:text-[#f1cf7c]">×</button></div><div className="max-h-[calc(100vh-9rem)] space-y-5 overflow-y-auto p-5 sm:p-7"><div className="grid gap-3 sm:grid-cols-3"><ReviewField label="Status"><span className={reviewStatus === "pending" ? "text-[#e5b85c]" : reviewStatus === "approved" ? "text-[#8bcca6]" : "text-[#ef675d]"}>{reviewStatus}</span></ReviewField><ReviewField label="Level">{item.jlptLevel ?? "Unassigned"}</ReviewField><ReviewField label="Difficulty">{item.difficulty} / 5</ReviewField></div><div className="grid gap-3 sm:grid-cols-2"><ReviewField label="Source">{provenance || "Not recorded"}</ReviewField><ReviewField label="Classification">{item.classification ? `${item.classification.level} · ${item.classification.band} · ${item.classification.confidence}${item.classification.conflict ? " · conflict" : ""}` : `${itemBand} · inferred`}</ReviewField></div>{relatedSources.length ? <ReviewField label="External references"><ReviewPills values={relatedSources} /></ReviewField> : null}<ReviewField label="Why this surfaced">{item.reason}</ReviewField><ReadableRecord item={item} vocabulary={vocabulary} kanji={kanji} />{onReview && reviewStatus === "pending" ? <div className="flex flex-wrap gap-2 border-t border-white/10 pt-5"><button type="button" onClick={() => onReview(item.id, "approved")} className="rounded-lg bg-[#6fb98f] px-4 py-2.5 text-xs font-semibold text-[#0b0b0d] hover:bg-[#8bcca6]">Approve</button><button type="button" onClick={() => onReview(item.id, "rejected")} className="rounded-lg border border-[#713b37] px-4 py-2.5 text-xs font-semibold text-[#ef675d] hover:border-[#ef675d]">Reject</button></div> : null}<div className="flex flex-wrap gap-2 border-t border-white/10 pt-5"><button type="button" onClick={() => { onClose(); onEdit(item.category, item.id); }} className="rounded-lg border border-[#e5b85c] px-4 py-2.5 text-xs font-semibold text-[#f1cf7c] hover:bg-[#302818]">Edit record</button><button type="button" onClick={onClose} className="rounded-lg border border-[#3f4652] px-4 py-2.5 text-xs text-[#c3c7ce] hover:border-[#e5b85c]">Close</button></div></div></section></div>;
}

function ContentReviewCard({ item, sourceById, vocabulary, kanji, onOpen }: Readonly<{
  item: ReviewItem;
  sourceById: Map<string, ContentSource>;
  vocabulary: VocabularyItem[];
  kanji: KanjiItem[];
  ...
  return <article className="border-b border-white/10 text-left transition last:border-b-0 hover:bg-[#101b2b]/55">
    <button type="button" onClick={() => onOpen(item)} className="block w-full p-3 text-left outline-none focus-visible:ring-2 focus-visible:ring-[#e5b85c]/60">
      <div className="flex items-start justify-between gap-3">
        <span className="jp-serif text-xl text-[#f5f5f2]"><JapaneseText text={preview.japanese} vocabulary={vocabulary} kanji={kanji} readingEntries={preview.reading ? [[preview.japanese, preview.reading]] : []} always inspect={false} /></span>
...
function QuestionReview({ raw, fallback, vocabulary, kanji, onReview, onEdit }: Readonly<{ raw: string; fallback: PracticeQuestion[]; vocabulary: VocabularyItem[]; kanji: KanjiItem[]; onReview?: (id: string, status: ExerciseValidationStatus, metadata: ReviewMetadata) => void; onEdit: () => void }>) {
  ...
  const questionTypes = [...new Set(allQuestions.map((question) => question.questionType).filter(Boolean))].sort();
  const filteredQuestions = allQuestions.filter((question) => (questionTypeFilter === "all" || question.questionType === questionTypeFilter) && (!query.trim() || [question.id, question.itemId, question.category, question.questionType, question.prompt].join(" ").toLocaleLowerCase().includes(query.trim().toLocaleLowerCase())));
  ...
  useEffect(() => setPage(0), [query, questionTypeFilter]);
  useEffect(() => {
