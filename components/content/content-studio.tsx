...
function ContentReview({ module, onEdit, onReview }: Readonly<{ module: N5Module; onEdit: (kind?: EditableKind, id?: string) => void; onReview?: (id: string, status: ContentReviewStatus) => void }>) {
  ...
  return <div className="rounded-xl border border-white/10 bg-[#0d1522]/65 p-4"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm text-[#f5f5f2]">Content review queue</p><p className="mt-1 text-xs text-[#9297a1]">Pending source records appear first. Approve or reject only after enrichment and real lesson assignment.</p></div><div className="flex flex-wrap items-center gap-2"><span className="text-xs text-[#9297a1]">{statusFilter === "pending" ? `${pendingItems.length} pending` : `${allItems.length} total`}</span><button type="button" onClick={() => setStatusFilter("pending")} className={`rounded-lg px-3 py-2 text-xs ${statusFilter === "pending" ? "bg-[#e5b85c] text-[#0b0b0d]" : "border border-[#3f4652] text-[#9297a1]"}`}>Pending</button><button type="button" onClick={() => setStatusFilter("all")} className={`rounded-lg px-3 py-2 text-xs ${statusFilter === "all" ? "bg-[#e5b85c] text-[#0b0b0d]" : "border border-[#3f4652] text-[#9297a1]"}`}>All records</button><button type="button" onClick={() => onEdit()} className="rounded-lg border border-[#e5b85c] px-3 py-2 text-xs font-semibold text-[#f1cf7c] hover:bg-[#302818]">Edit records</button></div></div><div className="mb-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4"><input aria-label="Search review records" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search title, ID, topic…" className="rounded-lg border border-[#3f4652] bg-[#101b2b]/90 px-3 py-2.5 text-xs text-[#f5f5f2] outline-none placeholder:text-[#676c75] focus:border-[#e5b85c]" /><select aria-label="Filter JLPT level" value={levelFilter} onChange={(event) => setLevelFilter(event.target.value as typeof levelFilter)} className="rounded-lg border border-[#3f4652] bg-[#101b2b]/90 px-3 py-2.5 text-xs text-[#f5f5f2] outline-none focus:border-[#e5b85c]"><option value="all">All levels</option><option value="N5">N5 core path</option><option value="N4">N4 bridge</option></select><select aria-label="Filter curriculum band" value={bandFilter} onChange={(event) => setBandFilter(event.target.value as typeof bandFilter)} className="rounded-lg border border-[#3f4652] bg-[#101b2b]/90 px-3 py-2.5 text-xs text-[#f5f5f2] outline-none focus:border-[#e5b85c]"><option value="all">All curriculum bands</option><option value="core">Core</option><option value="extended">Extended</option><option value="bridge">Bridge</option></select><select aria-label="Filter source" value={sourceFilter} onChange={(event) => setSourceFilter(event.target.value)} className="min-w-0 rounded-lg border border-[#3f4652] bg-[#101b2b]/90 px-3 py-2.5 text-xs text-[#f5f5f2] outline-none focus:border-[#e5b85c]"><option value="all">All sources</option>{sourceOptions.map((source) => <option key={source} value={source}>{source}</option>)}</select></div>{!items.length ? <div className="rounded-lg border border-[#315d4b] bg-[#162b26]/70 p-4"><p className="text-sm text-[#c3c7ce]">No records match this review filter.</p><button type="button" onClick={() => { setStatusFilter("all"); setLevelFilter("all"); setBandFilter("all"); setSourceFilter("all"); setQuery(""); }} className="mt-2 text-xs font-semibold text-[#e5b85c] hover:text-[#f1cf7c]">Clear filters →</button></div> : <><div className="mb-3 flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-[#676c75]">Showing {page * pageSize + 1}–{Math.min((page + 1) * pageSize, filteredItems.length)} of {filteredItems.length} matching records.</p>{pageCount > 1 ? <div className="flex items-center gap-2"><button type="button" disabled={page === 0} onClick={() => setPage((current) => current - 1)} className="rounded-lg border border-[#3f4652] px-3 py-2 text-xs text-[#c3c7ce] enabled:hover:border-[#e5b85c] disabled:cursor-not-allowed disabled:opacity-40">Previous</button><span className="text-xs text-[#9297a1]">Page {page + 1} of {pageCount}</span><button type="button" disabled={page === pageCount - 1} onClick={() => setPage((current) => current + 1)} className="rounded-lg border border-[#3f4652] px-3 py-2 text-xs text-[#c3c7ce] enabled:hover:border-[#e5b85c] disabled:cursor-not-allowed disabled:opacity-40">Next</button></div> : null}</div><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{items.map((item) => { const preview = itemPreview(item); const reviewStatus = getContentReviewStatus(item); const itemBand = getCurriculumBand(item); const provenance = (item.sourceIds ?? []).map((id) => { const source = sourceById.get(id); return source ? `${source.name}${source.sha256 ? ` · ${source.sha256.slice(0, 12)}` : ""}` : id; }).join(" · "); return <div key={item.id} role="button" tabIndex={0} onClick={() => onEdit(item.category, item.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onEdit(item.category, item.id); } }} className="cursor-pointer rounded-lg border border-white/10 bg-[#101b2b]/75 p-3 text-left transition hover:border-[#e5b85c]/60 focus:outline-none focus:ring-2 focus:ring-[#e5b85c]/60"><div className="flex items-start justify-between gap-3"><span className="jp-serif text-xl text-[#f5f5f2]">{preview.reading ? <ruby>{preview.japanese}<rt className="text-[.45em] text-[#e5b85c]">{preview.reading}</rt></ruby> : preview.japanese}</span><div className="flex shrink-0 flex-col items-end gap-1 text-[10px] uppercase tracking-[.12em]"><span className="text-[#e5b85c]">{item.category}</span>{reviewStatus !== "approved" ? <span className={reviewStatus === "pending" ? "text-[#e5b85c]" : "text-[#ef675d]"}>{reviewStatus}</span> : null}</div></div><p className="mt-2 line-clamp-2 text-xs leading-5 text-[#9297a1]">{preview.detail}</p><div className="mt-2 flex flex-wrap gap-2 text-[10px] uppercase tracking-[.1em] text-[#676c75]"><span title={provenance || undefined}>{provenance || "source pending"}</span><span>{item.classification ? `${item.classification.band} · ${item.classification.confidence}` : `${itemBand} · inferred`}</span><span title={item.reason}>{item.reason}</span></div><p className="mt-1 truncate font-mono text-[10px] text-[#676c75]">{item.id}</p>{onReview && reviewStatus === "pending" ? <div className="mt-3 flex gap-2 border-t border-white/10 pt-3" onClick={(event) => event.stopPropagation()}><button type="button" onClick={() => onReview(item.id, "approved")} className="rounded-lg bg-[#6fb98f] px-3 py-2 text-xs font-semibold text-[#0b0b0d] hover:bg-[#8bcca6]">Approve</button><button type="button" onClick={() => onReview(item.id, "rejected")} className="rounded-lg border border-[#713b37] px-3 py-2 text-xs font-semibold text-[#ef675d] hover:border-[#ef675d]">Reject</button></div> : null}</div>; })}</div></>}</div>;
}
...
function QuestionReview({ raw, fallback, onReview, onEdit }: Readonly<{ raw: string; fallback: PracticeQuestion[]; onReview?: (id: string, status: ExerciseValidationStatus) => void; onEdit: () => void }>) {
  const questions = questionsForReview(raw, fallback).filter((question) => question.validationStatus === "generated" && question.generatedBy?.startsWith("openrouter:"));
  if (!questions.length) return <div className="rounded-xl border border-white/10 bg-[#0d1522]/65 p-5"><p className="text-sm text-[#f5f5f2]">No drafts waiting for review.</p><p className="mt-1 text-xs text-[#9297a1]">Generate a question above and it will appear here. Approved drafts move into the active question pipeline.</p></div>;

  return <div className="rounded-xl border border-white/10 bg-[#0d1522]/65 p-4"><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><div><p className="text-sm text-[#f5f5f2]">Pending question drafts</p><p className="mt-1 text-xs text-[#9297a1]">Approve good drafts to move them into the active pipeline, or reject them to keep them out.</p></div><div className="flex items-center gap-3"><span className="text-xs text-[#9297a1]">Showing {Math.min(questions.length, 20)} of {questions.length}</span><button type="button" onClick={onEdit} className="rounded-lg border border-[#e5b85c] px-3 py-2 text-xs font-semibold text-[#f1cf7c] hover:bg-[#302818]">Advanced JSON</button></div></div><div className="space-y-2">{questions.slice(0, 20).map((question, index) => { const choices = Array.isArray(question.options) ? question.options : []; const isOrdering = question.questionType === "sentence ordering"; const answer = question.answerMode === "text" ? question.acceptedAnswers?.[0] : isOrdering && question.tokens && question.correctOrder ? question.correctOrder.map((tokenIndex) => question.tokens?.[tokenIndex]).join(" ") : choices[question.correctIndex]; return <article key={question.id ?? index} className="rounded-lg border border-white/10 bg-[#101b2b]/75 p-4"><div className="flex flex-wrap items-center gap-2 text-[10px] uppercase tracking-[.12em] text-[#e5b85c]"><span>{question.category ?? "unknown"}</span><span className="text-[#676c75]">{question.questionType ?? "question"}</span><span className="text-[#9297a1]">draft</span></div><p className="mt-2 whitespace-pre-line text-sm leading-6 text-[#f5f5f2]">{question.prompt ?? "Untitled question"}</p>{choices.length && !isOrdering ? <div className="mt-3 grid gap-2 sm:grid-cols-2">{choices.map((choice, choiceIndex) => <span key={`${choice}-${choiceIndex}`} className={`rounded-md px-3 py-2 text-xs ${choiceIndex === question.correctIndex ? "bg-[#183225] text-[#8bcca6]" : "bg-[#17181d] text-[#9297a1]"}`}>{choice}</span>)}</div> : <p className="mt-3 text-xs text-[#8bcca6]">{isOrdering ? "Correct order: " : "Accepted: "}{answer ?? "not set"}</p>}<p className="mt-3 text-xs leading-5 text-[#9297a1]">{question.explanation ?? "No explanation yet."}</p>{onReview ? <div className="mt-4 flex flex-wrap gap-2 border-t border-white/10 pt-3"><button type="button" onClick={() => onReview(question.id, "validated")} className="rounded-lg bg-[#6fb98f] px-3 py-2 text-xs font-semibold text-[#0b0b0d]">Approve · add to pipeline</button><button type="button" onClick={() => onReview(question.id, "rejected")} className="rounded-lg border border-[#713b37] px-3 py-2 text-xs font-semibold text-[#ef675d]">Reject</button></div> : null}</article>; })}</div></div>;
}

export function ContentStudio({ seed: initialSeed, seedHealth, questionHealth, questions, knownItemIds, sources = contentSources }: Readonly<{ seed: N5Module; seedHealth: ContentValidationResult; questionHealth: ContentValidationResult; questions: PracticeQuestion[]; knownItemIds: string[]; sources?: ContentSource[] }>) {
  ...
  const updateQuestionStatus = (questionId: string, status: ExerciseValidationStatus) => {
    try {
      const current = JSON.parse(questionRaw) as unknown;
      ...
      const next = current.map((entry) => {
        ...
        return { ...record, validationStatus: status, ...(typeof review === "object" && review !== null ? { review: { ...(review as Record<string, unknown>), status: status === "validated" ? "approved" : "rejected", reviewedBy: "owner", reviewedAt } } : {}) };
      });
