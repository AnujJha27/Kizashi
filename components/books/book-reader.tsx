...
export function BookReader({ book }: Readonly<{ book: StudyBook }>) {
  ...
  useEffect(() => {
    ...
    setScreenshot(readBookScreenshot(book.id, 1));
  }, [book.id]);
  ...
  const chooseChapter = (id: string) => {
    ...
    setScreenshot(readBookScreenshot(book.id, chapter.page));
    setSaved(false);
  };
  ...
  const saveScreenshot = (file: File | undefined) => {
    ...
    if (!file.type.startsWith("image/") || file.size > 450_000) {
      setScreenshotMessage("Choose an image smaller than 450 KB.");
      return;
    }
