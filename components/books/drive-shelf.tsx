...
export function DriveShelf() {
  ...
  if (pdfUrl) return <section className="surface-panel overflow-hidden p-2 sm:p-3"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 px-2 pb-3"><div><p className="eyebrow">Google Drive · 私の本棚</p><h1 className="jp-serif mt-1 text-2xl text-[#f5f5f2]">{pdfTitle}</h1></div><button type="button" onClick={() => { URL.revokeObjectURL(pdfUrl); objectUrl.current = ""; setPdfUrl(""); }} className="rounded-lg border border-[#e5b85c] px-3 py-2 text-xs font-semibold text-[#f1cf7c] hover:bg-[#302818]">← Back to shelf</button></div><iframe title={pdfTitle} src={pdfUrl} className="mt-2 h-[72vh] min-h-[38rem] w-full rounded-lg bg-white" /></section>;
