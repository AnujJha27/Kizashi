...
export function ExamCountdown() {
  ...
  useEffect(() => {
    const refresh = () => {
      ...
      setPlan(getAdaptivePlan({ examDate: preferences.examDate, paused: preferences.paused, dueCount: getDueReviewIds().length, weakCount: Math.max(Object.keys(readMistakes()).length, getDueRepairs(readRepairRecords()).length), recentAccuracy, readiness: recentAccuracy !== null && recentAccuracy >= 0.85 ? "strong" : "untested" }));
    };
    refresh();
  ...
  if (plan.state === "paused") return <section className="surface-panel-raised p-6"><p className="eyebrow mb-2">Exam plan · 試験計画</p><p className="text-lg font-medium text-[#f5f5f2]">Plan paused.</p><p className="mt-2 text-sm text-[#9297a1]">Your reviews and progress are safe. Resume whenever you want recommendations tied to an exam date.</p><Link href="/profile" className="mt-5 inline-flex rounded-xl border border-[#5d3936] px-4 py-3 text-sm font-semibold text-[#f5f5f2] hover:border-[#e34a3f]">Resume plan <span className="ml-2" aria-hidden="true">→</span></Link></section>;
