...
Current implementation: valid non-rejected generated questions now enter normal Practice with a visible `Provisional · review while studying` badge and the existing post-answer question review controls. Approval/readiness metrics still exclude them, and rejected questions remain inactive.
...
Implemented in the learner flow: provisional encounter controls are mounted in Learn, Library, Practice feedback, Reading word inspection, Kanji writing, and Mistake repair. Item controls provide `Looks good`, reasoned flags, and an `Edit in Studio` handoff; answered questions provide `Correct`, `Ambiguous`, `Wrong answer`, and `Bad distractor`. Reading word inspection, both Kanji writing states, and factual Mistake repair entries now prefill the in-place correction field from the encountered meaning or explanation. Actions remain on the current card, persist in the existing synced `michi.content-flags` collection with origin and timestamp, and are visible in Content Studio's learner-review summary. Provider-hosted Marugoto activities remain available during encounter with source provenance visible; only explicitly reviewed item mappings are treated as Kizashi-linked evidence.

Still open from this report: final review of the expanded reservoir. The owner can now hide provisional review controls from Profile while provisional content stays learner-visible. Vocabulary/kanji meanings, grammar explanations, and answered question choices can be corrected in the encounter; those owner edits persist in the existing synced review record and are reapplied by the learner module, while complex metadata still opens Studio. Valid non-rejected generated questions now also enter Practice with a provisional badge and post-answer review controls; approval/readiness metrics remain separate. Flagged factual items remain viewable but move behind unflagged/context items at lesson entry, while flagged questions are excluded from active Practice queues. Studio now reports available, personally reviewed, provisional, flagged, and rejected counts from the live package; the N5/N4 lexical and grammar union panels also show canonical missing counts, live availability, review state, Kanji useful-word/stroke coverage, and active text-grammar counts. Rejected records remain excluded by the learner release gate and provisional records remain learner-visible and visibly labeled.

The Immersion source registry now keeps Moshi Moshi Yusuke, Natural
Japanese/NIJ, and Organic Japanese with Cure Dolly as one provider-hosted
YouTube family with distinct real-life, comprehensible-input, and
guided-understanding roles. Provider levels remain separate from Kizashi's
recommended N5/N4 level; catalogs are bounded and cached, playback stays
