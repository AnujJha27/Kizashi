...
- [~] The checked-in `data/kanjivg-strokes.json` currently contains source/license metadata and no records because the sandbox rejected the required public KanjiVG download. Practice Focus routing reuses the same bounded trainer, local writing-familiarity state is backed up/snapshot-ready with account-sync events, and Content Studio now reports N5/N4 local stroke-path coverage. Populated N5/N4 paths, deployed sync verification, and real mobile drawing verification remain open; the trainer honestly shows a source fallback until data exists.
