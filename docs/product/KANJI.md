...
- [~] Kanji Entry pages now mount a native pointer-event Watch/Trace/Write trainer with Japanese practice-grid guides, step controls, reset/undo/clear actions, reduced-motion-safe current-stroke emphasis, forgiving stroke-order feedback, and writing kept separate from JLPT readiness. The shared source viewer provides a lazy Jisho iframe attempt plus direct Jisho/Tanoshii fallbacks, and the private frame-unlocker allowlists Jisho alongside existing provider frames.
- [~] `lib/kanji-writing-core.js` centralizes KanjiVG code-point URLs, Jisho/Tanoshii reference URLs, stroke-record validation, and basic order feedback. `scripts/import_kanjivg.py` reads the canonical N5/N4 inventory and deterministically imports ordered KanjiVG paths into `data/kanjivg-strokes.json`.
- [~] The checked-in `data/kanjivg-strokes.json` currently contains source/license metadata and no records because the sandbox rejected the required public KanjiVG download. Practice Focus routing reuses the same bounded trainer, and local writing-familiarity state is backed up/snapshot-ready with account-sync events; populated N5/N4 paths, deployed sync verification, Content Studio counts, and real mobile drawing verification remain open. The trainer honestly shows a source fallback until data exists.
