...
- [~] The checked-in `data/kanjivg-strokes.json` currently contains source/license metadata and no records because the sandbox rejected the required public KanjiVG download. Practice Focus routing now reuses the same bounded trainer, while populated N5/N4 paths, writing-state persistence, Content Studio counts, and real mobile drawing verification remain open; the trainer honestly shows a source fallback until data exists.
