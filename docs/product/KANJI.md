...
- [~] `lib/kanji-writing-core.js` centralizes pinned KanjiVG `r20260714` code-point URLs, Jisho/Tanoshii reference URLs, Unicode-matching stroke-record validation, and basic order feedback. `scripts/import_kanjivg.py` reads the canonical N5/N4 inventory and deterministically imports ordered KanjiVG paths into `data/kanjivg-strokes.json`.
