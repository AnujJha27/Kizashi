...
- [~] Kanji Entry pages now mount a native pointer-event Watch/Trace/Write trainer with Japanese practice-grid guides, visible per-stroke number markers, source-derived component groups, a clean Show components toggle that color-separates imported component stroke groups, optional deterministic “What comes next?” and positional component recognition checks using the same imported metadata, an optional useful-word session that walks fully covered multi-kanji words character by character, bounded Practice Focus queues prioritized by current lesson, due reviews, frequent mistakes, unwritten kanji, and an explicit initial target, and an occasional Today writing detour when current-lesson kanji are useful to write. Step controls, reset/undo/clear actions, cancellation-safe touch/stylus drawing, reduced-motion-safe current-stroke emphasis, forgiving stroke-order feedback, and writing kept separate from JLPT readiness remain. Kanji and orthography misses now suggest targeted Writing repair with the missed item selected in Practice Focus. The shared source viewer provides a lazy Jisho iframe attempt plus direct Jisho/Tanoshii fallbacks; when Jisho blocks framing, the original page remains the fallback.
- [~] `lib/kanji-writing-core.js` centralizes pinned KanjiVG `r20260714` code-point URLs, the KanjiVG repository source link, Jisho/Tanoshii reference URLs, Unicode-matching stroke-record validation, component-group metadata, deterministic stroke/component quiz options, bounded writing-session prioritization, and basic order feedback. `scripts/import_kanjivg.py` reads the canonical N5/N4 inventory and deterministically imports ordered KanjiVG paths, top-level component groups with their stroke ranges, plus source/license attribution metadata into `data/kanjivg-strokes.json`; Content Studio exposes component-group coverage and the exact repository link.
- [x] Kanji records now carry optional `confusableKanji` groups for reviewed current-curriculum pairs (`人`/`入` and `右`/`左`). Content Studio exposes the provisional values as an owner-editable list, and the Supabase schema/export/runtime path preserves them without changing the native KanjiVG stroke data.
- [~] The checked-in `data/kanjivg-strokes.json` now contains 254 ordered KanjiVG records for the canonical N5/N4 union (125 N5 claims, 166 N4 claims, with 37 characters shared across levels), with source/license metadata, 216 records carrying verified top-level component groups, and zero missing or unordered paths; the `kanjivg` source is registered in the app and deployable seed manifests. Practice Focus routing reuses the same bounded trainer, local writing-familiarity state is backed up/snapshot-ready with account-sync events, and Content Studio reports the pinned release/license, component-group coverage, and N5/N4 local stroke-path, writing-enabled, missing-path, and stroke-count-mismatch coverage. Deployed sync verification and real mobile drawing verification remain open; the trainer still shows a source fallback for characters outside this canonical union.
...
```text
Jisho embedding is attempted directly in the shared source viewer.

If the provider blocks framing, devices fall back to opening Jisho externally.
```
...
Kanji writing now exposes the same provisional encounter review controls as Library and Learn, with the encountered meaning prefilled for correction. The local KanjiVG trainer now covers the 254-character canonical N5/N4 union and includes optional next-stroke, component, and fully covered useful-word writing sessions; Practice Focus now prioritizes current-lesson, due, mistake, and unwritten signals within its 2/4/8-card bounds, and Today can offer a conditional two-minute writing detour, while Jisho iframe/fallback remains intact. Kanji misses route directly to the writing repair target and each practice card resets its trainer state. Deployed sync verification and real mobile drawing verification remain open.
