...
| [Irodori](https://www.irodori.jpf.go.jp/en/) | Practical Can-do reinforcement on overlapping Learn lessons, with Starter Lessons 1, 3–6 and Elementary 1 Lessons 1 and 6 currently surfaced in Immersion's `実際に使う` lane. | The existing three Irodori item ingestors remain the staging path. The resource pipeline also preserves activity type, course level, Can-do, categorized target IDs, provenance, and provider-hosted lesson/audio pages; no media mirror is created. | The Kizashi lesson remains functional and the official source link remains available if Irodori media or framing fails. |
