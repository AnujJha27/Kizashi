...
- [x] Provider video cards now support self-reported `Clear` / `Shaky` / `Missed` comprehension reviews, stored locally by video ID with a timestamp and kept separate from source approval and JLPT readiness. Content Studio reports cached health for the bounded YouTube feeds and preserves the official-channel fallback when a feed is unavailable.
- [~] Reviewed grammar-to-video mappings remain open until reliable provider metadata or a reviewed mapping table exists.
