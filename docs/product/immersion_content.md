...
- [x] Shared external-source progress now migrates legacy opened flags and records monotonic opened/started/completed states from viewer entry, iframe load or native media play/end, plus explicit self-completion; the source viewer moves keyboard focus into the retained iframe dialog, contains Tab navigation, and restores the opener on close; YouTube playback remains completion-unknown.
