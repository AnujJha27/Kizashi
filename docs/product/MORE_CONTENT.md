...
- [~] Shared furigana scaffolding now honors `Always`, `Unknown`, `Tap`, and `Hide` in learner text renderers, including Immersion passage previews, Practice text, and the Content Studio review modal; Practice builds a compact reading-only index from the full learner module so bounded question loading does not remove readings, while complete fading behavior and a few raw/reference fields remain.
