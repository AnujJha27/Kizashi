...
- [x] Provider video encounters now accept a local `Clear` / `Shaky` / `Missed` comprehension signal keyed to the provider video, while Content Studio exposes cached health for the bounded YouTube feeds; neither signal is treated as source approval, native review, or JLPT readiness.
