...
- [~] Practical reading visuals now pair four generated no-text WebP scene assets with Kizashi-owned format-aware HTML layouts (notice, menu/sale, timetable/schedule, and table-like practical formats) driven by explicit metadata in the 27 information-retrieval assets; `lib/learning-visual-assets.ts` keeps their generated-raster provenance centralized, and Japanese remains accessible HTML rather than generated pixels. Lazy loading and intrinsic dimensions keep the asset budget practical. Broader format variety and native review remain open.
