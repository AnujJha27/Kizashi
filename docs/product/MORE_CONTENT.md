...
- [x] Content Studio now runs a deterministic authored-bank quality audit: reading is 115/115 unique normalized templates with 0 near-duplicate clusters and 0 structural answer-choice failures; listening is 160/160 unique normalized templates with 0 near-duplicate clusters, with question-family and source-type counts visible. The generator now varies listening prompts/scripts and removes the prior exact and cross-level duplicates; all 160 current scenarios remain browser-TTS, so naturalness still needs human-speaker review.
