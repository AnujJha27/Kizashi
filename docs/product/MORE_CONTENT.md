...
- [x] 2026-09-05 level-coverage checkpoint: the same family floor is visible by level; N5 flags sequence, reference, simple inference, condition detail, and task-based response, while N4 flags main idea, sequence, reason, condition detail, and task-based response.
...
- [~] The learner curriculum contains the original reading target: 115 assets (N5 60, N4 55) with 143 questions across short-detail, mid-length, information-retrieval, main-idea, sequence, reason, reference, simple-inference, condition-detail, task-based-response, and appropriate-action families; the 27 information-retrieval assets carry explicit visual formats and render accessible HTML reading aids in the shared ReadingPanel. The listening bank contains 160 N5/N4 scenarios across task-based, key-point, verbal-expression, and quick-response families. Immersion now exposes the full banks progressively instead of showing only the first eight; broader reading coverage and native review remain open.
...
- [~] Priority 4 — the deterministic original-reading generator and the complete initial N5/N4 bank are implemented at the required 60 N5 + 55 N4 distribution, with 143 questions across 11 families, including repeated reason, reference, simple inference, and appropriate action coverage; the quality audit finds 115 unique normalized passage templates and no near-duplicate clusters, while broader family balance, human review, and non-template expansion remain follow-up work.
