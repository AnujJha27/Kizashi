...
- [~] The grammar lesson-contract audit now measures aliases, dedicated mini-contexts, linked reading/listening appearances, and context-bearing assessment coverage in the same completeness pipeline; Content Studio exposes the alias/context fields for editing and review and provides a direct queue for the 101 records missing one or both fields, while validation warns when they are missing. The current package is 15/116 contract-ready, with 111/116 items carrying two linked assessment contexts and the remaining alias/context fields still under review.
