...
- [~] Kanji entries and Practice Focus now include a native Watch/Trace/Write trainer shell, targeted Writing repair from kanji/orthography misses, local writing-familiarity state with sync events, and lazy Jisho/Tanoshii references through the existing source viewer; KanjiVG path import and deployed sync verification remain pending until the local stroke dataset is populated.
