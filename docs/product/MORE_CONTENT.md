...
- [~] The learner curriculum contains the original reading target: 115 assets (N5 60, N4 55) with 125 questions across short-detail, mid-length, information-retrieval, main-idea, sequence, condition-detail, and task-based-response families; the listening bank contains 160 N5/N4 scenarios across task-based, key-point, verbal-expression, and quick-response families. Immersion now exposes the full banks progressively instead of showing only the first eight; broader reading coverage and native review remain open.
