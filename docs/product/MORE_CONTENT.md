...
- [x] Provider video encounters now accept a local-first `Clear` / `Shaky` / `Missed` comprehension signal keyed to the provider video and include it in opt-in account sync when enabled, while Content Studio exposes cached health for the bounded YouTube feeds; neither signal is treated as source approval, native review, or JLPT readiness.
