...
- [~] Mobile, external-frame, performance, and source-failure behavior have been improved in affected flows; Practice mode/length/topic changes update the controls immediately and defer the keyed question-panel swap behind a visible transition state, saved-question validation is idle-scheduled, Studio's route has a loading fallback and its full review-package/local-draft work is idle/user initiated, its repeated 8k-item review/coverage scans are memoized, Japanese text uses cached text-scoped reading entries even with the full learner module released, AI target selection no longer renders the full package as DOM options, the service worker caches the released learner package after first load, document vertical scrolling uses browser-compatible overflow handling, and queue identity is isolated, but offline writes and a full end-to-end audit remain.
