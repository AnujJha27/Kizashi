...
- [~] Shared furigana scaffolding now honors `Always`, `Unknown`, `Tap`, and `Hide` in learner text renderers, including Immersion list/player titles and passages, Aozora/Tadoku titles, Practice text, lesson prompts/answers, selected reading words, learner-assistant output, Library entries, Knowledge Map, and the Content Studio review modal; Practice builds a compact reading-only index from the full learner module so bounded question loading does not remove readings, while complete fading behavior and a few raw/reference fields remain. Learner routes no longer synchronously parse the large local Studio draft before loading the released package.
...
- [~] Mobile, external-frame, performance, and source-failure behavior have been improved in affected flows; Practice mode/length/topic changes update the controls immediately and defer the keyed question-panel swap behind a visible transition state, route navigation shows immediate feedback and avoids speculative heavy-route prefetching, saved-question validation is idle-scheduled, Practice remounts reuse the cached learner module and merge small local question drafts into the full generated bank, Studio's route no longer generates the full question bank during server render, its full review-package/local-draft work is idle/user initiated, generated question coverage is explicitly on-demand, its repeated 8k-item review scans are memoized and page-local ranking avoids blocking the first render, Japanese text uses cached text-scoped reading entries even with the full learner module released, AI target selection no longer renders the full package as DOM options, the service worker caches the released learner package after first load, document vertical scrolling uses browser-compatible overflow handling, queue identity is isolated, and private-book loading accepts both deployed Storage path conventions with a bounded client timeout; offline writes and a full end-to-end audit remain.
