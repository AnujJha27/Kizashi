...
- [x] Aozora has a rights-filtered metadata shelf and dynamic reader (1 currently curated reusable work); Tadoku has a provider-hosted graded-reading shelf with local opened progress (4 currently curated books).
...
- [~] Shared furigana scaffolding now honors `Always`, `Unknown`, `Tap`, and `Hide` in learner text renderers, including Immersion list/player titles and passages, Aozora/Tadoku titles, Practice text, lesson prompts/answers, selected reading words, learner-assistant output, Library cards and entry headers, Mistake Center entries, Knowledge Map, and the Content Studio review modal; LessonPlayer now fades experienced grammar formation and listening transcripts to explicit support buttons, while Practice builds a compact reading-only index from the full learner module so bounded question loading does not remove readings. Production models are already explicit reveals; a few raw/reference fields remain. Learner routes no longer synchronously parse the large local Studio draft before loading the released package.
