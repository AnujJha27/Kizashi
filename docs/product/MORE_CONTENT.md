...
- [~] Vocabulary context audit — `lib/content-completeness-core.js` and Content Studio now report examples, collocations, related words, persisted audio, contextual/paraphrase assessment links, and N4 usage coverage; validation also warns on the 96 current `commonness >= 5` records with fewer than two examples. Across 169 current records, 72 have two examples, all have collocations/related words, 16 have usage assessments, and none yet meet the complete persisted contract because generated fallback questions are not counted.
