...
- [~] Priority 5 — the deterministic original-listening generator and 160-scenario N5/N4 bank (20 task-based, 20 key-point, 15 verbal-expression, and 25 quick-response items per level) are implemented; the generator now accepts level/family/situation/vocabulary/forbidden-context filters and records the remaining grammar, speaker, duration, and question constraints for review. Richer dialogue variation and review remain. Immersion rotates the first listening slice per browser session so the bank is not presented as the same fixed eight items.
