...
- [~] The grammar lesson-contract audit now measures aliases, dedicated mini-contexts, linked reading/listening appearances, and context-bearing assessment coverage in the same completeness pipeline; Content Studio exposes the alias/context fields for editing and review and provides a direct queue for the 74 records missing one or both fields, while validation warns when they are missing. The current package is 42/116 contract-ready, with 111/116 items carrying two linked assessment contexts; explicit fields for the 27 foundational N5 records live in `data/grammar-contract-fields.json`, and the remaining alias/context fields stay under review.
