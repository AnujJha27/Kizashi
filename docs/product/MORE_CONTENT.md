...
- [x] 2026-09-06 provider-enrichment patch — Learn’s Marugoto Plus follow-up now shows its seven-entry provider-hosted Can-do activity catalog with source level, JLPT relevance, modality, local opened tracking, and an iframe; the direct provider source remains the fallback for `a1.marugotoweb.jp`’s frame-blocking headers.
...
- [x] 2026-09-05 level-coverage checkpoint: the same family floor is visible by level; the current N5 and N4 reports have no under-minimum supplemental families.
...
- [x] 2026-09-05 quality checkpoint: the shared authored-bank audit now identifies distractor choices whose normalized text is mentioned in reading passages and Content Studio exposes a bounded review list plus N5/N4 source-length signals. The current 115-passage bank has 34 affected question sets and 56 source-mentioned choices, with 39/51 average source characters by level; these structural signals do not replace plausibility, length calibration, or native review.

- [x] 2026-09-05 target-level checkpoint: Content Studio now resolves reading vocabulary/grammar/kanji links against the active merged package and reports their N5/N4/other/unresolved distribution. The current merged package has 0 unresolved reading links; N5 passages resolve 342 N5 + 2 N4 vocabulary, 215 N5 grammar, and 224 N5 kanji links, while N4 passages resolve 255 N5 + 1 N4 vocabulary, 116 N5 + 116 N4 grammar, and 189 N5 kanji links. Cross-level prerequisite links remain visible for review.
...
- [~] The learner curriculum contains the original reading target: 115 assets (N5 60, N4 55) with 166 questions across short-detail, mid-length, information-retrieval, main-idea, sequence, reason, reference, simple-inference, condition-detail, task-based-response, and appropriate-action families; every supplemental family now reaches the three-question floor both globally and within each level, while 34 question sets with 56 source-mentioned distractor choices remain a review signal. The 27 information-retrieval assets carry explicit visual formats and render accessible HTML reading aids in the shared ReadingPanel, which also exposes A−/A＋ passage sizing from 16–28px. The listening bank contains 160 N5/N4 scenarios across task-based, key-point, verbal-expression, and quick-response families. Immersion now exposes the full banks progressively instead of showing only the first eight; distractor plausibility, level calibration, and native review remain open.
...
- [x] Japanese with Shun, Moshi Moshi Yusuke, Natural Japanese/NIJ, Cure Dolly, and Nihongo con Teppei are learner-facing provider-hosted entries in the shared registry and Immersion shelf; the three YouTube additions keep distinct real-life, comprehensible-input, and guided-understanding roles, use one bounded catalog/player path, retain provider level separately from Kizashi recommendation level, and store no media.
- [x] Provider video encounters now accept a local-first `Clear` / `Shaky` / `Missed` comprehension signal keyed to the provider video, reuse the existing study-later collection for `Save · あとで`, and include both collections in opt-in account sync when enabled, while Content Studio exposes cached health for the bounded YouTube feeds; neither signal is treated as source approval, native review, or JLPT readiness.
- [x] Shared external-source progress now migrates legacy opened flags and records monotonic opened/started/self-completed states from provider viewers without pretending YouTube playback completion is known; provider video saves continue through the existing study-later collection.
- [x] Profile stores up to three optional interest topics; Immersion uses them as a bounded tie-breaker for original reading/listening and provider catalog titles while preserving lesson level, weakness, coverage, and naturalness signals, with current Journey-area tags adding a contextual source boost.
- [x] Content Studio now separates package quantity, Journey lesson assignment, required learner fields, review workflow status, unique reading/listening contexts, reading answer-choice integrity, collocation structure, and grammar-depth readiness in a live JLPT completeness dashboard; all 116 learner-ready N5/N4 grammar items now meet the 4-example, 2-mistake, and 2-reviewed-practice-link gate. Visual listening-question counts are reported separately rather than implied by total question volume.
- [x] The grammar lesson-contract audit now measures aliases, dedicated mini-contexts, linked reading/listening appearances, and context-bearing assessment coverage in the same completeness pipeline; Content Studio exposes the alias/context fields for editing and review and provides a direct queue for records missing one or both fields, while validation warns when they are missing. The current package has all 116 records carrying aliases, mini-contexts, and two linked assessment contexts, so all 116 are contract-ready; the explicit authored fields live in `data/grammar-contract-fields.json`.
- [x] Original Kizashi grammar prose now has a consistency audit in `lib/content-completeness-core.js`; the 116-record authored package reports 0 duplicate-example items, 0 duplicate rows, 0 same-example translation collisions, and 0 empty examples after the repeated N5 prose was rewritten. Shared examples across concepts remain review-visible for native-language judgment.
- [~] Vocabulary context audit — `lib/content-completeness-core.js` and Content Studio now report examples, collocations, related words, persisted audio, approved contextual/paraphrase assessment links, pending draft counts, and N4 usage coverage; validation warns when high-frequency records fall below two examples, and the current package has none in that warning queue. Across 169 current records, all 169 have two examples (153/153 N5 and 16/16 N4), all have collocations/related words, 16 have usage assessments, and 338 deterministic contextual/paraphrase drafts are staged for review through `scripts/generate_vocabulary_assessment_drafts.mjs`; the pending question queue filters drafts by question family, valid non-rejected generated questions now enter Practice visibly marked provisional with review controls, and none yet meet the complete persisted contract because drafts are not counted as approved until review.
- [~] Collocation quality audit — the current 169-record package has 169 populated collocation lists with at least two entries each, 0 duplicate rows, and 0 headword-only entries. This is a structural check; source-backed frequency and native naturalness review remains open.
- [~] Content Studio now runs a deterministic authored-bank quality audit: reading is 115/115 unique normalized passage templates with 166 questions across 11 question families, 31 topic counts, 2 linked grammar-pattern signatures as a structural proxy, N5 difficulty metadata averaging 2 and N4 averaging 4, 166/166 unique correct choices, 0 near-duplicate clusters, 0 structural answer-choice failures, and 113 exact passage-answer echoes surfaced with a bounded record/question-family/answer review list; its report and rendered calibration card also expose linked vocabulary/grammar/kanji load ranges and blank/duplicate/under-three-distractor signals. Listening is 160/160 unique normalized templates across 20 context tags with N5 difficulty metadata averaging 2 and N4 averaging 4, 0 near-duplicate clusters and 160/160 unique correct choices, with question-family, context, source-type, turn-profile, speaker-pattern, and transcript-answer-echo counts plus a bounded record/question-family/answer review list visible; the current bank is 80 three-turn `A-B-A` and 80 four-turn `A-B-A-B`, with 94/160 answer echoes flagged for review. The reading generator now includes repeated reason, reference, simple-inference, condition-detail, task-based-response, and appropriate-action prompts, while the listening generator varies prompts/scripts and removes prior exact and cross-level duplicates; all 160 current scenarios remain browser-TTS, so naturalness still needs human-speaker review.
- [~] Practical reading visuals now pair eight generated no-text WebP scene assets with Kizashi-owned format-aware HTML layouts (notice, menu/sale, timetable/schedule, numbered directions, and table-like practical formats) driven by explicit metadata in the 27 information-retrieval assets; `lib/learning-visual-assets.ts` keeps their generated-raster provenance centralized, Japanese remains accessible HTML rather than generated pixels, shared furigana uses a readable scalable size with a 1rem floor in reading, lesson, and anatomy cards, and ReadingPanel offers A−/A＋ passage sizing from 16–28px. The Journey world uses eight area-specific generated WebP scenes for the hero/shell, eight distinct generated WebP scenes for Today, a separate tall `/world/journey-map.webp` map scene, a separate wide `/world/lesson-wide.webp` lesson-opening scene, and a separate tall `/world/lesson-tracker-portrait.webp` tracker scene; the Journey map’s lower landscape now ends in a dark terminal gradient so route labels remain readable. Lazy loading and intrinsic dimensions keep the asset budget practical. Broader format variety and native review remain open.
...
- [x] Original verbal-expression listening items now carry scene metadata and render four generated, reusable raster situation illustrations in Lesson and Practice; the Immersion discovery feed reuses those scenes as lazy context thumbnails; no third-party image or media is copied.
...
- [x] Content Studio leads source coverage with the registered package-provenance total, then separates selective provider links from package-source requirements; its live provenance-gap dashboard still shows missing records and unknown source IDs by content category, only labels the package fully connected when both are absent, merges the active loaded package manifest with the server registry so staged-package IDs are not falsely flagged, repairs stale local-draft provenance when a matching bundled record can supply a registered source, and makes the explicit full-package load replace the stale coverage input.
- [x] Content Studio now reports computed modality coverage for examples, attached audio, human-backed versus synthetic/fallback audio, reading/listening checks, and conjugation candidates alongside source coverage.
...
- [x] Immersion's Real life lane derives 80 speaking situations, 60 constrained writing prompts, 119 pragmatic contexts, 296 natural collocations, and 40 balanced N5/N4 casual-vs-polite recognition activities (20 per level; 10 casual / 10 polite) from released listening, reading, and vocabulary records. Model audio reuses the existing fallback, revealed attempts can be self-rated through the shared review scheduler with immediate due-order refresh, and open writing/register recognition are explicitly not auto-graded or counted as JLPT readiness.
- [x] Reading word lookup now exposes the existing Study later action inline, keeping unknown-word capture inside the activity instead of forcing a Library detour.
...
- [~] Kanji entries and Practice Focus now include a native Watch/Trace/Write trainer shell with visible per-stroke number markers, a Show components toggle for verified KanjiVG component groups, optional next-stroke and positional component recognition checks, an optional useful-word session for fully covered multi-kanji words, bounded queue prioritization from current-lesson, due, mistake, unwritten, and explicit-target signals, and an occasional conditional Today writing detour; cancellation-safe touch/stylus drawing, targeted Writing repair from kanji/orthography misses, local writing-familiarity state with sync events, and lazy Jisho/Tanoshii references continue through the existing source viewer; the local KanjiVG `r20260714` manifest now contains 254 ordered records for the canonical N5/N4 union, with Unicode-matching runtime validation, and KanjiVG is registered in both the app and seed source manifests. Content Studio also reports component-group coverage and owner-editable provisional confusable pairs alongside the pinned release/license, attribution, direct repository link, writing-enabled, missing-path, and stroke-count-mismatch totals by level; deployed sync verification and real mobile drawing verification remain open.
- [x] Practice empty queues now use a useful learner state with Ear warm-up and current-lesson links instead of a dead-end “no questions” message; exam queues retain a distinct unavailable state.
- [x] The sixteen authored N4 vocabulary bridge items now generate explicit natural-usage questions with complete Japanese sentence options and authored distractors; the larger staged N4 vocabulary reservoir remains review-only for this dimension.
- [x] The N4 learner path now includes an eight-item transitivity cluster for 開く／開ける, 閉まる／閉める, 始まる／始める, and 止まる／止める, with paired examples, particle cues, and explicit 自動詞／他動詞 notes; these are authored vocabulary records rather than a claim that the full staged verb inventory is reviewed.
...
- [x] Immersion Continue state now carries the selected reading/listening ID through the existing `/immersion` route and reopens only validated local activities.
- [x] Interrupted Today sessions now show the saved next stage and `N of M complete` context in the Journey path before the learner resumes; matching legacy `michi.current-lesson` records are read for the lesson they belong to so completed pre-migration lessons still unlock the next stop.
...
- [~] Mobile, external-frame, performance, and source-failure behavior have been improved in affected flows; Practice mode/length/topic changes update the controls immediately, unmount the old queue, and keep a visible transition until the new client queue reports ready, route navigation shows immediate feedback and avoids speculative heavy-route prefetching, saved-question validation is idle-scheduled, Practice remounts reuse the cached learner module and merge small local question drafts into the full generated bank, valid non-rejected generated/AI questions enter normal queues with a provisional badge while approval metrics stay separate, adaptive queue inputs are memoized so ranking does not re-trigger itself on each render, the diagnostic bank is client-side instead of server-generated during navigation, Studio's route no longer generates the full question bank during server render, its full review-package/local-draft work is idle/user initiated and no longer re-runs on question-bank state changes, generated question coverage is explicitly on-demand, its repeated 8k-item review scans are page-local, reading diagnostics are explicit and chunked, Practice furigana construction is bounded to the visible queue while one-record Studio inspection keeps the full learner index, JapaneseText shares cached preference/review-record reads across rendered instances, AI target selection no longer renders the full package as DOM options, generated learning images hide on load failure so surrounding gradients/captions/HTML remain usable, world raster scenes use lazy async decoding and stay under a 350 KB automated asset budget, the service worker caches the released learner package after first load, html owns vertical scrolling without a competing body scroll container, queue identity is isolated, lesson/Journey grids use `minmax(0, …)` and `min-w-0` to prevent narrow-card clipping, private-book loading accepts both deployed Storage path conventions with a bounded client timeout, failed account-sync writes now persist in a local outbox for reconnect retry, and the shell/source-viewer/review dialogs now provide keyboard focus rings, focus entry/containment/restoration, and a skip-to-content target; external frames now put their direct-source fallback guidance above a bounded iframe, Marugoto remains `frame-or-link`, and provider frame failures fall back to the original source; a full end-to-end audit remains.

### Priority tracking

- [x] The offline shell now pre-caches the current Journey/Today/lesson WebP scenes and local listening illustrations with the service-worker cache revision, so local visual study does not depend on the retired PNG atmosphere assets.
- [x] The offline shell also pre-caches the eight local reading scene assets used by `ReadingPanel`; generated imagery remains optional context and the accessible passage remains usable on failure.

- [~] Priority 1 — Irodori grammar now has 98 canonical concept mappings and an audit-backed learner reference path; source-only rows with no canonical meaning render as labeled Japanese pattern references in Learn and Library instead of blank grammar answers. Source-derived meanings, formations, and example translations remain pending review.
- [~] Grammar coverage union — `scripts/build_grammar_coverage_registry.mjs`, `data/grammar-coverage-union.json`, and Content Studio now compare 116 canonical concepts with cached OpenJLPT, Irodori, the 153 reviewed Irodori mapping references, 21 curated OpenJLPT alias references, Tae Kim, and Wikibooks evidence. The current baseline is N5 224 raw / 46 canonical / 23 complete / 18 partial / 5 missing / 80 unresolved and N4 188 raw / 70 canonical / 14 complete / 51 partial / 5 missing / 73 unresolved; sixty-five dedicated N4 expansion concepts and five N4 life-bridge concepts are placed in lessons, with direction, benefit-perspective, inference/appearance, conditional, potential-ability, passive/potential, causative, change/habit/decision, modality/inference, quotation/thought, nominalization/capability, contrast/conjunction, time/aspect, and ease/difficulty contrast clusters linked in the learner curriculum. Content Studio now exposes each canonical concept's exact mapped source-pattern labels, evidence count/source IDs, and level disagreements. Mapped records are removed from the unresolved queue, while remaining rows stay review-only.
...
- [x] 2026-09-05 lexical review checkpoint — Content Studio adds bounded N5/N4 vocabulary and kanji queues for partial/missing records with coverage status, teaching status, and source IDs; the full union remains available as generated evidence while the queue avoids rendering thousands of records at once.
- [~] Grammar assessment depth — Content Studio now reports 92 approved persisted authored N5 grammar questions across 92 normalized contexts and 55 approved persisted authored N4 grammar questions across 55 normalized contexts (147 form-selection, 5 ordering, 0 text-grammar, and 6 contrast-cluster questions), and 125 text-grammar drafts pending review (50 N5 / 75 N4), each with persisted context IDs/text shown in the review cards; `validatePracticeQuestions` now requires that context contract for text-grammar questions, while valid non-rejected generated questions can enter learner Practice as provisional and remain excluded from approved/readiness coverage. Explicit context-set metadata can collapse variants of one passage across both aggregate and per-family metrics. The 50 N5 / 75 N4 independent text-grammar target is staged for review, not yet learner-ready.
...
- [~] Priority 4 — the deterministic original-reading generator and the complete initial N5/N4 bank are implemented at the required 60 N5 + 55 N4 distribution, with 166 questions across 11 families and at least three questions per supplemental family at both levels; the quality audit finds 115 unique normalized passage templates and no near-duplicate clusters, while source-mentioned distractors still require question-design review alongside plausibility, level calibration, human review, and non-template expansion.
- [~] Priority 5 — the deterministic original-listening generator and 160-scenario N5/N4 bank (20 task-based, 20 key-point, 15 verbal-expression, and 25 quick-response items per level) are implemented; the generator accepts level/family/situation/vocabulary/forbidden-context filters, varies three dialogue/question shapes, gives N4 a distinct four-turn dependency path, and the quality audit now reports 160 unique templates, 20 contexts, and 0 near-duplicate clusters. All current items use browser TTS, so recorded-speaker naturalness review and richer scenario variation remain. Immersion rotates the first listening slice per browser session so the bank is not presented as the same fixed eight items.
...
- [x] Moshi Moshi Yusuke, Natural Japanese/NIJ, and Organic Japanese with Cure Dolly use the shared `/api/immersion/youtube/[provider]` route and YouTube feed parser; provider roles, aliases, provider/recommended levels, Journey-context tags, conservative Yusuke content-type labels, and one reviewed Cure Dolly verb-forms mapping stay in the registry, while the `しくみ` lane keeps Cure Dolly separate from native listening.
...
The review addendum is now part of the tracked product requirements. Its first learner-flow slice is implemented through shared provisional review controls, question verdicts, local persistence, account sync, and Studio visibility across the requested surfaces. Vocabulary/kanji meanings, grammar explanations, and answered question choices can be corrected in context and are reapplied from the synced review record; complex metadata still routes to Studio. The authenticated learner release now exposes the full non-rejected staged reservoir (7,328 vocabulary and 630 kanji records) while the bundled offline baseline remains the smaller authored set; final reservoir review, native quality review, and coverage-gap closure remain open.

The Kanji writing slice now also lands in Library as `漢字帳`: existing N5/N4 and progress filters are joined by current-journey, writing-due, recently-learned, and not-written views, while Kanji cards expose stroke count, one primary reading, meaning, writing state, and a Write link. No separate navigation or duplicate progress model was added.
