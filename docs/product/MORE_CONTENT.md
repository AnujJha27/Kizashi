...
- [~] Shared furigana scaffolding now honors `Always`, `Unknown`, `Tap`, and `Hide` in learner text renderers, including Immersion passage previews and Practice text; Practice builds a compact reading-only index from the full learner module so bounded question loading does not remove readings, while remaining raw/reference renderers still need the same treatment.
