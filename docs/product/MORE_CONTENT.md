...
- [~] Content Studio now runs a deterministic authored-bank quality audit: reading is 115/115 unique normalized passage templates with 125 questions, 0 near-duplicate clusters, and 0 structural answer-choice failures; listening is 160/160 unique normalized templates with 0 near-duplicate clusters, with question-family and source-type counts visible. The reading generator adds several question families, while the listening generator varies prompts/scripts and removes prior exact and cross-level duplicates; all 160 current scenarios remain browser-TTS, so naturalness still needs human-speaker review.
