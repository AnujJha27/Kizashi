...
- [x] Content Studio has computed source coverage plus a live provenance-gap dashboard showing missing records and unknown source IDs by content category from the current source manifest; the authored Irodori sentence-pattern and N4 grammar IDs are registered, so the current bundled package has no unknown provenance IDs.
