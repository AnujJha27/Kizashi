...
- [x] 2026-09-06 provider-enrichment patch — Learn’s Marugoto Plus follow-up now shows its mapped provider-hosted Can-do activity catalog with source level, JLPT relevance, modality, and local opened tracking; because `a1.marugotoweb.jp` blocks framing, the app exposes the direct provider source without a blank iframe.
