...
- [~] Phase 15 — Mobile/accessibility/performance (global keyboard focus ring, skip-to-content target, focus entry/containment/restoration for the command palette, source viewer, and Studio review modal, Marugoto Plus iframe plus direct-source fallback, and responsive min-width-safe lesson/Journey grids are wired; final device/browser audit remains)
- [x] Phase 16 — Tests/build verification (25/25 direct Node test files pass; strict package QA, typecheck, and production build are green)
- [x] 2026-09-06 provider-enrichment patch — Learn’s Marugoto Plus follow-up now renders its seven-entry provider-hosted Can-do activity catalog with per-activity source level, JLPT relevance, modality, target/provenance metadata, local opened tracking, and a selected-activity iframe; “View in iframe” changes the existing frame to the chosen Can-do page, and the original-source link remains the fallback when the provider blocks headers.
- [~] 2026-09-06 kanji-writing foundation — kanji Entry pages and Practice Focus now mount a native pointer-event trainer with Watch/Trace/Write modes, visible per-stroke number markers, a Show components toggle for verified component groups, optional next-stroke and positional component recognition checks, an optional useful-word session for fully covered multi-kanji words, bounded queue prioritization from current-lesson, due, mistake, unwritten, and explicit-target signals, and an occasional conditional Today writing detour; step controls, reset/undo/clear, cancellation-safe touch/stylus drawing, reduced-motion-safe current-stroke emphasis, local writing-familiarity persistence/backup/account-sync events, pinned KanjiVG `r20260714` Unicode-matching validation/URL helpers, a deterministic importer, a Content Studio coverage card that exposes component-group coverage, owner-editable provisional confusable pairs, release/license/attribution metadata, and the direct repository link, plus lazy Jisho/Tanoshii references through the shared source viewer. The checked-in KanjiVG manifest now has 254 ordered records for the canonical N5/N4 union; deployed sync verification and real mobile drawing verification remain open.

### Feature-to-code evidence

- [x] 2026-09-05 question-coverage checkpoint — the shared authored-bank report applies a three-question floor to supplemental reading families and Content Studio renders the underrepresented-family flags. Current evidence meets `3/3` or better for every supplemental family globally and within N5/N4; the signal keeps question diversity reviewable without auto-generating filler.

- [x] 2026-09-05 level-coverage checkpoint — the reading question-family floor is also summarized by N5/N4 so thin families are actionable at the target level; the current bank has no under-minimum supplemental family in either level after adding passage-grounded prompts.
...
- [x] 2026-09-05 quality checkpoint — the shared authored-bank report now flags reading distractors whose normalized text occurs in the passage, and Content Studio renders a bounded record/question-family/distractor review list plus N5/N4 source-length signals. The current bank has 34 affected question sets and 56 source-mentioned distractor choices, with 39/51 average source characters by level; this is structural evidence and keeps linguistic plausibility/native review open.

- [x] 2026-09-05 lexical review checkpoint — Content Studio now renders bounded N5/N4 vocabulary and kanji queues for partial/missing records with coverage status, teaching status, and source IDs, alongside the existing aggregate and level-disagreement summaries; it remains a review surface rather than a release claim.
...
- [x] **Today interruption resume** — an interrupted local Today flow restores by date, lesson, and target level, and the Journey path exposes the next stage plus completed-stage count before resuming; `readLessonState` also migrates a matching legacy `michi.current-lesson` record so completed pre-migration lessons do not leave the next route stop locked.
...
- [~] **Original reading practice** — `data/original-reading-bank.json`, `scripts/generate_original_reading_bank.py`, `scripts/merge_openjlpt_staging.py`, `lib/curriculum.ts`, and `components/learning/reading-panel.tsx`; the 115 N5/N4 original passages now provide 166 questions across 11 families, with every supplemental family meeting the three-question floor globally and independently at N5/N4, and the 27 information-retrieval assets pair generated no-text raster scene support with explicit accessible HTML visual formats for menus, notices, timetables, posters, and related practical material. The shared quality audit now reports 31 topic counts, 2 linked grammar-pattern signatures as a structural proxy, 166/166 unique correct choices, N5 difficulty metadata averaging 2 and N4 averaging 4, linked lexical-load ranges, 34 question sets with 56 source-mentioned distractor choices for review, and 113 exact passage-answer echoes; Content Studio now renders the per-level difficulty signal and a bounded list showing record IDs, question families, and echoed answers for review. Shared Japanese rendering keeps furigana at a readable scalable size with a 1rem floor across lesson, reading, and anatomy cards, and `ReadingPanel` exposes A−/A＋ passage sizing from 16–28px so the passage and its readings scale together. They are assigned to learner-facing reading lessons, the shared release package, and the existing PracticePlayer; distractor plausibility, level calibration, and native review remain open.
- [~] **Original listening practice** — `data/original-listening-bank.json`, `scripts/generate_original_listening_bank.py`, `scripts/audit_content_quality.mjs`, `lib/content-quality-core.js`, and `lib/curriculum.ts`; the 160 N5/N4 original scenarios cover task-based, key-point, verbal-expression, and quick-response families across 20 context tags in the shared release package, use deterministic constraint filters plus three dialogue/question shapes, and give N4 a distinct four-turn dependency path. The quality audit reports 160 unique normalized templates, 20 contexts, 0 near-duplicate clusters, 80 three-turn `A-B-A` and 80 four-turn `A-B-A-B` profiles, N5 difficulty metadata averaging 2 and N4 averaging 4, 160/160 unique correct choices, and 94 transcript-answer echoes; Content Studio now renders the per-level difficulty signal and a bounded list showing record IDs, question families, and echoed answers for review, alongside the 3-line/44-character N5 versus 4-line/91-character N4 transcript signal. Original listening transcripts remain browser TTS; sentence-sized lesson examples now try licensed Tatoeba/Commons recordings first, with browser speech fallback.
...
- [x] **Immersion feed/player split** — `components/learning/immersion-surface.tsx` and `components/learning/immersion-player.tsx`; feed selection and focused playback are separate states, the reading/listening feed rotates its bounded first page per browser session, the shared YouTube catalog presents Shun, Moshi Moshi Yusuke, Natural Japanese/NIJ, and Cure Dolly with provider-role labels and channel fallback, Teppei exposes RSS-selected native audio with original-site fallback, and the Detours shelf exposes bounded activity maps for the curated Marugoto/JFS/KC Yom Yom/Hirogaru/OJAD provider entries.
- [x] **Immersion provider review signals** — provider video cards persist local `Clear` / `Shaky` / `Missed` comprehension reviews, include them in opt-in account sync when enabled, reuse the existing study-later collection for `Save · あとで`, and keep both signals separate from approval or readiness; Content Studio checks the bounded YouTube provider feeds and keeps the official source fallback visible when a feed is unavailable.
...
- [x] **Interest-aware detours** — `components/profile/profile-settings.tsx`, `lib/interest-core.js`, and `components/learning/immersion-surface.tsx`; learners can save up to three optional topics, which rank free reading/listening and provider catalogs after the existing learning signals, while current Journey-area tags prioritize relevant source families.
- [x] **Guided output lane** — `lib/output-core.js`, `components/learning/output-practice.tsx`, and the shared review scheduler; Real life immersion derives 80 speaking situations, 60 writing prompts, 119 pragmatic contexts, 296 vocabulary collocations, and 40 balanced N5/N4 casual-vs-polite recognition activities (20 per level; 10 casual / 10 polite) from released records, with model audio, immediate shared-review due-order refresh, and namespaced persistent self-rating without changing JLPT readiness.
...
- [x] **External-source viewer and fallback** — `components/learning/external-source-viewer.tsx`, `components/learning/external-source-launcher.tsx`, and `lib/external-resources-runtime.js`; frame, native media, and original-source fallback paths are wired, blocked/blank provider frames now explain the direct-source fallback above a bounded frame, and all YouTube/RSS provider-feed failures show a calm status while retaining the official-source fallback.
...
- [x] **Japanese-aware search and contextual actions** — `components/shell/command-palette.tsx`, `app/api/search/route.ts`, `components/library/study-later.tsx`, and item action components; ReadingPanel inline word lookup now offers Study later without leaving the activity.
- [x] **Content completeness audit** — `components/content/completeness-dashboard.tsx`, `components/content/content-record-editor.tsx`, `components/content/content-studio.tsx`, `components/content/source-coverage.tsx`, `lib/content-completeness-core.js`, and `lib/content-quality-core.js` report quantity, lesson placement, required-field completeness, review workflow status, unique reading/listening contexts, grammar-depth readiness, the grammar lesson-contract audit, normalized template diversity, near-duplicate review signals, visual listening-question count, separate pronunciation/dictation counts, output-bank counts, and human-backed versus synthetic/fallback audio coverage from the live package/bank; Content Studio exposes grammar aliases and mini-contexts for editing/review, its provenance dashboard registers the Irodori sentence-pattern and authored N4 grammar source IDs instead of flagging those records as unknown, repairs matching stale local-draft IDs from the current package, replaces stale coverage input when the full package is explicitly loaded, and only labels the package fully connected when no record has missing or unknown source IDs; the N5/N4 grammar depth gate and the broader grammar contract audit are now 116/116.
- [x] **Content completeness audit** — `components/content/completeness-dashboard.tsx`, `components/content/content-record-editor.tsx`, `components/content/content-studio.tsx`, `lib/content-completeness-core.js`, and `lib/content-quality-core.js` report quantity, lesson placement, required-field completeness, review workflow status, unique reading/listening contexts, grammar-depth readiness, the grammar lesson-contract audit, normalized template diversity, near-duplicate review signals, visual listening-question count, separate pronunciation/dictation counts, and output-bank counts from the live package/bank; Content Studio exposes grammar aliases and mini-contexts for editing/review, its provenance dashboard registers the Irodori sentence-pattern and authored N4 grammar source IDs instead of flagging those records as unknown, and reading target-level links are resolved against the active merged catalogs; the N5/N4 grammar depth gate and the broader grammar contract audit are now 116/116.
...
- [~] **Vocabulary context contract** — `lib/content-completeness-core.js`, `lib/content-validation.ts`, and the Studio completeness surface now audit example depth, collocations, related words, persisted audio, approved contextual/paraphrase links, pending draft counts, N4 usage assessments, and structural collocation quality; all 169 current vocabulary records now have at least two authored examples, 338 deterministic contextual/paraphrase drafts are staged for human review from existing vocabulary facts, the pending Studio question queue can filter drafts by question family, valid non-rejected generated drafts now enter Practice as visibly provisional questions with encounter-time review controls, high-frequency records with fewer than two examples receive a review warning, and approval/readiness metrics still exclude drafts until explicit review metadata is present.
- [~] **Grammar assessment depth audit** — the same dashboard reports approved grammar question count, context-set-aware normalized contexts, form-selection, sentence-composition, sentence-ordering, text-grammar, contrast-cluster, N5/N4 counts, and pending review counts; validation and the direct Grammar contract queue surface missing grammar aliases and dedicated mini-contexts, while valid non-rejected draft questions can now enter Practice with a visible provisional badge and post-answer review controls. Approval/readiness metrics remain separate, and Content Studio renders each pending draft's persisted context ID and passage text in the review card. The current persisted authored bank has 92 approved N5 grammar questions across 92 contexts and 55 approved N4 questions across 55 contexts (including 6 contrast-cluster questions), plus 125 text-grammar drafts pending review (50 N5 / 75 N4), each with persisted context IDs/text for review, so it does not yet meet the learner-ready N5/N4 text-grammar targets.
...
- [~] **Visual refinement/card density** — Immersion and Practice navigation plus reading/listening, Irodori activities, source detours, Tadoku, Aozora, Journey module metrics, and the Studio source/review queues use compact/editorial layouts without nested shelf-card grids; the Immersion listening discovery feed now uses lazy generated scene thumbnails, while the shell/Journey hero keeps eight area-specific generated-raster WebP scenes, Today maps eight distinct generated scenes to those areas, and Learn uses a separate generated station-opening scene, with area-aware atmosphere, progression stages, deliberate desktop/mobile focal points, restrained completion transitions, and a dark lower landscape stop that protects Journey route-label contrast; screenshot review and the final U65/U66/U70 audit remain.
- [~] **Mobile/accessibility/performance** — responsive navigation, deferred module loading, bounded Practice questions, a dedicated Studio route fallback, learner routes that fetch the released package without synchronously parsing the large local Studio draft, idle-scheduled Studio draft/question validation work, memoized 8k-item Studio scans, page-local Studio ranking, bounded Practice furigana construction, one-record full-index Studio inspection, bounded question review, explicit html-owned document vertical scrolling with compatible horizontal clipping, scroll-safe overlays, immediate route transition feedback, disabled heavy-route prefetching, bounded Studio source-register loading, bounded private-book loading, responsive `minmax(0, …)` lesson/Journey grids, global keyboard focus rings, a skip-to-content target, a fresh service-worker shell revision, and lazy async decoding plus a 350 KB byte-budget check for world raster scenes are in code; YouTube provider feeds and provider Detours activity maps are bounded/cached, and the final cross-surface device/browser audit remains a follow-up.
- [~] **Persistence beyond the browser** — local resume/progress works through `lib/session.ts`; the opt-in account snapshot now includes default lesson state and Continue state alongside review/progress data. Supabase migration `0020_learning_context_fields.sql`, both authored-expansion blocks in `supabase/seed.sql`, the SQL renderer, and the remote loader now preserve and idempotently update grammar aliases/mini-context plus reading visual formats/questions; applying and verifying the migration on the deployed project remains.
- [x] **Verification** — strict package QA, `tsc --noEmit`, the production build, and all 25 direct Node test files pass.
- [~] **Remaining follow-up** — final all-surface accessibility/performance validation, including checking that faded scaffolding remains legible at each breakpoint. Dedicated area visuals, the tall Journey map and lesson tracker, progression-aware profile portrait evolution, and the N5→N4 “new road” plus restrained area-completion transitions are now learner-visible; account sync now keeps a local outbox for failed writes and retries it on reconnect; the service worker caches the released learner package after first load and is now on shell revision v13 so deployed clients refresh the updated learner visuals and Studio provenance surface; private review/admin and sync APIs remain online-only.
