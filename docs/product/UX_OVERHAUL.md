...
- [~] **Original reading practice** — `data/original-reading-bank.json`, `scripts/generate_original_reading_bank.py`, `scripts/merge_openjlpt_staging.py`, and `lib/curriculum.ts`; the 115 N5/N4 original passages now provide 125 questions across short-detail, mid-length, information-retrieval, main-idea, sequence, condition-detail, and task-based-response families, assigned to learner-facing reading lessons, the shared release package, and the existing PracticePlayer. Broader family balance and native review remain open.
