...
- [~] 2026-09-06 kanji-writing foundation — kanji Entry pages and Practice Focus now mount a native pointer-event trainer with Watch/Trace/Write modes, visible per-stroke number markers, step controls, reset/undo/clear, cancellation-safe touch/stylus drawing, reduced-motion-safe current-stroke emphasis, local writing-familiarity persistence/backup/account-sync events, pinned KanjiVG `r20260714` Unicode-matching validation/URL helpers, a deterministic importer, a Content Studio coverage card that exposes release/license/attribution metadata plus the direct repository link, and lazy Jisho/Tanoshii references through the shared source viewer. The checked-in KanjiVG manifest now has 254 ordered records for the canonical N5/N4 union; deployed sync verification and real mobile drawing verification remain open.
