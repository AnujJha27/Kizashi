...
- [~] Phase 10 — Adaptive scaffolding (core furigana, grammar formation, production model, and lesson-listening transcript reveals are wired; raw/reference field fading remains)
...
- [~] **Furigana/scaffolding** — `components/learning/japanese-text.tsx`, `components/learning/reading-panel.tsx`, `components/learning/lesson-player.tsx`, `components/learning/learner-assistant.tsx`, and `components/practice/local-practice.tsx` implement shared `Always`/`Unknown`/`Tap`/`Hide` behavior with per-word tap reveal; LessonPlayer fades experienced grammar formation and listening transcripts to explicit support buttons, while Practice keeps its bounded question bank and derives its reading index from the bounded visible queue. Production models are already explicit reveals; remaining work is limited to a few raw/reference renderers.
