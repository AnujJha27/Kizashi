...
- [x] **External-source viewer and fallback** — `components/learning/external-source-viewer.tsx`, `components/learning/external-source-launcher.tsx`, and `lib/external-resources-runtime.js`; frame, native media, and original-source fallback paths are wired, blocked/blank provider frames now explain the direct-source fallback above a bounded frame, and Shun/Teppei provider-feed failures show a calm status while retaining the official-source fallback.
