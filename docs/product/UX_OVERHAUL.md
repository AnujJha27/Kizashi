...
- [x] **Original listening practice** — `data/original-listening-bank.json`, `scripts/generate_original_listening_bank.py`, `scripts/merge_openjlpt_staging.py`, and `lib/curriculum.ts`; the 160 N5/N4 original scenarios cover task-based, key-point, verbal-expression, and quick-response families in the shared release package and use the existing BrowserSpeechProvider fallback.
