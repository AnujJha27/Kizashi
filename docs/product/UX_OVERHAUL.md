...
- [x] 2026-09-06 provider-enrichment patch — Learn’s Marugoto Plus follow-up now renders its mapped provider-hosted Can-do activity catalog with per-activity source level, JLPT relevance, modality, target/provenance metadata, local opened tracking, and the requested iframe; the frame panel names the private `browser/kizashi-private-frame-unlocker` reload path when the provider blocks headers, with the original-source link retained as fallback.
