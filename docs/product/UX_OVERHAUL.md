...
- [x] 2026-09-05 visual/accessibility patch — Journey uses a dedicated tall `/world/journey-map.webp` map scene, completed prior lessons unlock the next chapter’s first lesson from either persisted lesson completion or complete review records and the map refreshes on both lesson/review events, Learn’s lesson surface now uses the available content width, its lesson summary uses a dedicated `.lesson-summary` section with explicit full-width/auto-height sizing so it cannot inherit shell-sidebar `aside` styling, stays below the lesson column until the `2xl` breakpoint, and its progress labels cannot force horizontal overflow; shared furigana is readable at `.68em` across the shared and reading renderers; the content-aside background selector is scoped away from learner cards, and the private frame extension explicitly handles Marugoto Plus’s official `a1.marugotoweb.jp` frame host.
