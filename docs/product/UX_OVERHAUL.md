...
- [x] 2026-09-05 visual/accessibility patch — Journey uses a dedicated tall `/world/journey-map.webp` map scene, completed prior lessons unlock the next chapter’s first lesson, Learn’s lesson summary uses a dedicated `.lesson-summary` section with explicit full-width/auto-height sizing so it cannot inherit shell-sidebar `aside` styling, stays below the lesson column until the `xl` breakpoint, and shared furigana is readable at `.68em` across the shared and reading renderers; the content-aside background selector is scoped away from learner cards, and the private frame extension explicitly handles Marugoto Plus’s official `a1.marugotoweb.jp` frame host.
