...
- [~] **Grammar coverage audit** — `scripts/build_grammar_coverage_registry.mjs`, `data/grammar-coverage-union.json`, and `components/content/grammar-coverage.tsx` expose multi-source raw patterns, canonical concept status, evidence source IDs/counts, level disagreements, aliases, duplicates, and unresolved source rows; forty-seven dedicated N4 expansion concepts, five N4 life-bridge concepts, and six additional N5 concepts now have lesson placement, with ten linked contrast clusters for direction, benefit perspective, inference/appearance, conditionals, potential ability, passive/potential, causative, change/habit/decision, modality/inference, and quotation/thought forms, while the registry remains an evidence baseline rather than completed N5/N4 grammar coverage.
