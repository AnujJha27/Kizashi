...
- [~] 2026-09-06 kanji-writing foundation — kanji Entry pages now mount a native pointer-event trainer with Watch/Trace/Write modes, step controls, reset/undo/clear, reduced-motion-safe current-stroke emphasis, KanjiVG validation/URL helpers, a deterministic importer, and lazy Jisho/Tanoshii references through the shared source viewer. The checked-in KanjiVG manifest is metadata-only until the public source import is permitted, so populated stroke paths, Practice Focus routing, and writing-state persistence remain open.
