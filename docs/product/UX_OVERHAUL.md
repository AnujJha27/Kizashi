...
- [~] Furigana preference support now honors `Always`, `Unknown`, `Tap`, and `Hide` in the shared `JapaneseText` and `ReadingPanel` renderers; Immersion list previews, Practice text, and Content Studio review fields now use the same Japanese-aware renderer, `Tap` reveals the clicked word, and challenge mode remains strict. Complete fading for every raw/reference renderer is not finished.
...
- [x] **Mobile/accessibility/performance** — responsive navigation, deferred module loading, bounded Practice questions, a dedicated Studio route fallback, idle-scheduled Studio draft work, memoized 8k-item Studio scans, cached text-scoped furigana candidates over the full learner module, bounded question review, and scroll-safe overlays are in code; the final cross-surface audit remains a follow-up.
