...
Updated 2026-09-03. These checkboxes describe what is currently present in the
...
- [~] Phase 15 — Mobile/accessibility/performance (global keyboard focus ring and skip-to-content target are now wired; final device/browser audit remains)
...
- [x] **Original listening practice** — `data/original-listening-bank.json`, `scripts/generate_original_listening_bank.py`, `scripts/merge_openjlpt_staging.py`, and `lib/curriculum.ts`; the 160 N5/N4 original scenarios cover task-based, key-point, verbal-expression, and quick-response families in the shared release package, use deterministic constraint filters for targeted generation, and use the existing BrowserSpeechProvider fallback.
...
- [x] **Immersion feed/player split** — `components/learning/immersion-surface.tsx` and `components/learning/immersion-player.tsx`; feed selection and focused playback are separate states, and the reading/listening feed rotates its bounded first page per browser session while retaining progressive loading.
- [x] **Guided output lane** — `components/learning/output-practice.tsx`; Real life immersion includes speaking, writing, pragmatic interpretation, natural-phrase self-checks, model audio, and learner self-rating without changing JLPT readiness.
...
- [~] **Remaining follow-up** — complete fading scaffolding and the final all-surface accessibility/performance pass. Account sync now keeps a local outbox for failed writes and retries it on reconnect; the service worker caches the released learner package after first load and revisions are forced for deployed clients; private review/admin and sync APIs remain online-only.
