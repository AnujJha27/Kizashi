...
- [~] Furigana preference support now honors `Always`, `Unknown`, `Tap`, and `Hide` in the shared `JapaneseText` and `ReadingPanel` renderers; Immersion list previews, Practice text, Library entries, Knowledge Map, and Content Studio review fields now use the same Japanese-aware renderer, `Tap` reveals the clicked word, and challenge mode remains strict. Complete fading for every raw/reference renderer is not finished.
