...
- [~] **Original reading practice** — `data/original-reading-bank.json`, `scripts/generate_original_reading_bank.py`, `scripts/merge_openjlpt_staging.py`, `lib/curriculum.ts`, and `components/learning/reading-panel.tsx`; the 115 N5/N4 original passages now provide 143 questions across 11 families, with each of reason, reference, simple-inference, and appropriate-action represented at least three times, and the 27 information-retrieval assets pair generated no-text raster scene support with explicit accessible HTML visual formats for menus, notices, timetables, posters, and related practical material. The shared quality audit now reports 32 topic counts, 2 linked grammar-pattern signatures as a structural proxy, 143/143 unique correct choices, linked lexical-load ranges, structural distractor signals, and 104 exact passage-answer echoes, with a bounded Content Studio list showing record IDs, question families, and echoed answers for review. They are assigned to learner-facing reading lessons, the shared release package, and the existing PracticePlayer; distractor plausibility, level calibration, and native review remain open.
