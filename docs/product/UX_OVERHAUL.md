...
- [~] 2026-09-06 kanji-writing foundation — kanji Entry pages and Practice Focus now mount a native pointer-event trainer with Watch/Trace/Write modes, step controls, reset/undo/clear, reduced-motion-safe current-stroke emphasis, local writing-familiarity persistence/backup/account-sync events, KanjiVG validation/URL helpers, a deterministic importer, a Content Studio coverage card, and lazy Jisho/Tanoshii references through the shared source viewer. The checked-in KanjiVG manifest is metadata-only until the public source import is permitted, so populated stroke paths and deployed sync verification remain open.
...
- [~] **Remaining follow-up** — complete fading scaffolding and the final all-surface accessibility/performance pass. Dedicated area visuals, the tall Journey map, progression-aware profile portrait evolution, and the N5→N4 “new road” plus restrained area-completion transitions are now learner-visible; account sync now keeps a local outbox for failed writes and retries it on reconnect; the service worker caches the released learner package after first load and is now on shell revision v12 so deployed clients refresh the updated learner visuals and Studio provenance surface; private review/admin and sync APIs remain online-only.
