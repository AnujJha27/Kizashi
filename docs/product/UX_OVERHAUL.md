...
- [~] **Furigana/scaffolding** — `components/learning/japanese-text.tsx`, `components/learning/reading-panel.tsx`, `components/learning/lesson-player.tsx`, `components/learning/learner-assistant.tsx`, and `components/practice/local-practice.tsx` implement shared `Always`/`Unknown`/`Tap`/`Hide` behavior with per-word tap reveal; LessonPlayer now fades experienced grammar formation to an explicit hint button, while Practice keeps its bounded question bank and derives its reading index from the bounded visible queue, and remaining transcript/production/raw-reference fading is still partial.
