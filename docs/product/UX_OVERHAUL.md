...
- [x] **Immersion provider review signals** — provider video cards persist local `Clear` / `Shaky` / `Missed` comprehension reviews, include them in opt-in account sync when enabled, and keep them separate from approval or readiness; Content Studio checks the bounded YouTube provider feeds and keeps the official source fallback visible when a feed is unavailable.
