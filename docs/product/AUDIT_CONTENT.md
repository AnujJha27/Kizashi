...
- [x] The package source manifest registers both `irodori-sentence-patterns` and `michi-authored-n4-grammar`; the Content Studio provenance audit distinguishes selective external explanation links from package-wide source registration, merges the active loaded package manifest with the server registry so valid staged IDs are not falsely reported as unknown, repairs matching stale local-draft source IDs without inventing provenance, and only reports a fully connected package when every record has no missing or unknown source ID.
