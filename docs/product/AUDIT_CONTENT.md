...
- [~] The shared provider manifest now exposes bounded activity maps for Marugoto Plus (three mapped entries link to confirmed provider-hosted Can-do pages for greetings, ordering, and directions), JFS Reading Activities (menus/notices/prices), KC Yom Yom (graded-reader topics, audio/length/progress fields), Hirogaru (interest topics), and OJAD (optional prosody references), with source-level, JLPT-relevance, target, and provenance fields kept separate. Broader provider catalogs, validated per-activity source audits, and deeper provider-specific activities remain partial.
