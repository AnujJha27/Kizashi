...
- [~] `lib/content-completeness-core.js` now audits the full grammar contract alongside the existing depth gate, while `lib/content-validation.ts` warns on missing aliases and dedicated mini-contexts. Content Studio exposes aliases and mini dialogue/context fields in both the editor and review modal, plus a direct Grammar contract queue for the missing records. The current 116-item learner package has 42/116 records with persisted aliases and mini-contexts (27 N5 / 15 N4), 111/116 with two linked assessment contexts, 23 reading appearances, 5 listening appearances, and 42/116 contract-ready records. `data/grammar-contract-fields.json` adds explicit authored fields for the 27 foundational N5 records; the remaining alias/context work stays visible rather than being inferred from generic examples.
