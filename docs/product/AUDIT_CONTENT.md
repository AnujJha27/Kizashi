...
- [~] The current registry reports N5 224 raw patterns / 46 canonical concepts (23 complete, 18 partial, 5 missing, 80 unresolved) and N4 188 raw patterns / 70 canonical concepts (14 complete, 51 partial, 5 missing, 73 unresolved). Sixty-five N4 concepts are authored in the dedicated expansion lessons, five additional N4 bridge concepts remain in the life module, and six N5 concepts are authored from exact OpenJLPT evidence; fourteen explicit contrast clusters now connect direction, benefit perspective, inference/appearance, conditionals, potential ability, passive/potential, causative forms, change/habit/decision forms, modality/inference forms, quotation/thought forms, nominalization/capability forms, contrast/conjunction forms, time/aspect forms, and ease/difficulty forms. The registry still keeps source-level disagreements and unresolved rows visible. It now includes 153 reviewed Irodori mapping references plus 21 curated OpenJLPT alias references and marks mapped source records resolved; level disagreements remain visible (N5 7, N4 16, including the Elementary 1 `V-る前に` evidence, the Pre-Intermediate evidence for `なくてもいい`, and the Starter-level basic giving/receiving evidence alongside the Elementary 1 quotation/thought and nominalization mappings and the two Elementary 1 てみる／すぎる mappings). This is an honest coverage baseline, not a claim of grammar completeness.

- [x] The package source manifest registers both `irodori-sentence-patterns` and `michi-authored-n4-grammar`; the base app/seed registry now also includes every staged import source ID plus KanjiVG. The Content Studio provenance audit distinguishes selective external explanation links from package-wide source registration, merges the active loaded package manifest with the server registry so valid staged IDs are not falsely reported as unknown, repairs matching stale local-draft source IDs without inventing provenance, and only reports a fully connected package when every record has no missing or unknown source ID.
...
- [~] The 125 authored text-grammar drafts remain pending human linguistic review for approved/readiness coverage, while structurally valid non-rejected generated questions can enter learner Practice with an explicit provisional badge and encounter-time review controls; `validatePracticeQuestions` still requires reviewer/timestamp metadata before a generated question can claim validated/approved status.
...
- [~] Kanji Entry pages, Library, and Practice Focus now expose the native Watch/Trace/Write trainer shell, pointer input, visible per-stroke number markers, a Show components toggle for source-derived component groups, optional next-stroke and positional component recognition checks, an optional useful-word session for fully covered multi-kanji words, a Library `漢字帳` notebook with current-journey/N5/N4/writing-due/weak/recent/not-written filters and compact writing-aware cards, bounded queue prioritization from current-lesson, due, mistake, unwritten, and explicit-target signals, and an occasional conditional Today writing detour, step/reset/undo/clear controls, cancellation-safe touch/stylus drawing, targeted Writing repair links for kanji/orthography misses, local writing-familiarity persistence with account-sync events, pinned KanjiVG `r20260714` Unicode-matching source validation helpers, and lazy Jisho/Tanoshii references through the shared source viewer. `scripts/import_kanjivg.py` now imports 254 ordered records covering the canonical N5/N4 union (125 N5 claims and 166 N4 claims, with 37 shared), with zero missing or unordered paths; Content Studio reports component-group coverage alongside the pinned release/license, attribution, and direct KanjiVG repository link plus stroke-path, writing-enabled, missing-path, and stroke-count-mismatch totals by N5/N4. Deployed sync verification and real mobile drawing audits remain open.
...
- [x] 2026-09-05 checkpoint: the shared report now flags distractor choices whose normalized text occurs in the source passage, and Content Studio shows a bounded record/question-family/distractor review list plus N5/N4 source-length signals. The current bank has 34 affected question sets and 56 source-mentioned distractor choices; this is structural evidence, not an automatic plausibility, length-calibration, or native-language verdict.
...
- [~] `lib/content-quality-core.js` now audits authored reading passages for normalized template uniqueness, near-duplicate clusters, topic counts, linked grammar-pattern signature diversity, question-family distribution, answer-choice presence/uniqueness/index validity, correct-answer uniqueness, estimated difficulty by level, linked vocabulary/grammar/kanji load ranges, structural distractor signals, exact passage-answer echoes, source type, and N5/N4 breakdown. The current 115-passage bank provides 166 questions across 11 families, with every supplemental family at least three times globally and within each level, is 115/115 unique with 0 near-duplicate clusters, 0 structural answer-choice failures, 166/166 unique correct choices, N5 difficulty metadata averaging 2 and N4 averaging 4, and 113 passage-answer echoes surfaced with bounded record/question-family/answer review evidence; Content Studio renders the per-level difficulty signal, while topic, grammar/vocabulary calibration, distractor plausibility, and native review remain open.
...
- [x] 2026-09-05 checkpoint: reading distractor source-overlap review and N5/N4 source-length signals are now visible in the shared quality report and Content Studio, with 34 affected question sets, 56 source-mentioned distractor choices, and 39/51 average source characters in the current bank. These are structural signals and keep human plausibility review open.
...
- [x] 2026-09-05 checkpoint: `lib/content-quality-core.js` now reports declared practical-reading format counts, and Content Studio renders the distribution. The current 27 information-retrieval readings cover 17 formats; this verifies format coverage, not source realism or native-language quality.

- [~] The 27 authored information-retrieval readings now carry explicit visual formats and render generated WebP scene support plus Kizashi-owned accessible HTML reading aids for notices, menus, timetables, schedules, posters, numbered directions, and other practical formats in `components/learning/reading-panel.tsx`. Japanese remains real text below the image/layout; the panel also provides A−/A＋ passage sizing from 16–28px, generated images contain no learner-facing text, intrinsic dimensions avoid layout shift, and source-backed realism, broader layout variety, and native review remain open.
...
- [~] The shared `ReadingPanel` now renders eight generated WebP scene assets reused by format plus Kizashi-original HTML layouts for the 27 practical reading assets, using declared format metadata, compact notice cards, menu/sale grids, timetable/schedule tables, numbered directions, Japanese typography, and structured rows while keeping the accessible text passage intact. It provides A−/A＋ passage sizing from 16–28px so Japanese text and furigana can be enlarged together. `lib/learning-visual-assets.ts` centralizes the generated-raster source/license/attribution metadata and both learner surfaces consume it. Images are atmospheric/no-text support, lazy-loaded with intrinsic dimensions; Japanese stays in HTML for accuracy and accessibility. Journey now keeps eight area-specific generated WebP scenes for the hero/shell, eight area-mapped Today scenes, a distinct wide lesson-opening scene, a separate tall current-lesson tracker scene, and the tall route-map scene. Broader layout variety and native review remain open.
...
- [~] This expansion preserves source IDs, target levels, frequency fields, model reveal/audio, and non-JLPT scoring. Self-ratings now reuse the shared review scheduler under namespaced output IDs without polluting canonical mistake queues, production hints fade after stable/strong or three recorded attempts with a manual support reveal, and the output lane refreshes its due ordering immediately on the shared review event. Register recognition now covers balanced N5/N4 starter lanes derived from authored listening lines; canonical grammar mapping for chunks and human-source pragmatics review remain follow-up quality work.
...
- [x] Moshi Moshi Yusuke, Natural Japanese/NIJ, and Organic Japanese with Cure Dolly have verified channel identities, normalized provider roles, separate provider/recommended level metadata, bounded shared YouTube catalogs, and learner-visible real-life/comprehensible/understanding presentation; one conservative Cure Dolly verb-forms mapping is attached as metadata, and Kizashi stores no video media.
...
- [x] Provider-hosted YouTube catalogs now expose local-first per-video comprehension signals (`Clear`, `Shaky`, `Missed`) that join opt-in account sync when enabled, and Content Studio reports cached feed health for the bounded provider set; failed feeds retain the official-channel fallback and do not affect source approval or readiness counts.
- [x] YouTube/RSS provider-feed failures now report a calm actionable status while preserving the official channel/site fallback.
...
- [~] The shared provider manifest now exposes bounded activity maps for Marugoto Plus (seven entries link to confirmed provider-hosted Can-do pages for greetings, ordering, directions, scripts, menus, homes, and events), JFS Reading Activities (menus/notices/prices), KC Yom Yom (graded-reader topics, audio/length/progress fields), Hirogaru (interest topics), and OJAD (optional prosody references), with source-level, JLPT-relevance, target, and provenance fields kept separate. Broader provider catalogs, validated per-activity source audits, and deeper provider-specific activities remain partial.
...
- [x] `components/content/completeness-dashboard.tsx`, `components/content/source-coverage.tsx`, and `lib/content-completeness-core.js` expose live package metrics for quantity, N5/N4 counts, Journey lesson assignment, required learner-field completeness, review workflow status, unique reading/listening contexts, N5/N4 reading/listening question families and duplicate clusters, reading answer-choice integrity, grammar assessment families/contexts, grammar prose consistency findings, vocabulary context-contract findings including pending contextual/paraphrase draft counts, structural collocation quality, human-backed versus synthetic/fallback audio coverage, and a grammar-depth gate (4 examples, 2 mistakes, and 2 reviewed practice links); the completeness core also computes the grammar lesson-contract audit. Visual listening-question count, the separate pronunciation bank (20 lessons / 60 discrimination items / 10 topics), derived dictation lanes/counts, and output-bank counts remain visible.
