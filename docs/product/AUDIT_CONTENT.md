...
- [x] The package source manifest registers both `irodori-sentence-patterns` and `michi-authored-n4-grammar`; the Content Studio provenance audit distinguishes selective external explanation links from package-wide source registration and merges the active loaded package manifest with the server registry so valid staged IDs are not falsely reported as unknown.
