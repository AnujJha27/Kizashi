...
- [~] Kanji Entry pages and Practice Focus now expose the native Watch/Trace/Write trainer shell, pointer input, step/reset/undo/clear controls, targeted Writing repair links for kanji/orthography misses, local writing-familiarity persistence with account-sync events, KanjiVG source validation helpers, a Content Studio stroke-path coverage card by N5/N4, and lazy Jisho/Tanoshii references through the shared source viewer. `scripts/import_kanjivg.py` now imports 254 ordered records covering the canonical N5/N4 union (125 N5 claims and 166 N4 claims, with 37 shared), with zero missing paths in that union; deployed sync verification and real mobile drawing audits remain open.
