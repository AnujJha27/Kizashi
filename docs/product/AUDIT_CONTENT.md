...
- [x] Provider-hosted YouTube catalogs now expose local per-video comprehension signals (`Clear`, `Shaky`, `Missed`) and Content Studio reports cached feed health for the bounded provider set; failed feeds retain the official-channel fallback and do not affect source approval or readiness counts.
