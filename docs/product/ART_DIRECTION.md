...
- [~] Lesson openings, area-completion next-stop handoffs, profile scenery, and restrained route/portrait overlays now use the shared visual manifest; its deliberate desktop/mobile focal points are wired through the shell, Today, Journey, and Learn surfaces. Eight dedicated generated-raster WebP area scenes remain the route/shell family, eight distinct generated `/world/today*.webp` scenes follow the current Today area, `/world/journey-map.webp` is a separate tall route-map composition, `/world/lesson-wide.webp` serves the focused lesson opening, and `/world/lesson-tracker-portrait.webp` is a separate tall current-lesson tracker scene; all carry role-level provenance metadata in `lib/journey-world-core.js`, while CSS overlays preserve progression-stage cues and the Journey map’s lower landscape ends in a dark terminal stop for label contrast. Authored practical reading and listening surfaces also use generated no-text WebP scene support with centralized provenance metadata in `lib/learning-visual-assets.ts`, and Immersion reuses listening scenes as lazy discovery thumbnails while provider roles separate real-life, comprehensible-input, and guided-understanding detours; accurate Japanese remains HTML; intrinsic dimensions, lazy async decoding, a 350 KB world-asset budget check, and hide-on-error fallbacks address the first asset-performance/source-failure pass, while screenshot review and the final mobile crop audit remain before this art direction can be marked complete.
- [x] Immersion provider cards keep visual role labels and local-first comprehension review controls subordinate to the provider-hosted media; opt-in sync preserves those signals when enabled, while feed-health fallback states use the same quiet status language and do not turn the source shelf into a dashboard.
