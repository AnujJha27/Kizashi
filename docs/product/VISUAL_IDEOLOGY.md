...
- [x] 2026-09-05 map refinement — the Journey route now uses a dedicated tall `/world/journey-map.webp` raster scene with its own manifest role and intrinsic dimensions, separate from the hero, Today, and lesson-opening assets; the Learn lesson summary is forced to one responsive full-width grid track and sized to its content so shell navigation styling cannot stretch it.
...
- [x] Shun, Moshi Moshi Yusuke, Natural Japanese/NIJ, and Cure Dolly provider immersion use bounded provider-hosted YouTube catalogs through the shared player; role labels separate real-life, comprehensible, and structural input, and no media is stored.
- [x] Immersion's authored listening discovery feed reuses the generated no-text scene family as lazy context thumbnails; audio and Japanese transcript remain the learning content.
- [x] Provider video discovery keeps the provider role, source level, Kizashi recommendation, and local-first `Clear` / `Shaky` / `Missed` comprehension signal visibly separate; the signal joins opt-in account sync when enabled, and feed failures preserve the official-source fallback rather than replacing provider media.
- [~] Practical reading material pairs eight generated no-text raster scene assets with Kizashi-owned format-aware HTML visual aids for authored notices, menus, schedules, posters, and numbered directions while preserving Japanese as accessible text; `ReadingPanel` also offers A−/A＋ passage sizing from 16–28px so text and furigana scale together. Generated learning images hide on load failure so the HTML aid remains usable, shared furigana stays readable with a scalable size and a 1rem floor across learning surfaces, `lib/learning-visual-assets.ts` centralizes generated-asset provenance, and richer format variety and screenshot review remain open.
- [~] The manifest now points the Journey hero, Today, Journey map, Journey current-lesson tracker, and Learn lesson opening at different generated-raster WebP scenes: eight area-specific scenes for the route/shell, eight mapped `/world/today*.webp` scenes for the current-place Today surface, a dedicated tall `/world/journey-map.webp` route composition, `/world/lesson-wide.webp` for the focused lesson opening, and tall `/world/lesson-tracker-portrait.webp` for the current-lesson tracker. Each role records source type, creator, license, attribution, focal point, and mood metadata. The route landscape and profile portrait use the area scenes plus restrained CSS overlays for arrival/lived-in/settled cues; area-completion “Arrived / Next stop” handoffs, the N5→N4 “new road” transition, profile scenery, and deliberate desktop/mobile focal points are wired. World `<img>` scenes use intrinsic dimensions, lazy async decoding, and a 350 KB automated byte-budget check; the Journey map’s landscape now keeps a dark terminal gradient stop so lower route labels do not wash out over the photo; practical reading also uses restrained HTML visual aids for authored notices, menus, schedules, and posters. Screenshot review and the final mobile/device visual audit remain.
...
generated `/world/lesson-wide.webp` lesson-opening scene
generated `/world/lesson-tracker-portrait.webp` tall lesson-tracker scene
