...
- [x] 2026-09-05 map refinement — the Journey route now uses a dedicated tall `/world/journey-map.webp` raster scene with its own manifest role and intrinsic dimensions, separate from the hero, Today, and lesson-opening assets; the Learn lesson summary uses an explicitly sized full-width-until-`2xl` section so shell navigation styling cannot stretch it.
