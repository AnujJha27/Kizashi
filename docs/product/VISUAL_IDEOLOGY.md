...
- [~] Practical reading material pairs generated no-text raster scene support with Kizashi-owned format-aware HTML visual aids for authored notices, menus, schedules, posters, and directions while preserving Japanese as accessible text; `lib/learning-visual-assets.ts` centralizes generated-asset provenance, and richer format variety and screenshot review remain open.
- [~] The manifest now points each mapped area at a distinct repo-owned SVG environment variant under `public/world/` and records role, source type, creator, license, attribution, focal point, and mood metadata. The route landscape and profile portrait add native station, storefront, river, garden, library, and train landmarks plus arrival/lived-in/settled cues; lesson openings, area-completion “Arrived / Next stop” handoffs, the N5→N4 “new road” transition, profile scenery, and deliberate desktop/mobile focal points are wired. Practical reading also uses restrained HTML visual aids for authored notices, menus, schedules, and posters. Screenshot review and the final mobile/device visual audit remain.
