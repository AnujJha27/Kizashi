...
- [x] Provider video discovery keeps the provider role, source level, Kizashi recommendation, and local `Clear` / `Shaky` / `Missed` comprehension signal visibly separate; feed failures preserve the official-source fallback rather than replacing provider media.
